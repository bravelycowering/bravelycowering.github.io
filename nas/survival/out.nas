...
#mine
	...
	ifnot minetimer|>|0 jump #if_mpZGuGtdqIjIiOPw
	...
	#if_mpZGuGtdqIjIiOPw
	set minepos
	if toomuch jump #ifnot_ciPEGijVhWvZroES
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_ciPEGijVhWvZroES
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_xTLPaBuNiOlLzVwn
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_xTLPaBuNiOlLzVwn
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_VWLZzjNmHJMqXVVW
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_GNsGuRSJCrgAtTap
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_GNsGuRSJCrgAtTap
	set {runArg1} {{runArg1}}&0
	#while_dzGfsHHroyJNQquM
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_dzGfsHHroyJNQquM
quit

#input
	ifnot runArg1|=|"craft" jump #if_elqZySFSBRjDiBIU
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_hcMkuukVGCnqFvsR
			set craftArgs[1] 1
			setsplit craftArgs ,
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_JupnAnJaxPiYLvQc
				msg &cInvalid item name or ID
				quit
			#if_JupnAnJaxPiYLvQc
			call #getRecipeByOutput|recipeID|{blockID}
			ifnot recipeID|=|"" jump #if_ZpzqLMwBETbkGQli
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_ZpzqLMwBETbkGQli
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_hcMkuukVGCnqFvsR
		set i 0
		#while_aqmuoFJwRXQnJctz
			call #checkRecipeAfford|{i}|canAfford|1
			if canAfford msg {i}: {blocks[{recipes[{i}].output.id}].name} x{recipes[{i}].output.count}
			setadd i 1
		if i|<|{recipes.Length} jump #while_aqmuoFJwRXQnJctz
		quit
	#if_elqZySFSBRjDiBIU
	set i 0
	msg &eResources:
	#while_vtigwNExdMZepVRE
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_vtigwNExdMZepVRE
	msg &eTools:
...
#doCraft
	...
	ifnot canAfford jump #if_DVHQxNVkOhCfkqHV
		set j 0
		#while_NQalWTgSlwNiHlra
			...
			setadd j 1
		if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_NQalWTgSlwNiHlra
...
#checkRecipeAfford
	set j 0
	#while_CPljigTynNgIaUiv
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		setmul count {runArg3}
		ifnot count|>|{inventory[{id}]} jump #if_RBEwIhEqBXvEVxvV
			set {runArg2} false
			quit
		#if_RBEwIhEqBXvEVxvV
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_CPljigTynNgIaUiv
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_ZNNYCLbTfbWmJRmo
		set {runArg1} {runArg2}
		quit
	#ifnot_ZNNYCLbTfbWmJRmo
	set i 0
	#while_jrmhetsTbwodWvgi
		ifnot blocks[{i}].name|=|runArg2 jump #if_NILmTURoTaauMFPV
			set {runArg1} {i}
			quit
		#if_NILmTURoTaauMFPV
		setadd i 1
	if i|<|{blocks.Length} jump #while_jrmhetsTbwodWvgi
quit

#getRecipeByOutput
	set {runArg1}
	set i 0
	#while_vYUWnarwESONZnld
		ifnot recipes[{i}].output.id|=|runArg2 jump #if_JPXrxbiBLTDRonhk
			set {runArg1} {i}
			quit
		#if_JPXrxbiBLTDRonhk
		setadd i 1
	if i|<|{recipes.Length} jump #while_vYUWnarwESONZnld
quit
