...
#mine
	...
	ifnot minetimer|>|0 jump #if_MshFmDlRfxDjOIsz
	...
	#if_MshFmDlRfxDjOIsz
	set minepos
	if toomuch jump #ifnot_fvrNVapMcyUmOThZ
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_fvrNVapMcyUmOThZ
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_vlfdLOskodKnDFtc
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_vlfdLOskodKnDFtc
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_UGoTqNEEeklvJggB
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_hUYCfKTLAGZEwFxT
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_hUYCfKTLAGZEwFxT
	set {runArg1} {{runArg1}}&0
	#while_CsevzmKIwtCsAJbA
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_CsevzmKIwtCsAJbA
quit

#input
	ifnot runArg1|=|"craft" jump #if_DBBNNEDgCdRDvCgh
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_whgdUgLMYyVXHuTk
			setsplit craftArgs " "
			call #getBlockByName|blockID|{craftArgs[0]}
			if blockID jump #ifnot_VujvOxNqaTrbXkSJ
				msg &cInvalid item name or ID
				quit
			#ifnot_VujvOxNqaTrbXkSJ
			msg block id {blockID}
			quit
		#ifnot_whgdUgLMYyVXHuTk
		set i 0
		#while_inrIikqESubHkJWS
			call #checkRecipeAfford|{i}|canAfford
			if canAfford msg {i}: {blocks[{recipes[{i}].output.id}].name} x{recipes[{i}].output.count}
			setadd i 1
		if i|<|{recipes.Length} jump #while_inrIikqESubHkJWS
		quit
	#if_DBBNNEDgCdRDvCgh
	set i 0
	msg &eResources:
	#while_BSFVjJfPpjrrBphJ
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_BSFVjJfPpjrrBphJ
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	#while_oFVrloHmgpQnvbLZ
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		ifnot count|>|{inventory[{id}]} jump #if_kIpLUKLkPqMLrbAm
			set {runArg2} false
			quit
		#if_kIpLUKLkPqMLrbAm
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_oFVrloHmgpQnvbLZ
	set {runArg2} true
quit

#getBlockByName
	ifnot blocks[{runArg2}].name jump #if_VtBxIkDPRmMHxKnF
		set {runArg1} {runArg2}
		quit
	#if_VtBxIkDPRmMHxKnF
	set i 0
	#while_DEjxYYANpAxCpvGN
		ifnot blocks[{i}].name|=|{runArg2} jump #if_hpTwYWSbEHrKmDHD
			set {runArg1} {i}
			quit
		#if_hpTwYWSbEHrKmDHD
	if i|<|{blocks.Length} jump #while_DEjxYYANpAxCpvGN
quit
