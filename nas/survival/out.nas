...
#mine
	...
	ifnot minetimer|>|0 jump #if_XDfqIAGBwFFqOrhN
	...
	#if_XDfqIAGBwFFqOrhN
	set minepos
	if toomuch jump #ifnot_qhfnDtUErGgQsEuZ
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_qhfnDtUErGgQsEuZ
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_RtFcMUXHYPnoSaFu
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_RtFcMUXHYPnoSaFu
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_YvIryYGyvUBnwHtP
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_uxplKyeFjUkbhNRa
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_uxplKyeFjUkbhNRa
	set {runArg1} {{runArg1}}&0
	#while_CGihciEEXMCUjOTc
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_CGihciEEXMCUjOTc
quit

#input
	ifnot runArg1|=|"craft" jump #if_qsXfBZCoEKBSPalj
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_cEczmmNNnFLyXtxV
			set craftArgs[1] 1
			setsplit craftArgs *
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_HwYoZcdHouTqZUgc
				msg &cInvalid item name or ID
				quit
			#if_HwYoZcdHouTqZUgc
			call #getRecipeByOutput|recipeID|{blockID}
			ifnot recipeID|=|"" jump #if_CHWZSTrpkQOHWsPq
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_CHWZSTrpkQOHWsPq
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_cEczmmNNnFLyXtxV
		msg &eRecipes:
		set i 0
		#while_iqdkuLJuuSRuwdmq
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_QJFAXFVimKqScNkH
				msg &f> &6{blocks[{recipes[{i}].output.id}].name}&f (x{recipes[{i}].output.count}):
				set j 0
				#while_FfqLDrpIMAsiPBia
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_FfqLDrpIMAsiPBia
				msg {ingrediantList}
			#if_QJFAXFVimKqScNkH
			setadd i 1
		if i|<|{recipes.Length} jump #while_iqdkuLJuuSRuwdmq
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_qsXfBZCoEKBSPalj
	set i 0
	msg &eResources:
	#while_HBkkaSrmNLljiyGY
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_HBkkaSrmNLljiyGY
	msg &eTools:
...
#doCraft
	...
	ifnot canAfford jump #if_bIkcdzwiIibymZtE
		set j 0
		#while_uSqkxMgJwVvUzIlx
			...
			setadd j 1
		if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_uSqkxMgJwVvUzIlx
...
#checkRecipeAfford
	set j 0
	#while_gKHoYUUtCZpHrGnT
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		setmul count {runArg3}
		ifnot count|>|{inventory[{id}]} jump #if_THzpaADBWmESRWyY
			set {runArg2} false
			quit
		#if_THzpaADBWmESRWyY
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_gKHoYUUtCZpHrGnT
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_ZzEbrgHvUzZpatLG
		set {runArg1} {runArg2}
		quit
	#ifnot_ZzEbrgHvUzZpatLG
	set i 0
	#while_KsDqNMGSLLGhxvIt
		ifnot blocks[{i}].name|=|runArg2 jump #if_TDfKGXItcahEirLt
			set {runArg1} {i}
			quit
		#if_TDfKGXItcahEirLt
		setadd i 1
	if i|<|{blocks.Length} jump #while_KsDqNMGSLLGhxvIt
quit

#getRecipeByOutput
	set {runArg1}
	set i 0
	#while_RJiqOkAvJUnSfiJn
		ifnot recipes[{i}].output.id|=|runArg2 jump #if_BTUvCRETrhIYyXem
			set {runArg1} {i}
			quit
		#if_BTUvCRETrhIYyXem
		setadd i 1
	if i|<|{recipes.Length} jump #while_RJiqOkAvJUnSfiJn
quit
