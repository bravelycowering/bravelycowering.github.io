...
#mine
	...
	ifnot minetimer|>|0 jump #if_PthDUEgJiFKFusLa
	...
	#if_PthDUEgJiFKFusLa
	set minepos
	if toomuch jump #ifnot_IlwKDxggvYJUlaaV
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_IlwKDxggvYJUlaaV
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_acDbmUpGVxiYwCye
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_acDbmUpGVxiYwCye
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_NrDSMKJYLboVFePC
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_xOjuzCkOXNfLkUfJ
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_xOjuzCkOXNfLkUfJ
	set {runArg1} {{runArg1}}&0
	#while_HrVmyIUMuuPWfRut
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_HrVmyIUMuuPWfRut
quit

#input
	ifnot runArg1|=|"craft" jump #if_FhoRkmfuWXLPIAzN
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_sHbBNQofbYgsQtqH
			set craftArgs[1] 1
			setsplit craftArgs *
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_YlpzmTboRLcgmLpR
				msg &cInvalid item name or ID
				quit
			#if_YlpzmTboRLcgmLpR
			call #getRecipeByOutput|recipeID|{blockID}
			ifnot recipeID|=|"" jump #if_LqPEXIoGYrCHomPT
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_LqPEXIoGYrCHomPT
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_sHbBNQofbYgsQtqH
		msg &eRecipes:
		set i 0
		#while_ZZyLLzxGeeOJiFUz
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_DfVUgUCPTdXIrILD
				msg &f> &6{blocks[{recipes[{i}].output.id}].name}&f (x{recipes[{i}].output.count}):
				set j 0
				#while_UVUXzpLqnVNSkYhj
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_UVUXzpLqnVNSkYhj
				msg {ingrediantList}
			#if_DfVUgUCPTdXIrILD
			setadd i 1
		if i|<|{recipes.Length} jump #while_ZZyLLzxGeeOJiFUz
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_FhoRkmfuWXLPIAzN
	set i 0
	msg &eResources:
	#while_djacayyZIbhMXNxN
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_djacayyZIbhMXNxN
	msg &eTools:
...
#doCraft
	...
	ifnot canAfford jump #if_DVHjDPkLSoeKlkrr
		set j 0
		#while_RDUkpTAxtVrwSgvT
			...
			setadd j 1
		if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_RDUkpTAxtVrwSgvT
...
#checkRecipeAfford
	set j 0
	#while_tCxMzdXMrCmpxqJh
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		setmul count {runArg3}
		ifnot count|>|{inventory[{id}]} jump #if_SqkVnCCRgEGineWD
			set {runArg2} false
			quit
		#if_SqkVnCCRgEGineWD
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_tCxMzdXMrCmpxqJh
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_uKOTFXcvCxHHSzRc
		set {runArg1} {runArg2}
		quit
	#ifnot_uKOTFXcvCxHHSzRc
	set i 0
	#while_wjpJOJnxjqKlplnC
		ifnot blocks[{i}].name|=|runArg2 jump #if_EPtoaSdYxXXUilTs
			set {runArg1} {i}
			quit
		#if_EPtoaSdYxXXUilTs
		setadd i 1
	if i|<|{blocks.Length} jump #while_wjpJOJnxjqKlplnC
quit

#getRecipeByOutput
	set {runArg1}
	set i 0
	#while_DTBDTYwoJbpFueAz
		ifnot recipes[{i}].output.id|=|runArg2 jump #if_ThLNhHWFwlIkSzpW
			set {runArg1} {i}
			quit
		#if_ThLNhHWFwlIkSzpW
		setadd i 1
	if i|<|{recipes.Length} jump #while_DTBDTYwoJbpFueAz
quit
