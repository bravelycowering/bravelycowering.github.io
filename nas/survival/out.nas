...
#mine
	...
	ifnot minetimer|>|0 jump #if_ydaYTPdqweGqFXQo
	...
	#if_ydaYTPdqweGqFXQo
	set minepos
	if toomuch jump #ifnot_jHAAWzLnwBuZtKgp
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_jHAAWzLnwBuZtKgp
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_XCxWEYKPbraIsLKt
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_XCxWEYKPbraIsLKt
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_kSmjNDpHgUtiTffo
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_rjeLddufOzUscchc
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_rjeLddufOzUscchc
	set {runArg1} {{runArg1}}&0
	#while_IKsVMkSGTIUhCmED
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_IKsVMkSGTIUhCmED
quit

#input
	ifnot runArg1|=|"craft" jump #if_RSNcZZNMELBIDXmp
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_uHRhrBvhdvAJryjn
			setsplit craftArgs " "
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_kxDFnMtZJKsnkLdZ
				msg &cInvalid item name or ID
				quit
			#if_kxDFnMtZJKsnkLdZ
			msg block id {blockID}
			quit
		#ifnot_uHRhrBvhdvAJryjn
		set i 0
		#while_bFCdiplmNUTapaYi
			call #checkRecipeAfford|{i}|canAfford
			if canAfford msg {i}: {blocks[{recipes[{i}].output.id}].name} x{recipes[{i}].output.count}
			setadd i 1
		if i|<|{recipes.Length} jump #while_bFCdiplmNUTapaYi
		quit
	#if_RSNcZZNMELBIDXmp
	set i 0
	msg &eResources:
	#while_LfQVnRzeqmPVEfCa
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_LfQVnRzeqmPVEfCa
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	#while_MHcLsgZRieCscRgv
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		ifnot count|>|{inventory[{id}]} jump #if_DBhPxGsqDAsaitqS
			set {runArg2} false
			quit
		#if_DBhPxGsqDAsaitqS
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_MHcLsgZRieCscRgv
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_eyOkZQzKGtMedjGC
		set {runArg1} {runArg2}
		quit
	#ifnot_eyOkZQzKGtMedjGC
	set i 0
	#while_gcWRHEOqiPUiWnKj
		ifnot blocks[{i}].name|=|runArg2 jump #if_EMZjYzUMPPWsbFgp
			set {runArg1} {i}
			quit
		#if_EMZjYzUMPPWsbFgp
		setadd i 1
	if i|<|{blocks.Length} jump #while_gcWRHEOqiPUiWnKj
quit
