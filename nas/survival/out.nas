...
#mine
	...
	ifnot minetimer|>|0 jump #if_OKBYOZIDdTKUkxHr
	...
	#if_OKBYOZIDdTKUkxHr
	set minepos
	if toomuch jump #ifnot_xUdJSxieeKRiiVXh
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_xUdJSxieeKRiiVXh
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_XCefWQpibhzJgofW
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_XCefWQpibhzJgofW
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_DfpjGKynIOIYMyJK
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_CKzuvsWPwHsQFloL
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_CKzuvsWPwHsQFloL
	set {runArg1} {{runArg1}}&0
	#while_nhpVCtfuZJjiVEWz
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_nhpVCtfuZJjiVEWz
quit

#input
	ifnot runArg1|=|"craft" jump #if_UCOYVaPrjmCtuNCW
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_HqkfdsOEygDdyClp
			setsplit craftArgs " "
			call #getBlockByName|blockID|{craftArgs[0]}
			if blockID|=|"" jump #ifnot_OCwpInAnZYqcssgl
				msg &cInvalid item name or ID
				quit
			#ifnot_OCwpInAnZYqcssgl
			msg block id {blockID}
			quit
		#ifnot_HqkfdsOEygDdyClp
		set i 0
		#while_MgiBrWDITOyquiIk
			call #checkRecipeAfford|{i}|canAfford
			if canAfford msg {i}: {blocks[{recipes[{i}].output.id}].name} x{recipes[{i}].output.count}
			setadd i 1
		if i|<|{recipes.Length} jump #while_MgiBrWDITOyquiIk
		quit
	#if_UCOYVaPrjmCtuNCW
	set i 0
	msg &eResources:
	#while_TVUEkufsBOcFcnch
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_TVUEkufsBOcFcnch
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	#while_hdoILemOHyVidhsN
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		ifnot count|>|{inventory[{id}]} jump #if_FWtFsNPZlLkKtZlI
			set {runArg2} false
			quit
		#if_FWtFsNPZlLkKtZlI
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_hdoILemOHyVidhsN
	set {runArg2} true
quit

#getBlockByName
	if blocks[{runArg2}].name|=|"" jump #ifnot_MqzgVBnyuqtLdLzd
		set {runArg1} {runArg2}
		quit
	#ifnot_MqzgVBnyuqtLdLzd
	set i 0
	#while_bXmMvnwbiLqjzgWx
		ifnot blocks[{i}].name|=|runArg2 jump #if_RLpoBQFIGdkuQICp
			set {runArg1} {i}
			quit
		#if_RLpoBQFIGdkuQICp
		setadd i 1
	if i|<|{blocks.Length} jump #while_bXmMvnwbiLqjzgWx
quit
