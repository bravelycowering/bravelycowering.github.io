...
#mine
	...
	ifnot minetimer|>|0 jump #if_hKIZNLLwoFhZrocZ
	...
	#if_hKIZNLLwoFhZrocZ
	set minepos
	if toomuch jump #ifnot_JfZOteytJmaRtilU
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_JfZOteytJmaRtilU
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_RRuYYjwZvOZFMlaS
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_RRuYYjwZvOZFMlaS
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_HVrFmYwVnxuVLfMa
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_aWljiVIwJNcRHMbp
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_aWljiVIwJNcRHMbp
	set {runArg1} {{runArg1}}&0
	#while_xmAdzxAmuBDAEfJA
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_xmAdzxAmuBDAEfJA
quit

#input
	ifnot runArg1|=|"craft" jump #if_jFcNEpfJOZHihVDj
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_UTCoWQuPMUJmtLjl
			setsplit craftArgs " "
			call #getBlockByName|blockID|{craftArgs[0]}
			if blockID jump #ifnot_dAmNLemTnXLaGHIB
				msg &cInvalid item name or ID
				quit
			#ifnot_dAmNLemTnXLaGHIB
			msg block id {blockID}
			quit
		#ifnot_UTCoWQuPMUJmtLjl
		set i 0
		#while_ZHnQPkCjGxxSWKXS
			call #checkRecipeAfford|{i}|canAfford
			if canAfford msg {i}: {blocks[{recipes[{i}].output.id}].name} x{recipes[{i}].output.count}
			setadd i 1
		if i|<|{recipes.Length} jump #while_ZHnQPkCjGxxSWKXS
		quit
	#if_jFcNEpfJOZHihVDj
	set i 0
	msg &eResources:
	#while_drdmBHGWzATwJATN
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_drdmBHGWzATwJATN
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	#while_QWCzyZodmlEmzLbO
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		ifnot count|>|{inventory[{id}]} jump #if_gLwDwqnKVcbsxqlx
			set {runArg2} false
			quit
		#if_gLwDwqnKVcbsxqlx
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_QWCzyZodmlEmzLbO
	set {runArg2} true
quit

#getBlockByName
	if blocks[{runArg2}].name|=|"" jump #ifnot_eURYWZybPxbGPzJJ
		set {runArg1} {runArg2}
		quit
	#ifnot_eURYWZybPxbGPzJJ
	set i 0
	#while_bRkNKtEfOQZydVUF
		ifnot blocks[{i}].name|=|runArg2 jump #if_sbhtedmPdUslPmbC
			set {runArg1} {i}
			quit
		#if_sbhtedmPdUslPmbC
		setadd i 1
	if i|<|{blocks.Length} jump #while_bRkNKtEfOQZydVUF
quit
