...
#mine
	...
	ifnot minetimer|>|0 jump #if_gzfyeWKwSQtzOPGm
	...
	#if_gzfyeWKwSQtzOPGm
	set minepos
	if toomuch jump #ifnot_MfvfKYGMUNhhFcWq
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_MfvfKYGMUNhhFcWq
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_BExUGrBgcSvIwVKE
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_BExUGrBgcSvIwVKE
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_QzJfgjCNMNZLhOsH
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_IOMDvLltfxxINDcO
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_IOMDvLltfxxINDcO
	set {runArg1} {{runArg1}}&0
	#while_nehkENrAkThMHDeC
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_nehkENrAkThMHDeC
quit

#input
	ifnot runArg1|=|"craft" jump #if_CtohkImczOBhbjGw
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_HcRNRsYdsQcREmDO
			setsplit craftArgs " "
			call #getBlockByName|blockID|{craftArgs[0]}
			if blockID|=|"" jump #ifnot_fINZEqieVOwyyras
				msg &cInvalid item name or ID
				quit
			#ifnot_fINZEqieVOwyyras
			msg block id {blockID}
			quit
		#ifnot_HcRNRsYdsQcREmDO
		set i 0
		#while_nRCfhCDUeecYPexo
			call #checkRecipeAfford|{i}|canAfford
			if canAfford msg {i}: {blocks[{recipes[{i}].output.id}].name} x{recipes[{i}].output.count}
			setadd i 1
		if i|<|{recipes.Length} jump #while_nRCfhCDUeecYPexo
		quit
	#if_CtohkImczOBhbjGw
	set i 0
	msg &eResources:
	#while_DWcGeqPNEosBxBzb
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_DWcGeqPNEosBxBzb
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	#while_kTcjALqaSLXxsESM
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		ifnot count|>|{inventory[{id}]} jump #if_BbqBobJCGGzMocRH
			set {runArg2} false
			quit
		#if_BbqBobJCGGzMocRH
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_kTcjALqaSLXxsESM
	set {runArg2} true
quit

#getBlockByName
	if blocks[{runArg2}].name|=|"" jump #ifnot_EOSQsqnLnLWUJsry
		set {runArg1} {runArg2}
		quit
	#ifnot_EOSQsqnLnLWUJsry
	set i 0
	#while_uhNrVIvjIPHAVNjA
		ifnot blocks[{i}].name|=|runArg2 jump #if_DVTDxtuVbPZOAtmL
			set {runArg1} {i}
			quit
		#if_DVTDxtuVbPZOAtmL
		setadd i 1
	if i|<|{blocks.Length} jump #while_uhNrVIvjIPHAVNjA
quit
