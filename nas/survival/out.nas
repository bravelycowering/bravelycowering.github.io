...
#mine
	...
	ifnot minetimer|>|0 jump #if_cWslVWrIaCtFLEuF
	...
	#if_cWslVWrIaCtFLEuF
	set minepos
	if toomuch jump #ifnot_QAVBGQGTPTSNIynH
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_QAVBGQGTPTSNIynH
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_WOWyLjlnwZMRgKhE
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_WOWyLjlnwZMRgKhE
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_vBQGkdeijXObpZXW
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_AiuckOILCZFaoxib
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_AiuckOILCZFaoxib
	set {runArg1} {{runArg1}}&0
	#while_BYEEcMmiFPQRQnzi
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_BYEEcMmiFPQRQnzi
quit

#input
	ifnot runArg1|=|"craft" jump #if_dJnOHQAsITyqRjuJ
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_yeGJYJoPbrbzuAeR
			setsplit craftArgs " "
			call #getBlockByName|blockID|{craftArgs[0]}
			if blockID jump #ifnot_WQfDCicGbPOKVvdF
				msg &cInvalid item name or ID
				quit
			#ifnot_WQfDCicGbPOKVvdF
			msg block id {blockID}
			quit
		#ifnot_yeGJYJoPbrbzuAeR
		set i 0
		#while_eCrMziPhvCFmBPEi
			call #checkRecipeAfford|{i}|canAfford
			if canAfford msg {i}: {blocks[{recipes[{i}].output.id}].name} x{recipes[{i}].output.count}
			setadd i 1
		if i|<|{recipes.Length} jump #while_eCrMziPhvCFmBPEi
		quit
	#if_dJnOHQAsITyqRjuJ
	set i 0
	msg &eResources:
	#while_eOKZfzKTFKhdsdML
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_eOKZfzKTFKhdsdML
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	#while_WiMhqoShLEYjVXRa
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		ifnot count|>|{inventory[{id}]} jump #if_xCHLFWpbLBLvibpV
			set {runArg2} false
			quit
		#if_xCHLFWpbLBLvibpV
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_WiMhqoShLEYjVXRa
	set {runArg2} true
quit

#getBlockByName
	ifnot blocks[{runArg2}].name jump #if_dxNwYgwCjHYKkxxg
		set {runArg1} {runArg2}
		quit
	#if_dxNwYgwCjHYKkxxg
	set i 0
	#while_yfxmcmGZATazWEjV
		ifnot blocks[{i}].name|=|{runArg2} jump #if_kOsXNvxrPDhxsTrE
			set {runArg1} {i}
			quit
		#if_kOsXNvxrPDhxsTrE
		setadd i 1
	if i|<|{blocks.Length} jump #while_yfxmcmGZATazWEjV
quit
