...
#tick
	...
	ifnot blocks[{id}].kills jump #if_BRPXPTwxMhvZERLk
		msg your died
		kill
	#if_BRPXPTwxMhvZERLk
...
#mine
	...
	ifnot minetimer|>|0 jump #if_XvDKoDIjmbbHQTgP
	...
	#if_XvDKoDIjmbbHQTgP
	set minepos
	if toomuch jump #ifnot_KHoZoQhfZqNPcCET
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_KHoZoQhfZqNPcCET
...
#give
	ifnot isTool({runArg1}) jump #if_keFuVIdVNrMYrnJu
		set {runArg1} {runArg2}
		quit
	#if_keFuVIdVNrMYrnJu
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_KoPVWZbqmjxSVSOn
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_KoPVWZbqmjxSVSOn
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_jNhopzlPYtFyNvDn
		...
		#while_MtzcduUgZctwXyNU
			...
			ifnot canAfford jump #if_vnkBTOqtUZQcfoUR
				...
				#while_TEIUvuvbnOkmvVWP
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_TEIUvuvbnOkmvVWP
				msg {ingrediantList}
			#if_vnkBTOqtUZQcfoUR
			setadd i 1
		if i|<|{recipes.Length} jump #while_MtzcduUgZctwXyNU
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_gxToqIFVDPReAuxN
	set i 0
	msg &eResources:
	#while_WLVxbtQcEVEEnPAA
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_WLVxbtQcEVEEnPAA
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_rbXrgNTEzylPGBNS
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_rbXrgNTEzylPGBNS
	#while_nTPECOrcmNQplkXh
		...
		ifnot isTool({id}) jump #if_uRZffdQCNBgazKsO
			if {id}|>=|count quit
		#if_uRZffdQCNBgazKsO
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_nTPECOrcmNQplkXh
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_dVXTdUGQiUTeyAlT
		set {runArg1} {runArg2}
		quit
	#ifnot_dVXTdUGQiUTeyAlT
	set i 0
	#while_PBdLqHNldeXTIVqI
		ifnot blocks[{i}].name|=|runArg2 jump #if_qwriynhcuhQFTDjh
			set {runArg1} {i}
			quit
		#if_qwriynhcuhQFTDjh
		setadd i 1
	if i|<|{blocks.Length} jump #while_PBdLqHNldeXTIVqI
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_LxmDpcyjRNNoOcrA
		ifnot recipes[{i}].output.id|=|bid jump #if_BoOVICMrbfKiDQGo
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_QXEtrZNjkFCrPfFb
				set {pname} {i}
				quit
			#if_QXEtrZNjkFCrPfFb
		#if_BoOVICMrbfKiDQGo
		setadd i 1
	if i|<|{recipes.Length} jump #while_LxmDpcyjRNNoOcrA
quit
