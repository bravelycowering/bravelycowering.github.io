...
#mine
	...
	ifnot minetimer|>|0 jump #if_vQAdCwBVQoEVgihf
	...
	#if_vQAdCwBVQoEVgihf
	set minepos
	if toomuch jump #ifnot_zEwIYXLnzDjtRbCU
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_zEwIYXLnzDjtRbCU
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_ONlCLxpPauFVuXBx
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_ONlCLxpPauFVuXBx
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_oxEhBlBriirslDzR
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_xRFfymibcfPmmTCG
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_xRFfymibcfPmmTCG
	set {runArg1} {{runArg1}}&0
	#while_UVcUhehcpDBrbieQ
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_UVcUhehcpDBrbieQ
quit

#input
	ifnot runArg1|=|"craft" jump #if_pWXGmCfDQvtMutTL
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_ErLxBNIRNOqrGETn
			setsplit craftArgs ,
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_jwJPXcPFlvEvrvAc
				msg &cInvalid item name or ID
				quit
			#if_jwJPXcPFlvEvrvAc
			call #getRecipeByOutput|recipeID|{blockID}
			ifnot recipeID|=|"" jump #if_oHQHXUxVYJGMiBAn
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_oHQHXUxVYJGMiBAn
			msg recipe id {recipeID}
			quit
		#ifnot_ErLxBNIRNOqrGETn
		set i 0
		#while_pvJEiNvRdWusHxOH
			call #checkRecipeAfford|{i}|canAfford
			if canAfford msg {i}: {blocks[{recipes[{i}].output.id}].name} x{recipes[{i}].output.count}
			setadd i 1
		if i|<|{recipes.Length} jump #while_pvJEiNvRdWusHxOH
		quit
	#if_pWXGmCfDQvtMutTL
	set i 0
	msg &eResources:
	#while_wttHOzubVSzMkGsM
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_wttHOzubVSzMkGsM
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	#while_DaeRdDxgGGOWUIcE
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		ifnot count|>|{inventory[{id}]} jump #if_suhkzepwVnlwWixN
			set {runArg2} false
			quit
		#if_suhkzepwVnlwWixN
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_DaeRdDxgGGOWUIcE
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_vtFbDOpidYDfHJzB
		set {runArg1} {runArg2}
		quit
	#ifnot_vtFbDOpidYDfHJzB
	set i 0
	#while_DatBUvOFxSXzHkDO
		ifnot blocks[{i}].name|=|runArg2 jump #if_kAUHJNyANVHfwGka
			set {runArg1} {i}
			quit
		#if_kAUHJNyANVHfwGka
		setadd i 1
	if i|<|{blocks.Length} jump #while_DatBUvOFxSXzHkDO
quit

#getRecipeByOutput
	set {runArg1}
	set i 0
	#while_nnglhKozEEwnqQsm
		ifnot recipes[{i}].output.id|=|runArg2 jump #if_dNIKJWbPWxZHQNuj
			set {runArg1} {i}
			quit
		#if_dNIKJWbPWxZHQNuj
		setadd i 1
	if i|<|{recipes.Length} jump #while_nnglhKozEEwnqQsm
quit
