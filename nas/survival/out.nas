...
#mine
	...
	ifnot minetimer|>|0 jump #if_VTPjSzGeRMZPFzZb
	...
	#if_VTPjSzGeRMZPFzZb
	set minepos
	if toomuch jump #ifnot_rJdhJhKPfuzMzmZK
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_rJdhJhKPfuzMzmZK
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_HZcfRQPNFOpkobeM
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_HZcfRQPNFOpkobeM
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_eDQLWthyFNgihiRq
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_SDkAUcGgsYPLxFSU
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_SDkAUcGgsYPLxFSU
	set {runArg1} {{runArg1}}&0
	#while_LiUXkOHwNNJAnarz
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_LiUXkOHwNNJAnarz
quit

#input
	ifnot runArg1|=|"craft" jump #if_EzlyNdOHLtlKkOzb
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_DPaFSGIFFkdaPfbA
			setsplit craftArgs " "
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_bMjsdlGsGuRqTjLP
				msg &cInvalid item name or ID
				quit
			#if_bMjsdlGsGuRqTjLP
			msg block id {blockID}
			quit
		#ifnot_DPaFSGIFFkdaPfbA
		set i 0
		#while_aoImowtMreSvtBMk
			call #checkRecipeAfford|{i}|canAfford
			if canAfford msg {i}: {blocks[{recipes[{i}].output.id}].name} x{recipes[{i}].output.count}
			setadd i 1
		if i|<|{recipes.Length} jump #while_aoImowtMreSvtBMk
		quit
	#if_EzlyNdOHLtlKkOzb
	set i 0
	msg &eResources:
	#while_HzgsaQUUCcEofNbI
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_HzgsaQUUCcEofNbI
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	#while_zVnyINgNiMXCnadM
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		ifnot count|>|{inventory[{id}]} jump #if_hsgIQdBZSJiQCXoa
			set {runArg2} false
			quit
		#if_hsgIQdBZSJiQCXoa
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_zVnyINgNiMXCnadM
	set {runArg2} true
quit

#getBlockByName
	if blocks[{runArg2}].name|=|"" jump #ifnot_AmUZHWnOhcPjAhIf
		set {runArg1} {runArg2}
		quit
	#ifnot_AmUZHWnOhcPjAhIf
	set i 0
	#while_qZcbPgyYgKVcVKGr
		ifnot blocks[{i}].name|=|runArg2 jump #if_nUIVbJbXsmnqzmDy
			set {runArg1} {i}
			quit
		#if_nUIVbJbXsmnqzmDy
		setadd i 1
	if i|<|{blocks.Length} jump #while_qZcbPgyYgKVcVKGr
quit
