...
#mine
	...
	ifnot minetimer|>|0 jump #if_SJdSISqRCuQdbrsp
	...
	#if_SJdSISqRCuQdbrsp
	set minepos
	if toomuch jump #ifnot_zOwzvPTYqOUZXwfQ
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_zOwzvPTYqOUZXwfQ
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_XXCrjRGShjYamBPp
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_XXCrjRGShjYamBPp
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_cmGZDbPFeUfLDQTo
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_ALIcMMokiFRrBdcc
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_ALIcMMokiFRrBdcc
	set {runArg1} {{runArg1}}&0
	#while_qRSgWTYGcaMpduGz
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_qRSgWTYGcaMpduGz
quit

#input
	ifnot runArg1|=|"craft" jump #if_ZgbJRcgJEMwXSVeE
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_vYAdmphkdWLEamwE
			set craftArgs[1] 1
			setsplit craftArgs *
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_UhOuddwFIQQLmIYE
				msg &cInvalid item name or ID
				quit
			#if_UhOuddwFIQQLmIYE
			call #getRecipeByOutput|recipeID|{blockID}
			ifnot recipeID|=|"" jump #if_QZsgtjLhBFLHtfHZ
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_QZsgtjLhBFLHtfHZ
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_vYAdmphkdWLEamwE
		msg &eRecipes:
		set i 0
		#while_GylynHYElktXSTsi
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_yDcjfxIfgLViQqOV
				msg &f> &6{blocks[{recipes[{i}].output.id}].name}&f (x{recipes[{i}].output.count}):
				set j 0
				#while_cpVpxCpvdUvOsEkW
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_cpVpxCpvdUvOsEkW
				msg {ingrediantList}
			#if_yDcjfxIfgLViQqOV
			setadd i 1
		if i|<|{recipes.Length} jump #while_GylynHYElktXSTsi
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_ZgbJRcgJEMwXSVeE
	set i 0
	msg &eResources:
	#while_hohISryOurnqLHEL
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_hohISryOurnqLHEL
	msg &eTools:
...
#doCraft
	...
	ifnot canAfford jump #if_TgMttNkljddLXXJr
		set j 0
		#while_VRceoEyejLfcScHr
			...
			setadd j 1
		if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_VRceoEyejLfcScHr
...
#checkRecipeAfford
	set j 0
	#while_MeDOkUZlimGVJjsp
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		setmul count {runArg3}
		ifnot count|>|{inventory[{id}]} jump #if_ZjuACvYcEbdxFSPj
			set {runArg2} false
			quit
		#if_ZjuACvYcEbdxFSPj
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_MeDOkUZlimGVJjsp
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_gKFVciaHnywsJcSR
		set {runArg1} {runArg2}
		quit
	#ifnot_gKFVciaHnywsJcSR
	set i 0
	#while_qYhFfjJHPWDhGgXx
		ifnot blocks[{i}].name|=|runArg2 jump #if_dictOWdjxTdOXXLK
			set {runArg1} {i}
			quit
		#if_dictOWdjxTdOXXLK
		setadd i 1
	if i|<|{blocks.Length} jump #while_qYhFfjJHPWDhGgXx
quit

#getRecipeByOutput
	set {runArg1}
	set i 0
	#while_DfkWCXxcemfzLSpv
		ifnot recipes[{i}].output.id|=|runArg2 jump #if_ohlbTsIDeHmLDPfn
			set {runArg1} {i}
			quit
		#if_ohlbTsIDeHmLDPfn
		setadd i 1
	if i|<|{recipes.Length} jump #while_DfkWCXxcemfzLSpv
quit
