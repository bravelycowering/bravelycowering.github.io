...
#mine
	...
	ifnot minetimer|>|0 jump #if_RTABrTCJtERGgeia
	...
	#if_RTABrTCJtERGgeia
	set minepos
	if toomuch jump #ifnot_bTXtxjEvHBEckRhm
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_bTXtxjEvHBEckRhm
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_NDfvzgLVkXADxuXP
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_NDfvzgLVkXADxuXP
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_qoLBqxTytxkcQOeS
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	#while_oQAKMJrkmAoCSzgO
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg3} jump #while_oQAKMJrkmAoCSzgO
	set {runArg1} {{runArg1}}&0
	#while_nmRhjMFisCRkIZai
		set {runArg1} {{runArg1}}|
		setadd i 1
	if i|<|{runArg4} jump #while_nmRhjMFisCRkIZai
quit
...
#input
	...
	set i 0
	msg &eResources:
	#while_SnixjCGuKzptKHYs
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_SnixjCGuKzptKHYs
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	#while_aiFaPYgMewPFidgC
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		ifnot count|>|{inventory[{id}]} jump #if_vMWEBzcSaDtxvnwz
			set {runArg2} false
			quit
		#if_vMWEBzcSaDtxvnwz
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_aiFaPYgMewPFidgC
	set {runArg2} true
quit

#input_craft
	set i 0
	#while_aAkECktfBHUPUbFt
		setadd i 1
		call #checkRecipeAfford|{i}|canAfford
		if canAfford msg {i}: {blocks[{recipes[{i}].output.id}].name} x{recipes[{i}].output.count}
	if i|<|{recipes.Length} jump #while_aAkECktfBHUPUbFt
quit
