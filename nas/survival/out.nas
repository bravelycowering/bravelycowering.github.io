...
set recipes[2].ingredients[0].count 4
set recipes[2].ingredients[0].id 5
set recipes[2].ingredients.Length 1
set recipes[2].output.count 1
set recipes[2].output.id 61
set recipes.Length 3
set toollevel[0] &cNo
...
#tick
	...
	ifnot blocks[{id}].kills jump #if_iEXSMZCUXNwTpesn
		msg your died
		kill
	#if_iEXSMZCUXNwTpesn
	if PlayerCoords|=|PrevPlayerCoords jump #ifnot_kGnHEeeApOSEgUPR
		// moved
		set usingWorkbench false
	#ifnot_kGnHEeeApOSEgUPR
	set PrevPlayerCoords {PlayerCoords}
	delay 100
...
#mine
	...
	ifnot minetimer|>|0 jump #if_lbAoiIxDzidADica
	...
	#if_lbAoiIxDzidADica
	set minepos
	if toomuch jump #ifnot_yqRjaNedEQryhZAh
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_yqRjaNedEQryhZAh
...
#give
	ifnot isTool({runArg1}) jump #if_VfYrRsYqnkcUAVjK
		set {runArg1} {runArg2}
		quit
	#if_VfYrRsYqnkcUAVjK
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_WIBtiTuLIvjMVNdX
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_WIBtiTuLIvjMVNdX
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_WUTZKduhBYcLHQFX
		...
		#while_eFnNlobLivEjLeJO
			...
			ifnot canAfford jump #if_MoLHeyiHTKIMtmZH
				...
				#while_UsycJjANOMyuWEqL
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_UsycJjANOMyuWEqL
				msg {ingrediantList}
			#if_MoLHeyiHTKIMtmZH
			setadd i 1
		if i|<|{recipes.Length} jump #while_eFnNlobLivEjLeJO
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_blzRgaowVaXvxwTc
	set i 0
	msg &eResources:
	#while_JqOxNZqnOAPJEswZ
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_JqOxNZqnOAPJEswZ
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	#while_kluZryDztVedPIiA
		set id {recipes[{runArg1}].ingredients[{j}].id}
		set count {recipes[{runArg1}].ingredients[{j}].count}
		setmul count {runArg3}
		ifnot count|>|{inventory[{id}]} jump #if_MgCGFfIrjxWUlCPT
			set {runArg2} false
			quit
		#if_MgCGFfIrjxWUlCPT
		ifnot isTool({id}) jump #if_LfTftsotbVGEgopx
			ifnot {id}|>=|count jump #if_vzTgqSVQfRIQffXq
				set {runArg2} false
				quit
			#if_vzTgqSVQfRIQffXq
		#if_LfTftsotbVGEgopx
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_kluZryDztVedPIiA
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_WojotNkjDMfuTkzf
		set {runArg1} {runArg2}
		quit
	#ifnot_WojotNkjDMfuTkzf
	set i 0
	#while_qZELhUViCBJUhghs
		ifnot blocks[{i}].name|=|runArg2 jump #if_VbLhAeTvxerQjptt
			set {runArg1} {i}
			quit
		#if_VbLhAeTvxerQjptt
		setadd i 1
	if i|<|{blocks.Length} jump #while_qZELhUViCBJUhghs
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_xjBSjWLOWRjAGTeP
		ifnot recipes[{i}].output.id|=|bid jump #if_CWryVdqCgWarrQIU
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_EGVlzrOfglGawELd
				set {pname} {i}
				quit
			#if_EGVlzrOfglGawELd
		#if_CWryVdqCgWarrQIU
		setadd i 1
	if i|<|{recipes.Length} jump #while_xjBSjWLOWRjAGTeP
quit
