...
// start scope
	set l_AevZbI_PrevPlayerCoords 
	set l_tssuBV_prevhp 
	set l_MjcOqX_hpbar 
	set l_iJeSmz_firebar 
	set l_HVRItm_myblock 
	set l_NfxvBr_firetickmod 
	set l_tYMKmc_temp 
	#tick
		call #getblock|l_HVRItm_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_HVRItm_myblock}].catchFire jump #if_ftrdwH
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_ftrdwH
		ifnot blocks[{l_HVRItm_myblock}].extinguishFire jump #if_UYSOWw
			ifnot fireticks|>|0 jump #if_YHotAU
				gui barSize 0
				set fireticks 0
			#if_YHotAU
		#if_UYSOWw
		ifnot blocks[{l_HVRItm_myblock}].damage|=|"" call #damage|{blocks[{l_HVRItm_myblock}].damage}|{blocks[{l_HVRItm_myblock}].damageType}
		ifnot PlayerCoords|=|l_AevZbI_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_AevZbI_PrevPlayerCoords set usingStonecutter false
		set l_AevZbI_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_tssuBV_prevhp jump #ifnot_cYbRKW
			set l_tssuBV_prevhp {hp}
			call #makebar|l_MjcOqX_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_MjcOqX_hpbar}
		#ifnot_cYbRKW
		...
		ifnot iframes|>|0 jump #if_GlBnpw
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_GlBnpw
		ifnot fireticks|>|0 jump #if_qVVtRu
			setsub fireticks 1
			set l_NfxvBr_firetickmod {fireticks}
			setmod l_NfxvBr_firetickmod 10
			ifnot l_NfxvBr_firetickmod|=|0 jump #if_UaXBGN
				ifnot fireticks|>|0 jump #if_ZMcLLv
					set l_tYMKmc_temp {fireticks}
					setdiv l_tYMKmc_temp 10
					call #makecharbar|l_iJeSmz_firebar|▐|6|{l_tYMKmc_temp}|10
					cpemsg smallannounce {l_iJeSmz_firebar}
				#if_ZMcLLv
...
#mine
	...
	ifnot minetimer|>|0 jump #if_KKLXpg
...
#give
	ifnot isTool({runArg1}) jump #if_OHDTcz
		set {runArg1} {runArg2}
		quit
	#if_OHDTcz
...
#giveall
	...
	#while_yzoxDU
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_yzoxDU
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_mpyLIo
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_UJzjNW
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_OZhJTH
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_oaIaal
	...
	call #getblock|id|{x}|{y}|{z}
	if blocks[{id}].mergeInto|=|"" jump #ifnot_ZQFswq
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_GMMiGB
			call #take|{playerHeldBlock}|1
			jump #setblock|{blocks[{id}].mergeInto}|{x}|{y}|{z}
			quit
		#if_GMMiGB
	...
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_oycQlJ
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_paSjDY
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_nHvKKO
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_gQDDny
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_ElJlye
		#while_AqQqta
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_AqQqta
	#if_ElJlye
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_vGqKay
		#while_aPABnz
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_aPABnz
	#if_vGqKay
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_kXEUFI
		#while_zkQixB
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_zkQixB
	#if_kXEUFI
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_OxBesK
		#while_kxnPai
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_kxnPai
	#if_OxBesK
quit

#input
	if runArg1|=|"changes" jump #changelog
	ifnot runArg1|=|"craft" jump #if_jlColM
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_dHLkIC
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_AwJvEb
				msg &cInvalid item name or ID
				quit
			#if_AwJvEb
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_LCaDHI
		...
		if usingWorkbench jump #ifnot_VHAkGs
		...
		#while_UarzzR
			...
			set ingrediantList
			ifnot canAfford|>|0 jump #if_ZPQrqP
				...
				#while_STTPlo
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_STTPlo
				msg {ingrediantList}
			#if_ZPQrqP
			setadd i 1
		if i|<|{recipes.Length} jump #while_UarzzR
	...
	#while_XVaFqk
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_XVaFqk
	msg &eTools:
...
#doCraft
	...
	#while_QCQMTo
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_QCQMTo
...
#checkRecipeAfford
	set j 0
	set {runArg2} 999
	if recipes[{runArg1}].condition|=|"" jump #ifnot_bYbpUV
		ifnot {recipes[{runArg1}].condition} set {runArg2} 0
	#ifnot_bYbpUV
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_IyXMbR
	...
	#while_OeeBRI
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_OeeBRI
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_FIFvdX
		set {runArg1} {runArg2}
		quit
	#ifnot_FIFvdX
	set i 0
	#while_uXJUMu
		ifnot blocks[{i}].name|=|runArg2 jump #if_JhatDg
			set {runArg1} {i}
			quit
		#if_JhatDg
		setadd i 1
	if i|<|{blocks.Length} jump #while_uXJUMu
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_AuddgT
		ifnot recipes[{i}].output.id|=|bid jump #if_zRoKjY
			call #checkRecipeAfford|{i}|canAfford
			ifnot canAfford|>=|c jump #if_PKvreN
				set {pname} {i}
				quit
			#if_PKvreN
		#if_zRoKjY
		setadd i 1
	if i|<|{recipes.Length} jump #while_AuddgT
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_Lyqjhe
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_BAnfCz
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_waJPFa
		call #take|80|1
		call #give|70|1
	#if_waJPFa
quit
