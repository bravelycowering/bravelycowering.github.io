...
#tick
	...
	if hp|=|prevhp jump #ifnot_kjhivbqxSFxRuxJp
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_kjhivbqxSFxRuxJp
	...
	ifnot iframes|>|0 jump #if_lpvRGxpsFiyvrYhV
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_lpvRGxpsFiyvrYhV
	ifnot fireticks|>|0 jump #if_DAnfYUJWPmEOEKQn
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_mXxGqUPIgHTjViMh
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_FHgqfmAjDyyVKZIS
...
#mine
	...
	ifnot minetimer|>|0 jump #if_TCpuZJVRRyIDksvt
	...
	#if_TCpuZJVRRyIDksvt
	set minepos
	if toomuch jump #ifnot_LAQzfmUkiPwMWBzd
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_LAQzfmUkiPwMWBzd
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_CvroFfTXOyKbXysa
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_FQzYwjKuSTaWQZUa
		set {runArg1} {runArg2}
		quit
	#if_FQzYwjKuSTaWQZUa
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_ltmGnAcgdtaQAXCX
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_ltmGnAcgdtaQAXCX
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_JglgfYbEQOkvYsbW
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_drwNfZIRbQJbAmat
		#while_DwPnHQgChNICegnv
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_DwPnHQgChNICegnv
	#if_drwNfZIRbQJbAmat
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_INgWJeUjLScyeGze
		#while_JhOLKiCAgiVvTmNW
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_JhOLKiCAgiVvTmNW
	#if_INgWJeUjLScyeGze
quit

#input
	ifnot runArg1|=|"craft" jump #if_BNOtfoCxsXOqNvPv
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_HWRdwGlIJsZNePJt
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_hoRyznULyqcxdmiS
				msg &cInvalid item name or ID
				quit
			#if_hoRyznULyqcxdmiS
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_CsDSdavZyEQBXuxt
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_CsDSdavZyEQBXuxt
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_HWRdwGlIJsZNePJt
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_HOXxGMLDCCNEFvlE
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_HOXxGMLDCCNEFvlE
		set i 0
		#while_meCrSLulLnOhZdkj
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_tJTXJCwagJbEjoCB
				...
				#while_uTLiFZgawJKbbQsB
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_uTLiFZgawJKbbQsB
				msg {ingrediantList}
			#if_tJTXJCwagJbEjoCB
			setadd i 1
		if i|<|{recipes.Length} jump #while_meCrSLulLnOhZdkj
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_BNOtfoCxsXOqNvPv
	set i 0
	msg &eResources:
	#while_ohOEWImZgaoLlNIQ
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_ohOEWImZgaoLlNIQ
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_ZOWHWGitFBZuiibD
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_ZOWHWGitFBZuiibD
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_XXDfJtvnVRwwxqZb
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_XXDfJtvnVRwwxqZb
	#while_xqTqFXzRffrzBeBU
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_xqTqFXzRffrzBeBU
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_tPwWWIKbFrlkUWmi
		set {runArg1} {runArg2}
		quit
	#ifnot_tPwWWIKbFrlkUWmi
	set i 0
	#while_kPFYCUTdTPOBieNT
		ifnot blocks[{i}].name|=|runArg2 jump #if_baUjYFyWBuAKCFOw
			set {runArg1} {i}
			quit
		#if_baUjYFyWBuAKCFOw
		setadd i 1
	if i|<|{blocks.Length} jump #while_kPFYCUTdTPOBieNT
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_hsuIMgNpEOKkHVqR
		ifnot recipes[{i}].output.id|=|bid jump #if_dSjLdkPTPBkpVsUE
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_kqRJSnOnjRvCPYvx
				set {pname} {i}
				quit
			#if_kqRJSnOnjRvCPYvx
		#if_dSjLdkPTPBkpVsUE
		setadd i 1
	if i|<|{recipes.Length} jump #while_hsuIMgNpEOKkHVqR
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_gHfRfJHSOGCuGTLX
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_bADOxJgUvyNnFQNX
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_bADOxJgUvyNnFQNX
	#ifnot_gHfRfJHSOGCuGTLX
...
#use[68]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_ZpRdumjGNnvQLCHv
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_zSDtvBDqcFkdjDSa
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
		#if_zSDtvBDqcFkdjDSa
	#ifnot_ZpRdumjGNnvQLCHv
	setdeathspawn {PlayerCoords} {PlayerYaw} {PlayerPitch}
	set spawnblock {runArg1} {runArg2} {runArg3}
	msg &fRespawn point set
quit
