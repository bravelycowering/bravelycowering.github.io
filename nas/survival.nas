...
// start scope
	set l_DgJvqL_PrevPlayerCoords 
	set l_OPKTjh_prevhp 
	set l_dImbYa_hpbar 
	set l_LURPRZ_firebar 
	set l_DYsSmD_myblock 
	set l_OcLpYT_firetickmod 
	set l_PRRtJW_temp 
	#tick
		call #getblock|l_DYsSmD_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_DYsSmD_myblock}].catchFire jump #if_jdFyST
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_jdFyST
		ifnot blocks[{l_DYsSmD_myblock}].extinguishFire jump #if_ODBTHi
			ifnot fireticks|>|0 jump #if_lqIIDV
				gui barSize 0
				set fireticks 0
			#if_lqIIDV
		#if_ODBTHi
		ifnot blocks[{l_DYsSmD_myblock}].damage|=|"" call #damage|{blocks[{l_DYsSmD_myblock}].damage}|{blocks[{l_DYsSmD_myblock}].damageType}
		ifnot PlayerCoords|=|l_DgJvqL_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_DgJvqL_PrevPlayerCoords set usingStonecutter false
		set l_DgJvqL_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_OPKTjh_prevhp jump #ifnot_qRZeCf
			set l_OPKTjh_prevhp {hp}
			call #makebar|l_dImbYa_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_dImbYa_hpbar}
		#ifnot_qRZeCf
		...
		ifnot iframes|>|0 jump #if_kbLHvY
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_kbLHvY
		ifnot fireticks|>|0 jump #if_dUymCl
			setsub fireticks 1
			set l_OcLpYT_firetickmod {fireticks}
			setmod l_OcLpYT_firetickmod 10
			ifnot l_OcLpYT_firetickmod|=|0 jump #if_OnRjlG
				ifnot fireticks|>|0 jump #if_BiBJLX
					set l_PRRtJW_temp {fireticks}
					setdiv l_PRRtJW_temp 10
					call #makecharbar|l_LURPRZ_firebar|▐|6|{l_PRRtJW_temp}|10
					cpemsg smallannounce {l_LURPRZ_firebar}
				#if_BiBJLX
...
#mine
	...
	ifnot minetimer|>|0 jump #if_mAmUzF
...
#give
	ifnot isTool({runArg1}) jump #if_FcTTKV
		set {runArg1} {runArg2}
		quit
	#if_FcTTKV
...
#giveall
	...
	#while_ZRuMat
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_ZRuMat
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_NvbQzo
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_SVxTfr
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_WdroHq
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_ziJHcy
	...
	call #getblock|id|{x}|{y}|{z}
	if blocks[{id}].mergeInto|=|"" jump #ifnot_LdGgBS
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_onVnXj
			call #take|{playerHeldBlock}|1
			jump #setblock|{blocks[{id}].mergeInto}|{x}|{y}|{z}
			quit
		#if_onVnXj
	...
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_UkdtHT
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_acCrNW
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_mfBbvj
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_pBRfKp
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_EFULpT
		#while_fXhuPU
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_fXhuPU
	#if_EFULpT
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_mZfhmh
		#while_nKsxHF
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_nKsxHF
	#if_mZfhmh
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_mJPDHM
		#while_XGRNaP
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_XGRNaP
	#if_mJPDHM
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_uPyBXi
		#while_VnnALN
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_VnnALN
	#if_uPyBXi
quit

#input
	if runArg1|=|"changes" jump #changelog
	ifnot runArg1|=|"craft" jump #if_lcIQVw
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_xMztwT
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_ivXkJv
				msg &cInvalid item name or ID
				quit
			#if_ivXkJv
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_DClSVG
		...
		if usingWorkbench jump #ifnot_GYpDIl
		...
		#while_UGeIFp
			...
			set ingrediantList
			ifnot canAfford|>|0 jump #if_EAIJCV
				...
				#while_TJsyWJ
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_TJsyWJ
				msg {ingrediantList}
			#if_EAIJCV
			setadd i 1
		if i|<|{recipes.Length} jump #while_UGeIFp
	...
	#while_fyZBWy
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_fyZBWy
	msg &eTools:
...
#doCraft
	...
	#while_XfjYZo
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_XfjYZo
...
#checkRecipeAfford
	set j 0
	set {runArg2} 999
	if recipes[{runArg1}].condition|=|"" jump #ifnot_XqgfDW
		ifnot {recipes[{runArg1}].condition} set {runArg2} 0
	#ifnot_XqgfDW
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_lnMuoP
	...
	#while_gvqQOH
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_gvqQOH
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_jhrqEw
		set {runArg1} {runArg2}
		quit
	#ifnot_jhrqEw
	set i 0
	#while_XwTEgG
		ifnot blocks[{i}].name|=|runArg2 jump #if_GwmuMi
			set {runArg1} {i}
			quit
		#if_GwmuMi
		setadd i 1
	if i|<|{blocks.Length} jump #while_XwTEgG
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_CwHTMv
		ifnot recipes[{i}].output.id|=|bid jump #if_ErpSPh
			call #checkRecipeAfford|{i}|canAfford
			ifnot canAfford|>=|c jump #if_LreEKV
				set {pname} {i}
				quit
			#if_LreEKV
		#if_ErpSPh
		setadd i 1
	if i|<|{recipes.Length} jump #while_CwHTMv
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_nLTraf
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_akorBb
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_CXrVtE
		call #take|80|1
		call #give|70|1
	#if_CXrVtE
quit
