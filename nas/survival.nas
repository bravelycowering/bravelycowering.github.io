...
// start scope
	set l_ZDfnuD_PrevPlayerCoords 
	set l_jLTANE_prevhp 
	set l_YZMdSw_hpbar 
	set l_xtjYdI_firebar 
	set l_WqaqRb_myblock 
	set l_mrJEcj_firetickmod 
	set l_gWbHZw_temp 
	#tick
		call #getblock|l_WqaqRb_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_WqaqRb_myblock}].catchFire jump #if_ZbBfVK
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_ZbBfVK
		ifnot blocks[{l_WqaqRb_myblock}].extinguishFire jump #if_uhniYX
			ifnot fireticks|>|0 jump #if_ROlXzF
				gui barSize 0
				set fireticks 0
			#if_ROlXzF
		#if_uhniYX
		ifnot blocks[{l_WqaqRb_myblock}].damage|=|"" call #damage|{blocks[{l_WqaqRb_myblock}].damage}|{blocks[{l_WqaqRb_myblock}].damageType}
		ifnot PlayerCoords|=|l_ZDfnuD_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_ZDfnuD_PrevPlayerCoords set usingStonecutter false
		set l_ZDfnuD_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_jLTANE_prevhp jump #ifnot_hwqtmP
			set l_jLTANE_prevhp {hp}
			call #makebar|l_YZMdSw_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_YZMdSw_hpbar}
		#ifnot_hwqtmP
		...
		ifnot iframes|>|0 jump #if_VIijjw
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_VIijjw
		ifnot fireticks|>|0 jump #if_GceFVi
			setsub fireticks 1
			set l_mrJEcj_firetickmod {fireticks}
			setmod l_mrJEcj_firetickmod 10
			ifnot l_mrJEcj_firetickmod|=|0 jump #if_HZXLUE
				ifnot fireticks|>|0 jump #if_pZuOWK
					set l_gWbHZw_temp {fireticks}
					setdiv l_gWbHZw_temp 10
					call #makecharbar|l_xtjYdI_firebar|▐|6|{l_gWbHZw_temp}|10
					cpemsg smallannounce {l_xtjYdI_firebar}
				#if_pZuOWK
...
#mine
	...
	ifnot minetimer|>|0 jump #if_IQMmbw
...
#give
	ifnot isTool({runArg1}) jump #if_nPUiOV
		set {runArg1} {runArg2}
		quit
	#if_nPUiOV
...
#giveall
	...
	#while_yEBuho
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_yEBuho
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_GlStFR
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_Fawehc
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_QBdpIB
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_EreNzv
	...
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_EqiMyf
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_yuvFiv
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_UvIrtf
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_RCdrWf
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_daqFpZ
		#while_mDJQma
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_mDJQma
	#if_daqFpZ
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_ycnfAZ
		#while_IzPWGZ
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_IzPWGZ
	#if_ycnfAZ
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_xaOeiC
		#while_XerKVT
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_XerKVT
	#if_xaOeiC
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_JLCBMw
		#while_tgHcrJ
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_tgHcrJ
	#if_JLCBMw
quit

#input
	ifnot runArg1|=|"craft" jump #if_JJUJqu
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_eBGZHJ
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_AYYCzG
				msg &cInvalid item name or ID
				quit
			#if_AYYCzG
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_UeinWP
		...
		if usingWorkbench jump #ifnot_XlDGBs
		...
		#while_URBmyt
			...
			set ingrediantList
			ifnot canAfford|>|0 jump #if_GskRkb
				...
				#while_CZloTq
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_CZloTq
				msg {ingrediantList}
			#if_GskRkb
			setadd i 1
		if i|<|{recipes.Length} jump #while_URBmyt
	...
	#while_nqoIvX
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_nqoIvX
	msg &eTools:
...
#doCraft
	...
	#while_GklJEC
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_GklJEC
...
#checkRecipeAfford
	set j 0
	set {runArg2} 999
	if recipes[{runArg1}].condition|=|"" jump #ifnot_WcLSzg
		ifnot {recipes[{runArg1}].condition} set {runArg2} 0
	#ifnot_WcLSzg
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_roROcy
	...
	#while_sMQXaG
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_sMQXaG
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_VSwYbi
		set {runArg1} {runArg2}
		quit
	#ifnot_VSwYbi
	set i 0
	#while_aUOGet
		ifnot blocks[{i}].name|=|runArg2 jump #if_ICmTdN
			set {runArg1} {i}
			quit
		#if_ICmTdN
		setadd i 1
	if i|<|{blocks.Length} jump #while_aUOGet
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_wqZOah
		ifnot recipes[{i}].output.id|=|bid jump #if_BSgYgI
			call #checkRecipeAfford|{i}|canAfford
			ifnot canAfford|>=|c jump #if_CLlkZy
				set {pname} {i}
				quit
			#if_CLlkZy
		#if_BSgYgI
		setadd i 1
	if i|<|{recipes.Length} jump #while_wqZOah
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_KfPfqz
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_RxfEqK
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_owwrRc
		call #take|80|1
		call #give|70|1
	#if_owwrRc
quit
