...
// start scope
	set l_PdGKQG_PrevPlayerCoords 
	set l_lGThSd_prevhp 
	set l_dyAgtw_hpbar 
	set l_HxTxpy_myblock 
	#tick
		call #getblock|l_HxTxpy_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_HxTxpy_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_HxTxpy_myblock}].extinguishFire jump #if_FQhLBS
			ifnot fireticks|>|0 jump #if_xjhZFo
				gui barSize 0
				set fireticks 0
			#if_xjhZFo
		#if_FQhLBS
		ifnot blocks[{l_HxTxpy_myblock}].damage|=|"" call #damage|{blocks[{l_HxTxpy_myblock}].damage}|{blocks[{l_HxTxpy_myblock}].damageType}
		ifnot PlayerCoords|=|l_PdGKQG_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_PdGKQG_PrevPlayerCoords set usingStonecutter false
		set l_PdGKQG_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_lGThSd_prevhp jump #ifnot_ZACcaW
			set l_lGThSd_prevhp {hp}
			call #makebar|l_dyAgtw_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_dyAgtw_hpbar}
		#ifnot_ZACcaW
		...
		ifnot iframes|>|0 jump #if_tKFmvC
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_tKFmvC
		ifnot fireticks|>|0 jump #if_ceelRy
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_MepGvh
...
#mine
	...
	ifnot minetimer|>|0 jump #if_dhjSRk
...
#give
	ifnot isTool({runArg1}) jump #if_Kkygbi
		set {runArg1} {runArg2}
		quit
	#if_Kkygbi
...
#giveall
	...
	#while_DXTqcT
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_DXTqcT
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_DwVhMt
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_DwVhMt
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_TacuBa
...
#setblock
	if allowMapChanges jump #ifnot_CPeYjH
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_dgfvAK
		#while_TVRqot
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_TVRqot
	#if_dgfvAK
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_NitEpX
		#while_JdnVqN
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_JdnVqN
	#if_NitEpX
quit

#input
	ifnot runArg1|=|"craft" jump #if_BJsRCC
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_BxlnQy
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_ewlAFB
				msg &cInvalid item name or ID
				quit
			#if_ewlAFB
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_OJfKOr
		...
		if usingWorkbench jump #ifnot_raGFjq
		...
		#while_tgsfXv
			...
			set ingrediantList
			ifnot canAfford jump #if_nQzgFx
				...
				#while_xdlcCl
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_xdlcCl
				msg {ingrediantList}
			#if_nQzgFx
			setadd i 1
		if i|<|{recipes.Length} jump #while_tgsfXv
	...
	#while_UIWzzH
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_UIWzzH
	msg &eTools:
...
#doCraft
	...
	#while_wWldDz
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_wWldDz
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_ggWaJl
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_ggWaJl
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_EcQkdb
	...
	#while_nGUcqH
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_nGUcqH
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_JMelnD
		set {runArg1} {runArg2}
		quit
	#ifnot_JMelnD
	set i 0
	#while_NjeXhj
		ifnot blocks[{i}].name|=|runArg2 jump #if_edRdkj
			set {runArg1} {i}
			quit
		#if_edRdkj
		setadd i 1
	if i|<|{blocks.Length} jump #while_NjeXhj
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_dNkjOv
		ifnot recipes[{i}].output.id|=|bid jump #if_uUoOeS
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_RWuDam
				set {pname} {i}
				quit
			#if_RWuDam
		#if_uUoOeS
		setadd i 1
	if i|<|{recipes.Length} jump #while_dNkjOv
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_WTarpB
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_hsivSq
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_zCAOvL
		call #take|80|1
		call #give|70|1
	#if_zCAOvL
quit
