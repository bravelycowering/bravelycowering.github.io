...
// start scope
	set l_WDIDAz_PrevPlayerCoords 
	set l_ttjROr_prevhp 
	set l_ypcnuI_hpbar 
	set l_PtSxlG_myblock 
	#tick
		call #getblock|l_PtSxlG_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_PtSxlG_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_PtSxlG_myblock}].extinguishFire jump #if_MTfnwM
			ifnot fireticks|>|0 jump #if_RIIbPh
				gui barSize 0
				set fireticks 0
			#if_RIIbPh
		#if_MTfnwM
		ifnot blocks[{l_PtSxlG_myblock}].damage|=|"" call #damage|{blocks[{l_PtSxlG_myblock}].damage}|{blocks[{l_PtSxlG_myblock}].damageType}
		ifnot PlayerCoords|=|l_WDIDAz_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_WDIDAz_PrevPlayerCoords set usingStonecutter false
		set l_WDIDAz_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_ttjROr_prevhp jump #ifnot_OcGDle
			set l_ttjROr_prevhp {hp}
			call #makebar|l_ypcnuI_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_ypcnuI_hpbar}
		#ifnot_OcGDle
		...
		ifnot iframes|>|0 jump #if_GPVszB
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_GPVszB
		ifnot fireticks|>|0 jump #if_fkwlfW
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_QdJzrp
...
#mine
	...
	ifnot minetimer|>|0 jump #if_welJkI
...
#give
	ifnot isTool({runArg1}) jump #if_wBIuCb
		set {runArg1} {runArg2}
		quit
	#if_wBIuCb
...
#giveall
	...
	#while_LZrNAj
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_LZrNAj
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_cQTQxy
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_cQTQxy
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_vxTVHp
...
#setblock
	if allowMapChanges jump #ifnot_IPvBdm
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_kVWgQi
		#while_mkTgiz
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_mkTgiz
	#if_kVWgQi
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_HJissj
		#while_WIzLRi
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_WIzLRi
	#if_HJissj
quit

#input
	ifnot runArg1|=|"craft" jump #if_tjKcmP
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_wuuhZY
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_bdzHHx
				msg &cInvalid item name or ID
				quit
			#if_bdzHHx
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_TNueNP
		...
		if usingWorkbench jump #ifnot_ekaWBb
		...
		#while_YBbzyT
			...
			set ingrediantList
			ifnot canAfford jump #if_lQJJzA
				...
				#while_rEfqMO
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_rEfqMO
				msg {ingrediantList}
			#if_lQJJzA
			setadd i 1
		if i|<|{recipes.Length} jump #while_YBbzyT
	...
	#while_JQJbgw
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_JQJbgw
	msg &eTools:
...
#doCraft
	...
	#while_CcpZzD
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_CcpZzD
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_kanDMc
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_kanDMc
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_Sudues
	...
	#while_lEWvzv
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_lEWvzv
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_GUUiQM
		set {runArg1} {runArg2}
		quit
	#ifnot_GUUiQM
	set i 0
	#while_bOHnCO
		ifnot blocks[{i}].name|=|runArg2 jump #if_yVELhH
			set {runArg1} {i}
			quit
		#if_yVELhH
		setadd i 1
	if i|<|{blocks.Length} jump #while_bOHnCO
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_VHUtVX
		ifnot recipes[{i}].output.id|=|bid jump #if_GwNbEv
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_dVQIVv
				set {pname} {i}
				quit
			#if_dVQIVv
		#if_GwNbEv
		setadd i 1
	if i|<|{recipes.Length} jump #while_VHUtVX
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_VZbyvX
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_CppcYs
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_kOxqVq
		call #take|80|1
		call #give|70|1
	#if_kOxqVq
quit
