...
// start scope
	set l_FTjLBd_PrevPlayerCoords 
	set l_MgUcfF_prevhp 
	set l_rvMNqA_hpbar 
	set l_bwyszt_firebar 
	set l_VWWjtu_myblock 
	set l_QJbgHs_temp 
	set l_UfUnGm_firetickmod 
	#tick
		call #getblock|l_VWWjtu_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_VWWjtu_myblock}].catchFire set fireticks 101
		ifnot blocks[{l_VWWjtu_myblock}].extinguishFire jump #if_QMgoFv
			ifnot fireticks|>|0 jump #if_LEfwgi
				gui barSize 0
				set fireticks 0
			#if_LEfwgi
		#if_QMgoFv
		ifnot blocks[{l_VWWjtu_myblock}].damage|=|"" call #damage|{blocks[{l_VWWjtu_myblock}].damage}|{blocks[{l_VWWjtu_myblock}].damageType}
		ifnot PlayerCoords|=|l_FTjLBd_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_FTjLBd_PrevPlayerCoords set usingStonecutter false
		set l_FTjLBd_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_MgUcfF_prevhp jump #ifnot_GoYysC
			set l_MgUcfF_prevhp {hp}
			call #makebar|l_rvMNqA_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_rvMNqA_hpbar}
		#ifnot_GoYysC
		...
		ifnot iframes|>|0 jump #if_BTKeXC
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_BTKeXC
		ifnot fireticks|>|0 jump #if_folKec
			setsub fireticks 1
			set l_UfUnGm_firetickmod {fireticks}
			setmod l_UfUnGm_firetickmod 10
			ifnot l_UfUnGm_firetickmod|=|0 jump #if_fsOvku
				call #damage|2|burn
				ifnot fireticks|>|0 jump #if_NJlCnj
					set l_QJbgHs_temp {fireticks}
					setdiv l_QJbgHs_temp 10
					call #makecharbar|l_bwyszt_firebar|▐|6|{l_QJbgHs_temp}|10
					cpemsg smallannounce {l_bwyszt_firebar}
				#if_NJlCnj
...
#mine
	...
	ifnot minetimer|>|0 jump #if_eMrbUP
...
#give
	ifnot isTool({runArg1}) jump #if_TCsyNU
		set {runArg1} {runArg2}
		quit
	#if_TCsyNU
...
#giveall
	...
	#while_lbknDl
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_lbknDl
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_bIGPJh
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_bIGPJh
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_LqgeMw
...
#setblock
	if allowMapChanges jump #ifnot_VCwMAm
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_dRPeGp
		#while_vfBMpN
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_vfBMpN
	#if_dRPeGp
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_easQOa
		#while_mtrDJU
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_mtrDJU
	#if_easQOa
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_jTVPAM
		#while_bXDPUI
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_bXDPUI
	#if_jTVPAM
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_PBoxhb
		#while_gNmWct
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_gNmWct
	#if_PBoxhb
quit

#input
	ifnot runArg1|=|"craft" jump #if_ZHruTD
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_XKggKu
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_NZyPpR
				msg &cInvalid item name or ID
				quit
			#if_NZyPpR
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_dGLRVw
		...
		if usingWorkbench jump #ifnot_nynDzD
		...
		#while_gCUeBJ
			...
			set ingrediantList
			ifnot canAfford jump #if_rHUtfd
				...
				#while_IWWZiY
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_IWWZiY
				msg {ingrediantList}
			#if_rHUtfd
			setadd i 1
		if i|<|{recipes.Length} jump #while_gCUeBJ
	...
	#while_tVWSqR
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_tVWSqR
	msg &eTools:
...
#doCraft
	...
	#while_MiHzCv
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_MiHzCv
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_QrWwDw
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_QrWwDw
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_bfWtsk
	...
	#while_DzOQHl
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_DzOQHl
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_VXNfiA
		set {runArg1} {runArg2}
		quit
	#ifnot_VXNfiA
	set i 0
	#while_fXovXL
		ifnot blocks[{i}].name|=|runArg2 jump #if_fbrhsl
			set {runArg1} {i}
			quit
		#if_fbrhsl
		setadd i 1
	if i|<|{blocks.Length} jump #while_fXovXL
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_iMPMmJ
		ifnot recipes[{i}].output.id|=|bid jump #if_CBUCPR
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_QgwLev
				set {pname} {i}
				quit
			#if_QgwLev
		#if_CBUCPR
		setadd i 1
	if i|<|{recipes.Length} jump #while_iMPMmJ
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_nZkUum
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_HJPbVP
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_YdReKj
		call #take|80|1
		call #give|70|1
	#if_YdReKj
quit
