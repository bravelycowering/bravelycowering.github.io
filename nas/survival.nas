...
// start scope
	set l_ZAJFTi_PrevPlayerCoords 
	set l_HiIqDa_prevhp 
	set l_yoMIQF_hpbar 
	set l_AniSQw_firebar 
	set l_hMiWqR_myblock 
	set l_ACgnlA_firetickmod 
	set l_goNxXz_temp 
	#tick
		call #getblock|l_hMiWqR_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_hMiWqR_myblock}].catchFire jump #if_hSgFkT
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_hSgFkT
		ifnot blocks[{l_hMiWqR_myblock}].extinguishFire jump #if_OPoeyR
			ifnot fireticks|>|0 jump #if_OLwkxS
				gui barSize 0
				set fireticks 0
			#if_OLwkxS
		#if_OPoeyR
		ifnot blocks[{l_hMiWqR_myblock}].damage|=|"" call #damage|{blocks[{l_hMiWqR_myblock}].damage}|{blocks[{l_hMiWqR_myblock}].damageType}
		ifnot PlayerCoords|=|l_ZAJFTi_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_ZAJFTi_PrevPlayerCoords set usingStonecutter false
		set l_ZAJFTi_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_HiIqDa_prevhp jump #ifnot_EJHifb
			set l_HiIqDa_prevhp {hp}
			call #makebar|l_yoMIQF_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_yoMIQF_hpbar}
		#ifnot_EJHifb
		...
		ifnot iframes|>|0 jump #if_aiZrEk
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_aiZrEk
		ifnot fireticks|>|0 jump #if_ySGHAE
			setsub fireticks 1
			set l_ACgnlA_firetickmod {fireticks}
			setmod l_ACgnlA_firetickmod 10
			ifnot l_ACgnlA_firetickmod|=|0 jump #if_jtNZqk
				ifnot fireticks|>|0 jump #if_tudJCd
					set l_goNxXz_temp {fireticks}
					setdiv l_goNxXz_temp 10
					call #makecharbar|l_AniSQw_firebar|▐|6|{l_goNxXz_temp}|10
					cpemsg smallannounce {l_AniSQw_firebar}
				#if_tudJCd
...
#mine
	...
	ifnot minetimer|>|0 jump #if_wOXQwb
...
#give
	ifnot isTool({runArg1}) jump #if_Atahgt
		set {runArg1} {runArg2}
		quit
	#if_Atahgt
...
#giveall
	...
	#while_lsAAZp
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_lsAAZp
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_FVJkZF
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_rMrRLZ
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_KOcZON
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_hojtEr
	...
	call #getblock|id|{x}|{y}|{z}
	if blocks[{id}].mergeInto|=|"" jump #ifnot_rGLaip
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_hBTEHx
			call #take|{playerHeldBlock}|1
			jump #setblock|{blocks[{id}].mergeInto}|{x}|{y}|{z}
			quit
		#if_hBTEHx
	...
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_bJWzTV
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_ZSfmco
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_Ljucfs
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_EGFndp
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_SOgHCY
		#while_tjmSlN
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_tjmSlN
	#if_SOgHCY
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_dRYQGH
		#while_ahORZD
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_ahORZD
	#if_dRYQGH
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_kbzQkD
		#while_JJRNVB
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_JJRNVB
	#if_kbzQkD
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_dsAXiL
		#while_XoCPFM
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_XoCPFM
	#if_dsAXiL
quit

#input
	ifnot runArg1|=|"craft" jump #if_CzLivW
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_esJdXB
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_ynNOcG
				msg &cInvalid item name or ID
				quit
			#if_ynNOcG
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_uiHwxV
		...
		if usingWorkbench jump #ifnot_UKWwfA
		...
		#while_umaObq
			...
			set ingrediantList
			ifnot canAfford|>|0 jump #if_bOyNLH
				...
				#while_TBezPN
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_TBezPN
				msg {ingrediantList}
			#if_bOyNLH
			setadd i 1
		if i|<|{recipes.Length} jump #while_umaObq
	...
	#while_klXSrl
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_klXSrl
	msg &eTools:
...
#doCraft
	...
	#while_RHcUOw
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_RHcUOw
...
#checkRecipeAfford
	set j 0
	set {runArg2} 999
	if recipes[{runArg1}].condition|=|"" jump #ifnot_pdSffY
		ifnot {recipes[{runArg1}].condition} set {runArg2} 0
	#ifnot_pdSffY
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_RaeoWv
	...
	#while_kmhQqU
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_kmhQqU
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_oqKdrd
		set {runArg1} {runArg2}
		quit
	#ifnot_oqKdrd
	set i 0
	#while_iMXHtx
		ifnot blocks[{i}].name|=|runArg2 jump #if_cFjrrq
			set {runArg1} {i}
			quit
		#if_cFjrrq
		setadd i 1
	if i|<|{blocks.Length} jump #while_iMXHtx
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_CwFYEq
		ifnot recipes[{i}].output.id|=|bid jump #if_mJQkEm
			call #checkRecipeAfford|{i}|canAfford
			ifnot canAfford|>=|c jump #if_VUHnSz
				set {pname} {i}
				quit
			#if_VUHnSz
		#if_mJQkEm
		setadd i 1
	if i|<|{recipes.Length} jump #while_CwFYEq
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_Bwxqtk
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_hZuutk
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_QBthLg
		call #take|80|1
		call #give|70|1
	#if_QBthLg
quit
