...
#tick
	...
	ifnot blocks[{id}].kills jump #if_pmkqyktiGJSvZAtA
		msg your died
		kill
	#if_pmkqyktiGJSvZAtA
...
#mine
	...
	ifnot minetimer|>|0 jump #if_rBCcMDRKFOBlDZkt
	...
	#if_rBCcMDRKFOBlDZkt
	set minepos
	if toomuch jump #ifnot_YlkBOqseWPVvjlSF
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_YlkBOqseWPVvjlSF
...
#give
	ifnot isTool({runArg1}) jump #if_iomeXczETkwYqJWF
		set {runArg1} {runArg2}
		quit
	#if_iomeXczETkwYqJWF
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_JFDJhrQACjHGqton
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_JFDJhrQACjHGqton
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_sJStpfqisKTMUzzU
		...
		#while_jxrdnIwzslsPWxCL
			...
			ifnot canAfford jump #if_tdHQXKvxiXqDojUf
				...
				#while_WAMWvfLWXGNsgPQb
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_WAMWvfLWXGNsgPQb
				msg {ingrediantList}
			#if_tdHQXKvxiXqDojUf
			setadd i 1
		if i|<|{recipes.Length} jump #while_jxrdnIwzslsPWxCL
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_pbyQbiXKaJINLzLk
	set i 0
	msg &eResources:
	#while_NFhQXFItyjaLabFu
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_NFhQXFItyjaLabFu
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_FaUVXqOWGDGEDNyZ
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_FaUVXqOWGDGEDNyZ
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_TwWRqzjmAuRlrzry
		if {recipes[{runArg1}].output.id}|>=|count quit
	#if_TwWRqzjmAuRlrzry
	#while_osPwtzZJlsUAfCld
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_osPwtzZJlsUAfCld
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_lPuPaVyynzlKlIZm
		set {runArg1} {runArg2}
		quit
	#ifnot_lPuPaVyynzlKlIZm
	set i 0
	#while_lyqXxfjooJRrvVxS
		ifnot blocks[{i}].name|=|runArg2 jump #if_fsiCdXTCIsfWIdLg
			set {runArg1} {i}
			quit
		#if_fsiCdXTCIsfWIdLg
		setadd i 1
	if i|<|{blocks.Length} jump #while_lyqXxfjooJRrvVxS
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_RJWbDMQglzlUrijc
		ifnot recipes[{i}].output.id|=|bid jump #if_RgyZrnLoRpGDAEpH
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_ivSSUIgYdVEICCPe
				set {pname} {i}
				quit
			#if_ivSSUIgYdVEICCPe
		#if_RgyZrnLoRpGDAEpH
		setadd i 1
	if i|<|{recipes.Length} jump #while_RJWbDMQglzlUrijc
quit
