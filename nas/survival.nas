...
// start scope
	set l_YJLyEC_PrevPlayerCoords 
	set l_BgSYDy_prevhp 
	set l_GkLizh_hpbar 
	set l_lpsjXR_myblock 
	#tick
		call #getblock|l_lpsjXR_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_lpsjXR_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_lpsjXR_myblock}].extinguishFire jump #if_PhDeqt
			ifnot fireticks|>|0 jump #if_lfWgMI
				gui barSize 0
				set fireticks 0
			#if_lfWgMI
		#if_PhDeqt
		ifnot blocks[{l_lpsjXR_myblock}].damage|=|"" call #damage|{blocks[{l_lpsjXR_myblock}].damage}|{blocks[{l_lpsjXR_myblock}].damageType}
		ifnot PlayerCoords|=|l_YJLyEC_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_YJLyEC_PrevPlayerCoords set usingStonecutter false
		set l_YJLyEC_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_BgSYDy_prevhp jump #ifnot_jyeapn
			set l_BgSYDy_prevhp {hp}
			call #makebar|l_GkLizh_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_GkLizh_hpbar}
		#ifnot_jyeapn
		...
		ifnot iframes|>|0 jump #if_WGSvGa
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_WGSvGa
		ifnot fireticks|>|0 jump #if_sHvUsY
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_BMeXQf
...
#mine
	...
	ifnot minetimer|>|0 jump #if_lLnAtj
...
#give
	ifnot isTool({runArg1}) jump #if_OeKUeI
		set {runArg1} {runArg2}
		quit
	#if_OeKUeI
...
#giveall
	...
	#while_MhZTyK
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_MhZTyK
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_bnYNbS
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_bnYNbS
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_AXIjaC
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_nuOZuJ
		#while_CMcOpT
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_CMcOpT
	#if_nuOZuJ
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_zhaQMh
		#while_XNOodn
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_XNOodn
	#if_zhaQMh
quit

#input
	ifnot runArg1|=|"craft" jump #if_kInZCR
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_JpFMQn
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_gLRATt
				msg &cInvalid item name or ID
				quit
			#if_gLRATt
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_aPYEUB
		...
		if usingWorkbench jump #ifnot_LHvMDM
		...
		#while_ATXzKx
			...
			set ingrediantList
			ifnot canAfford jump #if_IwrzQr
				...
				#while_WskOuW
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_WskOuW
				msg {ingrediantList}
			#if_IwrzQr
			setadd i 1
		if i|<|{recipes.Length} jump #while_ATXzKx
	...
	#while_LCFmnb
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_LCFmnb
	msg &eTools:
...
#doCraft
	...
	#while_dKwTJb
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_dKwTJb
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_OPSlyb
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_OPSlyb
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_vOEoMU
	...
	#while_mIBRtf
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_mIBRtf
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_quKUsG
		set {runArg1} {runArg2}
		quit
	#ifnot_quKUsG
	set i 0
	#while_cHCgrB
		ifnot blocks[{i}].name|=|runArg2 jump #if_hhNXtO
			set {runArg1} {i}
			quit
		#if_hhNXtO
		setadd i 1
	if i|<|{blocks.Length} jump #while_cHCgrB
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_qchYje
		ifnot recipes[{i}].output.id|=|bid jump #if_QQoDox
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_BhXFnv
				set {pname} {i}
				quit
			#if_BhXFnv
		#if_QQoDox
		setadd i 1
	if i|<|{recipes.Length} jump #while_qchYje
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_qzQbwh
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_vMxoeM
...
#use[68]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_MYfxld
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_sJvAch
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
