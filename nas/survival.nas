...
#tick
	...
	if hp|=|prevhp jump #ifnot_pMshwtvacQGLwYsr
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_pMshwtvacQGLwYsr
	...
	ifnot iframes|>|0 jump #if_wfWraFCYLcBbzdvi
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_wfWraFCYLcBbzdvi
	ifnot fireticks|>|0 jump #if_eHHYLslkRRljFuFS
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_YECiOfvrctpMgHkB
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_IZWvTgojBdUbXVDf
...
#mine
	...
	ifnot minetimer|>|0 jump #if_LWVdNDxnAxrIXdpq
	...
	#if_LWVdNDxnAxrIXdpq
	set minepos
	if toomuch jump #ifnot_uqiXqtKoFiNJCjKP
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_uqiXqtKoFiNJCjKP
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_vPEpFyxhsWkQtUsc
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_UhOPjHyAqDSdRybW
		set {runArg1} {runArg2}
		quit
	#if_UhOPjHyAqDSdRybW
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_WTcBSIZNduikDDNZ
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_WTcBSIZNduikDDNZ
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_hueLBGdpLqAqvyEJ
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_LqmjHmuLfSFZagUv
		#while_AUEQBUYrHZpweEYO
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_AUEQBUYrHZpweEYO
	#if_LqmjHmuLfSFZagUv
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_HfjvexZersffIhvG
		#while_LgspdQvFHDZnkiik
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_LgspdQvFHDZnkiik
	#if_HfjvexZersffIhvG
quit

#input
	ifnot runArg1|=|"craft" jump #if_VmJYHdJrwetaPMSK
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_KdJSZZSTshpTLwkf
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_reKAXrwKIhRWHClp
				msg &cInvalid item name or ID
				quit
			#if_reKAXrwKIhRWHClp
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_tiaUzRMHuZDGgLUG
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_tiaUzRMHuZDGgLUG
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_KdJSZZSTshpTLwkf
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_EfucnLfcrxVJvKUb
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_EfucnLfcrxVJvKUb
		set i 0
		#while_MIFFEbtuYBrdooRh
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_NMhZXQuHMqrZNYAH
				...
				#while_iNRSYeVvPnzQgcjf
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_iNRSYeVvPnzQgcjf
				msg {ingrediantList}
			#if_NMhZXQuHMqrZNYAH
			setadd i 1
		if i|<|{recipes.Length} jump #while_MIFFEbtuYBrdooRh
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_VmJYHdJrwetaPMSK
	set i 0
	msg &eResources:
	#while_oiqiASCTBNPlOEPl
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_oiqiASCTBNPlOEPl
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_iihteOtcqOjxkFIL
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_iihteOtcqOjxkFIL
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_ogeXMRaOBaXmpjbT
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_ogeXMRaOBaXmpjbT
	#while_JhhZjWcouLsMGKKN
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_JhhZjWcouLsMGKKN
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_tETwSTodSYKaxYVv
		set {runArg1} {runArg2}
		quit
	#ifnot_tETwSTodSYKaxYVv
	set i 0
	#while_GUhlNGLkHklUUgDG
		ifnot blocks[{i}].name|=|runArg2 jump #if_rAnjqeFgANGDiPJC
			set {runArg1} {i}
			quit
		#if_rAnjqeFgANGDiPJC
		setadd i 1
	if i|<|{blocks.Length} jump #while_GUhlNGLkHklUUgDG
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_LpxuXjeQhamkULjx
		ifnot recipes[{i}].output.id|=|bid jump #if_kutcztnqQFAOfVBb
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_cAGqQOswiLBJJMDu
				set {pname} {i}
				quit
			#if_cAGqQOswiLBJJMDu
		#if_kutcztnqQFAOfVBb
		setadd i 1
	if i|<|{recipes.Length} jump #while_LpxuXjeQhamkULjx
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_lVRVnjpFCBBinMyE
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_wnGalkDhXxxZfhSY
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
		#if_wnGalkDhXxxZfhSY
	#ifnot_lVRVnjpFCBBinMyE
quit

#use[68]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_uXCsUQVrjPSCJVrT
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_SeJfVsvXiyVOfguw
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
		#if_SeJfVsvXiyVOfguw
	#ifnot_uXCsUQVrjPSCJVrT
