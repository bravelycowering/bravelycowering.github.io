...
#tick
	...
	if hp|=|prevhp jump #ifnot_iKgEGRNSvMdKITUT
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_iKgEGRNSvMdKITUT
	...
	ifnot iframes|>|0 jump #if_eTeFARDPyISPxVff
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_eTeFARDPyISPxVff
	ifnot fireticks|>|0 jump #if_hsdImjbmnMALRGch
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_TYBBocmHuRdeVZVG
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_DbWGOgPbSqCmbpaS
...
#mine
	...
	ifnot minetimer|>|0 jump #if_xOyYKFwYFpmNcSnv
	...
	#if_xOyYKFwYFpmNcSnv
	set minepos
	if toomuch jump #ifnot_mMEoYKSLeIrzVahV
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_mMEoYKSLeIrzVahV
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_gtaEriJHhgUZbyJY
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_zMYwOdAnQRdyeipC
		set {runArg1} {runArg2}
		quit
	#if_zMYwOdAnQRdyeipC
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_xOyNJxhTiitNBBWG
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_xOyNJxhTiitNBBWG
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_wdqlBRRfjzFjaTef
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_KjUjsUminZtkOtBf
		#while_sXORftDhPqddpYmz
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_sXORftDhPqddpYmz
	#if_KjUjsUminZtkOtBf
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_NnhoZkXfgepQisBI
		#while_LOGIEiLgEGOMwFzx
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_LOGIEiLgEGOMwFzx
	#if_NnhoZkXfgepQisBI
quit

#input
	ifnot runArg1|=|"craft" jump #if_DgAbzSsUUnrzYjoc
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_bXoOjMOSHYHLrqXW
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_BzYPglqgefgIsgzr
				msg &cInvalid item name or ID
				quit
			#if_BzYPglqgefgIsgzr
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_LDuKEJXPYDdvLbfn
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_LDuKEJXPYDdvLbfn
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_bXoOjMOSHYHLrqXW
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_oAtaaPoPOUtWHgYJ
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_oAtaaPoPOUtWHgYJ
		set i 0
		#while_laCJSCKMZjWtSrYY
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_ADaCcAHgyOEmZwgE
				...
				#while_mmBbipfFkVrMiWzc
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_mmBbipfFkVrMiWzc
				msg {ingrediantList}
			#if_ADaCcAHgyOEmZwgE
			setadd i 1
		if i|<|{recipes.Length} jump #while_laCJSCKMZjWtSrYY
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_DgAbzSsUUnrzYjoc
	set i 0
	msg &eResources:
	#while_LxozmzsRLuanOqkB
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_LxozmzsRLuanOqkB
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_tAsNQsaenmUuAYgS
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_tAsNQsaenmUuAYgS
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_IuOaYSFTkuuPFBot
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_IuOaYSFTkuuPFBot
	#while_BvTkCAhFfBVjPuXS
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_BvTkCAhFfBVjPuXS
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_usfqXDebbMXqDHYW
		set {runArg1} {runArg2}
		quit
	#ifnot_usfqXDebbMXqDHYW
	set i 0
	#while_LKZmHdEVPBAHtzjZ
		ifnot blocks[{i}].name|=|runArg2 jump #if_tGtMVEIiGnfDSIno
			set {runArg1} {i}
			quit
		#if_tGtMVEIiGnfDSIno
		setadd i 1
	if i|<|{blocks.Length} jump #while_LKZmHdEVPBAHtzjZ
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_sMADvSCexPyUqTkN
		ifnot recipes[{i}].output.id|=|bid jump #if_rEKrXINzVNeQNmvo
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_BVQpOjtZzAhFCBrR
				set {pname} {i}
				quit
			#if_BVQpOjtZzAhFCBrR
		#if_rEKrXINzVNeQNmvo
		setadd i 1
	if i|<|{recipes.Length} jump #while_sMADvSCexPyUqTkN
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_NVrgWjhEGVkHLcHR
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_YilgVNGlPQaEBkTH
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
		#if_YilgVNGlPQaEBkTH
	#ifnot_NVrgWjhEGVkHLcHR
quit

#use[68]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_eKEkjYPIMVPugeHP
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_UeymypVXTslurTxY
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
		#if_UeymypVXTslurTxY
	#ifnot_eKEkjYPIMVPugeHP
