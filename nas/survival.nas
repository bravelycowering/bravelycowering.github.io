...
#tick
	...
	ifnot blocks[{id}].kills jump #if_FSSRvLoJzlyQuwAX
		msg your died
		kill
	#if_FSSRvLoJzlyQuwAX
...
#mine
	...
	ifnot minetimer|>|0 jump #if_McOpVhTlzxGMqKgP
	...
	#if_McOpVhTlzxGMqKgP
	set minepos
	if toomuch jump #ifnot_OWOcJYkmPwwWoyku
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_OWOcJYkmPwwWoyku
...
#give
	ifnot isTool({runArg1}) jump #if_eNTDSpJIiitgkPZm
		set {runArg1} {runArg2}
		quit
	#if_eNTDSpJIiitgkPZm
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_PGJyMQQKkwXHRWzn
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_PGJyMQQKkwXHRWzn
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_QkdhahLwiPnIUxAJ
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_KoChejWTIkMgPNcf
		#while_ANBLLmbLrQuvvZaa
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_ANBLLmbLrQuvvZaa
	#if_KoChejWTIkMgPNcf
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_ImrZZWzAQrSLeKcp
		#while_dqHhLMlCwoGVAlqs
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_dqHhLMlCwoGVAlqs
	#if_ImrZZWzAQrSLeKcp
quit

#input
	ifnot runArg1|=|"craft" jump #if_GsYwuHudXMAgKVnm
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_BZLDZbpuMLQcJuvR
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_JGSxtuRzwwfvquyR
				msg &cInvalid item name or ID
				quit
			#if_JGSxtuRzwwfvquyR
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_hpjGDXKFIEiOmHzI
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_hpjGDXKFIEiOmHzI
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_BZLDZbpuMLQcJuvR
		if usingWorkbench msg &eWorkbench Recipes:
		else msg &eRecipes:
		set i 0
		#while_qmSdEFBkdBZqtCxB
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_phFpwkHWhIYjjVWU
				...
				#while_ObafOlInXzzjWosx
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_ObafOlInXzzjWosx
				msg {ingrediantList}
			#if_phFpwkHWhIYjjVWU
			setadd i 1
		if i|<|{recipes.Length} jump #while_qmSdEFBkdBZqtCxB
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_GsYwuHudXMAgKVnm
	set i 0
	msg &eResources:
	#while_IIXruDLfwQoAnVDE
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_IIXruDLfwQoAnVDE
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_dtzTfblMRXHhHCJQ
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_dtzTfblMRXHhHCJQ
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_dftgFoSNuBdfhyTh
		if {recipes[{runArg1}].output.count}|>=|count quit
	#if_dftgFoSNuBdfhyTh
	#while_qNlOACSzsdNAhKkr
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_qNlOACSzsdNAhKkr
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_qhMqABnWxBHkyHSH
		set {runArg1} {runArg2}
		quit
	#ifnot_qhMqABnWxBHkyHSH
	set i 0
	#while_RwcRXemxknhipQWR
		ifnot blocks[{i}].name|=|runArg2 jump #if_dyzCXgnAFLYPBwFP
			set {runArg1} {i}
			quit
		#if_dyzCXgnAFLYPBwFP
		setadd i 1
	if i|<|{blocks.Length} jump #while_RwcRXemxknhipQWR
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_rekyymfYCotLTEEp
		ifnot recipes[{i}].output.id|=|bid jump #if_aWexosHhQumoHGxu
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_VKXbFSoiTSWJjEHn
				set {pname} {i}
				quit
			#if_VKXbFSoiTSWJjEHn
		#if_aWexosHhQumoHGxu
		setadd i 1
	if i|<|{recipes.Length} jump #while_rekyymfYCotLTEEp
quit
