...
#tick
	...
	ifnot blocks[{id}].kills jump #if_MLyVFEZHSTKvkOed
		msg your died
		kill
	#if_MLyVFEZHSTKvkOed
...
#mine
	...
	ifnot minetimer|>|0 jump #if_kPEizuDhkswcyahI
	...
	#if_kPEizuDhkswcyahI
	set minepos
	if toomuch jump #ifnot_diYlEXUnyaIYEXPN
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_diYlEXUnyaIYEXPN
...
#give
	ifnot isTool({runArg1}) jump #if_qxiGNJVtbSoNCHli
		set {runArg1} {runArg2}
		quit
	#if_qxiGNJVtbSoNCHli
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_tjWjOfFyJJZHZwIm
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_tjWjOfFyJJZHZwIm
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_yHipAuWUlpzTRXkn
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_qPpVqpZTWuotZsuw
		#while_vTsYUImZoFYQqtBf
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_vTsYUImZoFYQqtBf
	#if_qPpVqpZTWuotZsuw
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_LpEhjKMEDCJqlfmK
		#while_tbGTWATpiNlXaVBL
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_tbGTWATpiNlXaVBL
	#if_LpEhjKMEDCJqlfmK
quit

#input
	ifnot runArg1|=|"craft" jump #if_cxKuFIRrANhbmSTi
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_YANRiswMhYWmCCeI
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_DiqEmkPQidmpySFf
				msg &cInvalid item name or ID
				quit
			#if_DiqEmkPQidmpySFf
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_uYTyuPfFJQnrKBBm
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_uYTyuPfFJQnrKBBm
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_YANRiswMhYWmCCeI
		if usingWorkbench msg &eWorkbench Recipes:
		else msg &eRecipes:
		set i 0
		#while_XoDTTBTKBTvRnYlt
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_ndlzlqBbAfnmRlCv
				...
				#while_HVfqzejmWhxQDoVZ
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_HVfqzejmWhxQDoVZ
				msg {ingrediantList}
			#if_ndlzlqBbAfnmRlCv
			setadd i 1
		if i|<|{recipes.Length} jump #while_XoDTTBTKBTvRnYlt
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_cxKuFIRrANhbmSTi
	set i 0
	msg &eResources:
	#while_wQWvEDnrDDSvpocR
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_wQWvEDnrDDSvpocR
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_DhEgMHcByQpHeyfL
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_DhEgMHcByQpHeyfL
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_TihgMQLCEUeGWMEK
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_TihgMQLCEUeGWMEK
	#while_hLsFoVdBLRajVZJc
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_hLsFoVdBLRajVZJc
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_tptMszSKtHGqZZMA
		set {runArg1} {runArg2}
		quit
	#ifnot_tptMszSKtHGqZZMA
	set i 0
	#while_AkSThtMHmtHhRgCT
		ifnot blocks[{i}].name|=|runArg2 jump #if_TmiCnSrnDvEhrIxA
			set {runArg1} {i}
			quit
		#if_TmiCnSrnDvEhrIxA
		setadd i 1
	if i|<|{blocks.Length} jump #while_AkSThtMHmtHhRgCT
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_pTBxAprYPbauYNqC
		ifnot recipes[{i}].output.id|=|bid jump #if_dkUGdZJbcLkXtfsJ
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_OZXmRHUokgdohiPC
				set {pname} {i}
				quit
			#if_OZXmRHUokgdohiPC
		#if_dkUGdZJbcLkXtfsJ
		setadd i 1
	if i|<|{recipes.Length} jump #while_pTBxAprYPbauYNqC
quit
