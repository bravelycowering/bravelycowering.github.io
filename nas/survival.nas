...
#tick
	...
	ifnot blocks[{id}].kills jump #if_MfLqgHikfBlFRPgG
		msg your died
		kill
	#if_MfLqgHikfBlFRPgG
...
#mine
	...
	ifnot minetimer|>|0 jump #if_ZSUGalSBcPMIamfl
	...
	#if_ZSUGalSBcPMIamfl
	set minepos
	if toomuch jump #ifnot_ZiAgKEXHznldMVIo
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_ZiAgKEXHznldMVIo
...
#give
	ifnot isTool({runArg1}) jump #if_DSCDWwBmPlBsaceS
		set {runArg1} {runArg2}
		quit
	#if_DSCDWwBmPlBsaceS
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_JGZvAACyhvxTgXkz
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_JGZvAACyhvxTgXkz
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_dQodOHoRKYKRHOyh
		...
		#while_WHnRJLUPXHqJftFt
			...
			ifnot canAfford jump #if_kxfTtmtncbjNmZZC
				...
				#while_bHmkBdGUduXzRbtW
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_bHmkBdGUduXzRbtW
				msg {ingrediantList}
			#if_kxfTtmtncbjNmZZC
			setadd i 1
		if i|<|{recipes.Length} jump #while_WHnRJLUPXHqJftFt
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_alBYAXOVQSCuBXIg
	set i 0
	msg &eResources:
	#while_JqvcamPBBEQHwuwi
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_JqvcamPBBEQHwuwi
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_CwIHcicrjyNlLCno
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_CwIHcicrjyNlLCno
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_lfDRZrqJbqeQkkny
		if {recipes[{runArg1}].output.id}|>=|count quit
	#if_lfDRZrqJbqeQkkny
	#while_IyxuWSmEIXpGXeOc
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_IyxuWSmEIXpGXeOc
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_EouQgPXaMgelRZib
		set {runArg1} {runArg2}
		quit
	#ifnot_EouQgPXaMgelRZib
	set i 0
	#while_WXuybvkgrQVMnUvM
		ifnot blocks[{i}].name|=|runArg2 jump #if_XlWbncvpfvajWohS
			set {runArg1} {i}
			quit
		#if_XlWbncvpfvajWohS
		setadd i 1
	if i|<|{blocks.Length} jump #while_WXuybvkgrQVMnUvM
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_rDCDYSBubVfTYjhv
		ifnot recipes[{i}].output.id|=|bid jump #if_mwxiQWemmqpLZrAv
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_NOBbSyZZYnMDyazF
				set {pname} {i}
				quit
			#if_NOBbSyZZYnMDyazF
		#if_mwxiQWemmqpLZrAv
		setadd i 1
	if i|<|{recipes.Length} jump #while_rDCDYSBubVfTYjhv
quit
