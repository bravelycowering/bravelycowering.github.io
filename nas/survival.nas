...
// start scope
	set l_aQEpCa_PrevPlayerCoords 
	set l_bkLKVv_prevhp 
	set l_DAjXSS_hpbar 
	set l_RQjkfA_myblock 
	#tick
		call #getblock|l_RQjkfA_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_RQjkfA_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_RQjkfA_myblock}].extinguishFire jump #if_GLgBwL
			ifnot fireticks|>|0 jump #if_LXHDca
				gui barSize 0
				set fireticks 0
			#if_LXHDca
		#if_GLgBwL
		ifnot blocks[{l_RQjkfA_myblock}].damage|=|"" call #damage|{blocks[{l_RQjkfA_myblock}].damage}|{blocks[{l_RQjkfA_myblock}].damageType}
		ifnot PlayerCoords|=|l_aQEpCa_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_aQEpCa_PrevPlayerCoords set usingStonecutter false
		set l_aQEpCa_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_bkLKVv_prevhp jump #ifnot_PnBDsJ
			set l_bkLKVv_prevhp {hp}
			call #makebar|l_DAjXSS_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_DAjXSS_hpbar}
		#ifnot_PnBDsJ
		...
		ifnot iframes|>|0 jump #if_Onxlqo
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_Onxlqo
		ifnot fireticks|>|0 jump #if_OfFUyR
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_ZopeIG
...
#mine
	...
	ifnot minetimer|>|0 jump #if_qldyFF
...
#give
	ifnot isTool({runArg1}) jump #if_nBLrrD
		set {runArg1} {runArg2}
		quit
	#if_nBLrrD
...
#giveall
	...
	#while_dRnUCO
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_dRnUCO
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_rCgVKk
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_rCgVKk
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_cegzuQ
...
#setblock
	if allowMapChanges jump #ifnot_jfiKox
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_IFsCeK
		#while_qJUHEV
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_qJUHEV
	#if_IFsCeK
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_cwNyHj
		#while_jAcsod
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_jAcsod
	#if_cwNyHj
quit

#input
	ifnot runArg1|=|"craft" jump #if_ledMBe
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_fIpziA
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_BtfNoK
				msg &cInvalid item name or ID
				quit
			#if_BtfNoK
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_VrXHqr
		...
		if usingWorkbench jump #ifnot_VrieqO
		...
		#while_GzypfE
			...
			set ingrediantList
			ifnot canAfford jump #if_mQgklJ
				...
				#while_kEzbGG
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_kEzbGG
				msg {ingrediantList}
			#if_mQgklJ
			setadd i 1
		if i|<|{recipes.Length} jump #while_GzypfE
	...
	#while_hkGwlT
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_hkGwlT
	msg &eTools:
...
#doCraft
	...
	#while_bIOaic
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_bIOaic
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_GQHcoE
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_GQHcoE
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_zJPrjV
	...
	#while_uWcBEy
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_uWcBEy
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_ppAxEm
		set {runArg1} {runArg2}
		quit
	#ifnot_ppAxEm
	set i 0
	#while_akZhyr
		ifnot blocks[{i}].name|=|runArg2 jump #if_VYGWis
			set {runArg1} {i}
			quit
		#if_VYGWis
		setadd i 1
	if i|<|{blocks.Length} jump #while_akZhyr
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_wHbrZZ
		ifnot recipes[{i}].output.id|=|bid jump #if_kDDqJl
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_SqhelL
				set {pname} {i}
				quit
			#if_SqhelL
		#if_kDDqJl
		setadd i 1
	if i|<|{recipes.Length} jump #while_wHbrZZ
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_HXsMRR
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_QHDqbq
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_mNBVnf
		call #take|80|1
		call #give|70|1
	#if_mNBVnf
quit
