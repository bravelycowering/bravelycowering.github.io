...
// start scope
	set l_yjvJft_PrevPlayerCoords 
	set l_wUGTRJ_prevhp 
	set l_gxLpgz_hpbar 
	set l_wyIDOv_firebar 
	set l_cTovAC_myblock 
	set l_mhZYhF_firetickmod 
	set l_tOcarr_temp 
	#tick
		call #getblock|l_cTovAC_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_cTovAC_myblock}].catchFire jump #if_aHlSuy
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_aHlSuy
		ifnot blocks[{l_cTovAC_myblock}].extinguishFire jump #if_dFqeBY
			ifnot fireticks|>|0 jump #if_sbpmRk
				gui barSize 0
				set fireticks 0
			#if_sbpmRk
		#if_dFqeBY
		ifnot blocks[{l_cTovAC_myblock}].damage|=|"" call #damage|{blocks[{l_cTovAC_myblock}].damage}|{blocks[{l_cTovAC_myblock}].damageType}
		ifnot PlayerCoords|=|l_yjvJft_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_yjvJft_PrevPlayerCoords set usingStonecutter false
		set l_yjvJft_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_wUGTRJ_prevhp jump #ifnot_JbKcdS
			set l_wUGTRJ_prevhp {hp}
			call #makebar|l_gxLpgz_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_gxLpgz_hpbar}
		#ifnot_JbKcdS
		...
		ifnot iframes|>|0 jump #if_SJMfDj
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_SJMfDj
		ifnot fireticks|>|0 jump #if_qIDScv
			setsub fireticks 1
			set l_mhZYhF_firetickmod {fireticks}
			setmod l_mhZYhF_firetickmod 10
			ifnot l_mhZYhF_firetickmod|=|0 jump #if_NSBmAA
				ifnot fireticks|>|0 jump #if_DCpBjh
					set l_tOcarr_temp {fireticks}
					setdiv l_tOcarr_temp 10
					call #makecharbar|l_wyIDOv_firebar|▐|6|{l_tOcarr_temp}|10
					cpemsg smallannounce {l_wyIDOv_firebar}
				#if_DCpBjh
...
#mine
	...
	ifnot minetimer|>|0 jump #if_fmqakx
...
#give
	ifnot isTool({runArg1}) jump #if_AtUqnk
		set {runArg1} {runArg2}
		quit
	#if_AtUqnk
...
#giveall
	...
	#while_VRcaRR
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_VRcaRR
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_DonOGl
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_LiKThb
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_tSDpXi
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_zdOnrr
	...
	call #getblock|id|{x}|{y}|{z}
	if blocks[{id}].mergeInto|=|"" jump #ifnot_liznJc
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_OtwRPl
			call #take|{playerHeldBlock}|1
			jump #setblock|{blocks[{id}].mergeInto}|{x}|{y}|{z}
			quit
		#if_OtwRPl
	...
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_cHlzwJ
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_TCeuZs
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_HopDZl
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_ANiHBD
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_sknwaN
		#while_TQKUUk
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_TQKUUk
	#if_sknwaN
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_wSEGkz
		#while_ZGrITU
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_ZGrITU
	#if_wSEGkz
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_mjdGhi
		#while_XwNYhP
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_XwNYhP
	#if_mjdGhi
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_WWpleG
		#while_UxBgBl
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_UxBgBl
	#if_WWpleG
quit

#input
	if runArg1|=|"changes" jump #changelog
	ifnot runArg1|=|"craft" jump #if_daxeCg
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_iRnTPQ
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_altIOM
				msg &cInvalid item name or ID
				quit
			#if_altIOM
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_dOGLrJ
		...
		if usingWorkbench jump #ifnot_soknbU
		...
		#while_YkRkCK
			...
			set ingrediantList
			ifnot canAfford|>|0 jump #if_XbGxlH
				...
				#while_jEtifR
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_jEtifR
				msg {ingrediantList}
			#if_XbGxlH
			setadd i 1
		if i|<|{recipes.Length} jump #while_YkRkCK
	...
	#while_hDWTTo
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_hDWTTo
	msg &eTools:
...
#doCraft
	...
	#while_AANkiH
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_AANkiH
...
#checkRecipeAfford
	set j 0
	set {runArg2} 999
	if recipes[{runArg1}].condition|=|"" jump #ifnot_wTWoej
		ifnot {recipes[{runArg1}].condition} set {runArg2} 0
	#ifnot_wTWoej
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_FUyXDm
	...
	#while_tOETNK
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_tOETNK
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_clAGpB
		set {runArg1} {runArg2}
		quit
	#ifnot_clAGpB
	set i 0
	#while_yEfiXQ
		ifnot blocks[{i}].name|=|runArg2 jump #if_fdDTOU
			set {runArg1} {i}
			quit
		#if_fdDTOU
		setadd i 1
	if i|<|{blocks.Length} jump #while_yEfiXQ
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_dhiLxx
		ifnot recipes[{i}].output.id|=|bid jump #if_kRJRhW
			call #checkRecipeAfford|{i}|canAfford
			ifnot canAfford|>=|c jump #if_eFTFWk
				set {pname} {i}
				quit
			#if_eFTFWk
		#if_kRJRhW
		setadd i 1
	if i|<|{recipes.Length} jump #while_dhiLxx
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_oYLGPK
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_RtAiIx
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_Hjebxx
		call #take|80|1
		call #give|70|1
	#if_Hjebxx
quit
