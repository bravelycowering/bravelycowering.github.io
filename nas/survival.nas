...
// start scope
	set l_ThOjKC_PrevPlayerCoords 
	set l_kIQJGC_prevhp 
	set l_ijmWmr_hpbar 
	set l_jmyJKz_myblock 
	#tick
		call #getblock|l_jmyJKz_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_jmyJKz_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_jmyJKz_myblock}].extinguishFire jump #if_MdFmBF
			ifnot fireticks|>|0 jump #if_KbMCVp
				gui barSize 0
				set fireticks 0
			#if_KbMCVp
		#if_MdFmBF
		ifnot blocks[{l_jmyJKz_myblock}].damage|=|"" call #damage|{blocks[{l_jmyJKz_myblock}].damage}|{blocks[{l_jmyJKz_myblock}].damageType}
		ifnot PlayerCoords|=|l_ThOjKC_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_ThOjKC_PrevPlayerCoords set usingStonecutter false
		set l_ThOjKC_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_kIQJGC_prevhp jump #ifnot_wUxZQu
			set l_kIQJGC_prevhp {hp}
			call #makebar|l_ijmWmr_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_ijmWmr_hpbar}
		#ifnot_wUxZQu
		...
		ifnot iframes|>|0 jump #if_wRXcQZ
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_wRXcQZ
		ifnot fireticks|>|0 jump #if_yQaBvG
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_gOlaXJ
...
#mine
	...
	ifnot minetimer|>|0 jump #if_pnOdRc
...
#give
	ifnot isTool({runArg1}) jump #if_bwDSJy
		set {runArg1} {runArg2}
		quit
	#if_bwDSJy
...
#giveall
	...
	#while_mbsioz
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_mbsioz
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_nTsbcx
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_nTsbcx
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_vhAtPI
...
#setblock
	if allowMapChanges jump #ifnot_BPHFYo
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_khyjTW
		#while_OrkdJT
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_OrkdJT
	#if_khyjTW
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_alExDo
		#while_txLcbp
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_txLcbp
	#if_alExDo
quit

#input
	ifnot runArg1|=|"craft" jump #if_lqbqBS
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_dvuzld
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_pKdewf
				msg &cInvalid item name or ID
				quit
			#if_pKdewf
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_YsrsdK
		...
		if usingWorkbench jump #ifnot_vlPWjy
		...
		#while_nPvSvU
			...
			set ingrediantList
			ifnot canAfford jump #if_AdEcNB
				...
				#while_YEJCdr
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_YEJCdr
				msg {ingrediantList}
			#if_AdEcNB
			setadd i 1
		if i|<|{recipes.Length} jump #while_nPvSvU
	...
	#while_FLcGxw
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_FLcGxw
	msg &eTools:
...
#doCraft
	...
	#while_aGlRSq
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_aGlRSq
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_GrKjQh
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_GrKjQh
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_qZIHnr
	...
	#while_sJlxFl
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_sJlxFl
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_ihDaAZ
		set {runArg1} {runArg2}
		quit
	#ifnot_ihDaAZ
	set i 0
	#while_wOPsCL
		ifnot blocks[{i}].name|=|runArg2 jump #if_EbrJpj
			set {runArg1} {i}
			quit
		#if_EbrJpj
		setadd i 1
	if i|<|{blocks.Length} jump #while_wOPsCL
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_nmTTPW
		ifnot recipes[{i}].output.id|=|bid jump #if_bBFybD
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_JwKRgv
				set {pname} {i}
				quit
			#if_JwKRgv
		#if_bBFybD
		setadd i 1
	if i|<|{recipes.Length} jump #while_nmTTPW
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_wcPyTo
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_YDpeLZ
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_SObxCu
		call #take|80|1
		call #give|70|1
	#if_SObxCu
quit
