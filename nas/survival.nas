...
#tick
	...
	if hp|=|prevhp jump #ifnot_ArlaVwZDTNEGmEzs
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_ArlaVwZDTNEGmEzs
	...
	ifnot iframes|>|0 jump #if_wStxDXTLhLqgBeto
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_wStxDXTLhLqgBeto
	ifnot fireticks|>|0 jump #if_ksiLkKwEedQBIPDe
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_FDYauMTJPBzZYkdB
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_cBAuImxCNyyFPrcD
...
#mine
	...
	ifnot minetimer|>|0 jump #if_pOLLQEYMAADxUcQw
	...
	#if_pOLLQEYMAADxUcQw
	set minepos
	if toomuch jump #ifnot_wPrdnQAOMVACccoj
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_wPrdnQAOMVACccoj
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_IZsfTZAYXDvHcbRd
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_hXZKzDymxfgLdftJ
		set {runArg1} {runArg2}
		quit
	#if_hXZKzDymxfgLdftJ
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_NqDvwioiIrNolzZk
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_NqDvwioiIrNolzZk
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_vIQyEbhqTZGZWWqy
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_KcyPgtjihWITCMFw
		#while_BhPwlUDIbEumcMIX
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_BhPwlUDIbEumcMIX
	#if_KcyPgtjihWITCMFw
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_pldJKMddvRxDIQlg
		#while_nxiAfZBveRObVKIH
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_nxiAfZBveRObVKIH
	#if_pldJKMddvRxDIQlg
quit

#input
	ifnot runArg1|=|"craft" jump #if_MlOeMNDrIMYDIWXk
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_OGrpTMtWKyeSUjyX
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_HQxswlhbtPJbFurw
				msg &cInvalid item name or ID
				quit
			#if_HQxswlhbtPJbFurw
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_jtnNpLfWUVoyRYqI
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_jtnNpLfWUVoyRYqI
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_OGrpTMtWKyeSUjyX
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_ZCaMNWjlZyGXJWGx
			ifnot usingStonecutter jump #if_NoIzgsvfcToPHCunmsg &eStonecutter Recipes:
			else msg &eRecipes:
		#if_NoIzgsvfcToPHCun
		set i 0
		#while_WlrCoBPwCJPkVGbj
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_xJFxCZwxwdntcDvJ
				...
				#while_wqGxaSzDbveZneqW
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_wqGxaSzDbveZneqW
				msg {ingrediantList}
			#if_xJFxCZwxwdntcDvJ
			setadd i 1
		if i|<|{recipes.Length} jump #while_WlrCoBPwCJPkVGbj
		msg &eType &a/in craft [name]&e to craft something
		quit
	#ifnot_ZCaMNWjlZyGXJWGx
	set i 0
	msg &eResources:
	#while_eIADyCnpxnZQJKdf
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_eIADyCnpxnZQJKdf
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_NkKsLAGdoVdbNfqy
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_NkKsLAGdoVdbNfqy
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_FivHsOBmLmIKidDx
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_FivHsOBmLmIKidDx
	#while_fGXdadQUtlhNpmHy
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_fGXdadQUtlhNpmHy
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_cHfaBWWRucViETVo
		set {runArg1} {runArg2}
		quit
	#ifnot_cHfaBWWRucViETVo
	set i 0
	#while_UMWSBlkVMpAdygAs
		ifnot blocks[{i}].name|=|runArg2 jump #if_xnRDNGCtSnTtdQyW
			set {runArg1} {i}
			quit
		#if_xnRDNGCtSnTtdQyW
		setadd i 1
	if i|<|{blocks.Length} jump #while_UMWSBlkVMpAdygAs
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_QOoJQoujqfDmLDUC
		ifnot recipes[{i}].output.id|=|bid jump #if_cFpDhqlDKWDMkHtL
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_ISrJaFHaaaEtvFag
				set {pname} {i}
				quit
			#if_ISrJaFHaaaEtvFag
		#if_cFpDhqlDKWDMkHtL
		setadd i 1
	if i|<|{recipes.Length} jump #while_QOoJQoujqfDmLDUC
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_VzUOZnifFOrBEMVW
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_okRQFgCVqyWlEUwz
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_okRQFgCVqyWlEUwz
	#ifnot_VzUOZnifFOrBEMVW
quit
