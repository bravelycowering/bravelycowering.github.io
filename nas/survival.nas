...
// start scope
	set l_slcvBk_PrevPlayerCoords 
	set l_fLOAwf_prevhp 
	set l_OsLkQH_hpbar 
	set l_iFNoNB_myblock 
	#tick
		call #getblock|l_iFNoNB_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_iFNoNB_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_iFNoNB_myblock}].extinguishFire jump #if_bkIHmU
			ifnot fireticks|>|0 jump #if_IiAQuC
				gui barSize 0
				set fireticks 0
			#if_IiAQuC
		#if_bkIHmU
		ifnot blocks[{l_iFNoNB_myblock}].damage|=|"" call #damage|{blocks[{l_iFNoNB_myblock}].damage}|{blocks[{l_iFNoNB_myblock}].damageType}
		ifnot PlayerCoords|=|l_slcvBk_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_slcvBk_PrevPlayerCoords set usingStonecutter false
		set l_slcvBk_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_fLOAwf_prevhp jump #ifnot_LEPQkJ
			set l_fLOAwf_prevhp {hp}
			call #makebar|l_OsLkQH_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_OsLkQH_hpbar}
		#ifnot_LEPQkJ
		...
		ifnot iframes|>|0 jump #if_OktCbF
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_OktCbF
		ifnot fireticks|>|0 jump #if_ORpxzi
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_rxCyIZ
...
#mine
	...
	ifnot minetimer|>|0 jump #if_WPImiZ
...
#give
	ifnot isTool({runArg1}) jump #if_hZaKcg
		set {runArg1} {runArg2}
		quit
	#if_hZaKcg
...
#giveall
	...
	#while_RdchYC
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_RdchYC
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_YLnmId
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_YLnmId
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_KXMlqi
...
#setblock
	if allowMapChanges jump #ifnot_efqlPn
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_WEHLhS
		#while_JVaqEp
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_JVaqEp
	#if_WEHLhS
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_yVkZKA
		#while_hjHDjX
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_hjHDjX
	#if_yVkZKA
quit

#input
	ifnot runArg1|=|"craft" jump #if_qksDLZ
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_qzgTCv
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_JRfQpz
				msg &cInvalid item name or ID
				quit
			#if_JRfQpz
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_RhWmum
		...
		if usingWorkbench jump #ifnot_BiIblf
		...
		#while_fLAdQA
			...
			set ingrediantList
			ifnot canAfford jump #if_iEVtth
				...
				#while_QLpBxZ
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_QLpBxZ
				msg {ingrediantList}
			#if_iEVtth
			setadd i 1
		if i|<|{recipes.Length} jump #while_fLAdQA
	...
	#while_nVQMvB
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_nVQMvB
	msg &eTools:
...
#doCraft
	...
	#while_LzxYHr
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_LzxYHr
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_XYIRvi
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_XYIRvi
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_MgRyfl
	...
	#while_ELODBZ
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_ELODBZ
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_ceWcFF
		set {runArg1} {runArg2}
		quit
	#ifnot_ceWcFF
	set i 0
	#while_FpoABJ
		ifnot blocks[{i}].name|=|runArg2 jump #if_OuzBCM
			set {runArg1} {i}
			quit
		#if_OuzBCM
		setadd i 1
	if i|<|{blocks.Length} jump #while_FpoABJ
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_xbxYkj
		ifnot recipes[{i}].output.id|=|bid jump #if_IfDOGT
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_HDCvWv
				set {pname} {i}
				quit
			#if_HDCvWv
		#if_IfDOGT
		setadd i 1
	if i|<|{recipes.Length} jump #while_xbxYkj
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_zQuunW
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_pcradw
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_LgCIyN
		call #take|80|1
		call #give|70|1
	#if_LgCIyN
quit
