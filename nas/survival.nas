...
// start scope
	set l_sDHLwi_PrevPlayerCoords 
	set l_weIcsA_prevhp 
	set l_ieBvUR_hpbar 
	set l_mUWhdq_firebar 
	set l_HwkZWw_myblock 
	set l_YBFkub_firetickmod 
	#tick
		call #getblock|l_HwkZWw_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_HwkZWw_myblock}].catchFire jump #if_RmMrTD
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_RmMrTD
		ifnot blocks[{l_HwkZWw_myblock}].extinguishFire jump #if_rkTSFf
			ifnot fireticks|>|0 jump #if_keYgcz
				gui barSize 0
				set fireticks 0
			#if_keYgcz
		#if_rkTSFf
		ifnot blocks[{l_HwkZWw_myblock}].damage|=|"" call #damage|{blocks[{l_HwkZWw_myblock}].damage}|{blocks[{l_HwkZWw_myblock}].damageType}
		ifnot PlayerCoords|=|l_sDHLwi_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_sDHLwi_PrevPlayerCoords set usingStonecutter false
		set l_sDHLwi_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_weIcsA_prevhp jump #ifnot_OiAcBo
			set l_weIcsA_prevhp {hp}
			call #makebar|l_ieBvUR_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_ieBvUR_hpbar}
		#ifnot_OiAcBo
		...
		ifnot iframes|>|0 jump #if_fvKwLZ
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_fvKwLZ
		ifnot fireticks|>|0 jump #if_MtGLTy
			setsub fireticks 1
			set l_YBFkub_firetickmod {fireticks}
			setmod l_YBFkub_firetickmod 10
			ifnot l_YBFkub_firetickmod|=|0 jump #if_uklYYb
				ifnot fireticks|>|0 jump #if_mhlOWG
					set temp {fireticks}
					setdiv temp 10
					call #makecharbar|l_mUWhdq_firebar|▐|6|{temp}|10
					cpemsg smallannounce {l_mUWhdq_firebar}
				#if_mhlOWG
...
#mine
	...
	ifnot minetimer|>|0 jump #if_RquYRE
...
#give
	ifnot isTool({runArg1}) jump #if_XUeIaz
		set {runArg1} {runArg2}
		quit
	#if_XUeIaz
...
#giveall
	...
	#while_NerQlJ
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_NerQlJ
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_eiCUWv
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_eiCUWv
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_iGVsdD
...
#setblock
	if allowMapChanges jump #ifnot_LDHvzZ
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_AQgSWx
		#while_GFyacj
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_GFyacj
	#if_AQgSWx
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_GUivmt
		#while_glYeHV
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_glYeHV
	#if_GUivmt
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_EOiVqA
		#while_KEiaUu
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_KEiaUu
	#if_EOiVqA
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_HYQAjH
		#while_HjKEYP
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_HjKEYP
	#if_HYQAjH
quit

#input
	ifnot runArg1|=|"craft" jump #if_FtHvKJ
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_hYIqjs
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_mYwQww
				msg &cInvalid item name or ID
				quit
			#if_mYwQww
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_MqtRSW
		...
		if usingWorkbench jump #ifnot_rmlIyF
		...
		#while_IrYwyI
			...
			set ingrediantList
			ifnot canAfford jump #if_TXKaRj
				...
				#while_gXCuBJ
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_gXCuBJ
				msg {ingrediantList}
			#if_TXKaRj
			setadd i 1
		if i|<|{recipes.Length} jump #while_IrYwyI
	...
	#while_ZUPnOT
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_ZUPnOT
	msg &eTools:
...
#doCraft
	...
	#while_KhtDrh
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_KhtDrh
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_oMAVGn
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_oMAVGn
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_TZJTjx
	...
	#while_VoHDJS
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_VoHDJS
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_BrBIzu
		set {runArg1} {runArg2}
		quit
	#ifnot_BrBIzu
	set i 0
	#while_rluuvX
		ifnot blocks[{i}].name|=|runArg2 jump #if_bERvEi
			set {runArg1} {i}
			quit
		#if_bERvEi
		setadd i 1
	if i|<|{blocks.Length} jump #while_rluuvX
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_HtIULG
		ifnot recipes[{i}].output.id|=|bid jump #if_BoucKj
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_hgPwKA
				set {pname} {i}
				quit
			#if_hgPwKA
		#if_BoucKj
		setadd i 1
	if i|<|{recipes.Length} jump #while_HtIULG
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_eNYZAK
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_KYXLhX
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_mMOcxy
		call #take|80|1
		call #give|70|1
	#if_mMOcxy
quit
