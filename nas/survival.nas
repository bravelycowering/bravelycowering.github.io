...
#tick
	...
	if hp|=|prevhp jump #ifnot_kwOtyeGupZjlOmRp
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_kwOtyeGupZjlOmRp
	...
	ifnot iframes|>|0 jump #if_cUohExUPsliZtspD
		setsub iframes 1
...
#damage
	...
	set iframes 4
	cs me ow
	ifnot hp|<=|0 jump #if_dJinwrtojLvbueZi
...
#mine
	...
	ifnot minetimer|>|0 jump #if_AisUCzhNYEmObaQI
	...
	#if_AisUCzhNYEmObaQI
	set minepos
	if toomuch jump #ifnot_zhcFvKwUewalKSyD
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_zhcFvKwUewalKSyD
...
#give
	ifnot isTool({runArg1}) jump #if_jBWSeSnbWwaxjMFu
		set {runArg1} {runArg2}
		quit
	#if_jBWSeSnbWwaxjMFu
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_kYWcrmCBDlUIuTKF
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_kYWcrmCBDlUIuTKF
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_WGKEYZLjBuVqgPwY
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_OUEmuxctZrDSQrkC
		#while_SpejkfPxcSbUnXVA
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_SpejkfPxcSbUnXVA
	#if_OUEmuxctZrDSQrkC
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_egWnBoDjurzhVekF
		#while_wFquGgKcliusJHYu
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_wFquGgKcliusJHYu
	#if_egWnBoDjurzhVekF
quit

#input
	ifnot runArg1|=|"craft" jump #if_eXhUSzZOhpbfANFJ
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_dPilcjWduGGcDbIu
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_qHakGkUeScWpDXRi
				msg &cInvalid item name or ID
				quit
			#if_qHakGkUeScWpDXRi
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_CYPrRGehwQppoELu
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_CYPrRGehwQppoELu
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_dPilcjWduGGcDbIu
		if usingWorkbench msg &eWorkbench Recipes:
		else msg &eRecipes:
		set i 0
		#while_FlnoranWgOAjmEQG
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_cGybEsbVyotzzdGZ
				...
				#while_RUOwkObZHfJirKQj
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_RUOwkObZHfJirKQj
				msg {ingrediantList}
			#if_cGybEsbVyotzzdGZ
			setadd i 1
		if i|<|{recipes.Length} jump #while_FlnoranWgOAjmEQG
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_eXhUSzZOhpbfANFJ
	set i 0
	msg &eResources:
	#while_WvnEsEdikHLqyBvT
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_WvnEsEdikHLqyBvT
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_mMRfoNhLFaFIvIyj
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_mMRfoNhLFaFIvIyj
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_zSAwgJjfWyyABgrk
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_zSAwgJjfWyyABgrk
	#while_MhTTGkQEuZhCrzof
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_MhTTGkQEuZhCrzof
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_cydqStOAPFDzeYAO
		set {runArg1} {runArg2}
		quit
	#ifnot_cydqStOAPFDzeYAO
	set i 0
	#while_MeImpjqdlDTkvnnH
		ifnot blocks[{i}].name|=|runArg2 jump #if_wzTkMyaUkwvJftnt
			set {runArg1} {i}
			quit
		#if_wzTkMyaUkwvJftnt
		setadd i 1
	if i|<|{blocks.Length} jump #while_MeImpjqdlDTkvnnH
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_IkxQdBFdguxfmvXP
		ifnot recipes[{i}].output.id|=|bid jump #if_TgtTiUMoYwNhadyT
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_WoQaQRHsIRMKkfmb
				set {pname} {i}
				quit
			#if_WoQaQRHsIRMKkfmb
		#if_TgtTiUMoYwNhadyT
		setadd i 1
	if i|<|{recipes.Length} jump #while_IkxQdBFdguxfmvXP
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_dqnAQfhhAXHZZflw
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_WRAimwpSnfHXMcCA
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_WRAimwpSnfHXMcCA
	#ifnot_dqnAQfhhAXHZZflw
quit
