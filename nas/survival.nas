...
// start scope
	set l_HPSkic_PrevPlayerCoords 
	set l_PxuSre_prevhp 
	set l_tBBdOA_hpbar 
	set l_GszRWq_myblock 
	#tick
		call #getblock|l_GszRWq_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_GszRWq_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_GszRWq_myblock}].extinguishFire jump #if_SeBxeN
			ifnot fireticks|>|0 jump #if_ScWtdS
				gui barSize 0
				set fireticks 0
			#if_ScWtdS
		#if_SeBxeN
		ifnot blocks[{l_GszRWq_myblock}].damage|=|"" call #damage|{blocks[{l_GszRWq_myblock}].damage}|{blocks[{l_GszRWq_myblock}].damageType}
		ifnot PlayerCoords|=|l_HPSkic_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_HPSkic_PrevPlayerCoords set usingStonecutter false
		set l_HPSkic_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_PxuSre_prevhp jump #ifnot_RdVUHK
			set l_PxuSre_prevhp {hp}
			call #makebar|l_tBBdOA_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_tBBdOA_hpbar}
		#ifnot_RdVUHK
		...
		ifnot iframes|>|0 jump #if_cmpRJE
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_cmpRJE
		ifnot fireticks|>|0 jump #if_yyRxQz
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_uQBHkq
...
#mine
	...
	ifnot minetimer|>|0 jump #if_gfKcYg
...
#give
	ifnot isTool({runArg1}) jump #if_CcwQzS
		set {runArg1} {runArg2}
		quit
	#if_CcwQzS
...
#giveall
	...
	#while_PZmVEL
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_PZmVEL
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_aQJjxv
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_aQJjxv
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_tSTJcy
...
#setblock
	if allowMapChanges jump #ifnot_SmZXXi
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_SZCcRk
		#while_dUMoIU
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_dUMoIU
	#if_SZCcRk
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_LpJjEo
		#while_VyCZoe
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_VyCZoe
	#if_LpJjEo
quit

#input
	ifnot runArg1|=|"craft" jump #if_RwJInQ
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_vhQkwz
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_nrNJzP
				msg &cInvalid item name or ID
				quit
			#if_nrNJzP
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_WCTCmD
		...
		if usingWorkbench jump #ifnot_ibtVDh
		...
		#while_EiiwKb
			...
			set ingrediantList
			ifnot canAfford jump #if_onQLhs
				...
				#while_xpvwTS
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_xpvwTS
				msg {ingrediantList}
			#if_onQLhs
			setadd i 1
		if i|<|{recipes.Length} jump #while_EiiwKb
	...
	#while_IsxmyB
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_IsxmyB
	msg &eTools:
...
#doCraft
	...
	#while_dYDurQ
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_dYDurQ
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_IxCrLL
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_IxCrLL
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_wDMnvS
	...
	#while_JTjKjv
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_JTjKjv
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_IbKhJD
		set {runArg1} {runArg2}
		quit
	#ifnot_IbKhJD
	set i 0
	#while_bOpOis
		ifnot blocks[{i}].name|=|runArg2 jump #if_ucqVHy
			set {runArg1} {i}
			quit
		#if_ucqVHy
		setadd i 1
	if i|<|{blocks.Length} jump #while_bOpOis
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_CrZFkw
		ifnot recipes[{i}].output.id|=|bid jump #if_DIIuMS
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_xDUarZ
				set {pname} {i}
				quit
			#if_xDUarZ
		#if_DIIuMS
		setadd i 1
	if i|<|{recipes.Length} jump #while_CrZFkw
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_qpPheE
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_DygBsQ
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_LjJdbp
		call #take|80|1
		call #give|70|1
	#if_LjJdbp
quit
