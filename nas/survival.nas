...
#tick
	...
	if hp|=|prevhp jump #ifnot_uDgSLqmqVZBdIeCB
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_uDgSLqmqVZBdIeCB
	...
	ifnot iframes|>|0 jump #if_EsQlPicmkiBEJqMB
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_EsQlPicmkiBEJqMB
	ifnot fireticks|>|0 jump #if_jSMdwmXMNXxfuVAw
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_pkaZgLENvDXijSrC
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_kiliZvRsJeNqpqaw
...
#mine
	...
	ifnot minetimer|>|0 jump #if_wBPVRIISmHVsLpMS
	...
	#if_wBPVRIISmHVsLpMS
	set minepos
	if toomuch jump #ifnot_thnYcXonKDumcqMw
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_thnYcXonKDumcqMw
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_wvcdjMXCfoGequZe
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_yOTjzpLjuZhgqpOD
		set {runArg1} {runArg2}
		quit
	#if_yOTjzpLjuZhgqpOD
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_oXtWDSuWdRxmuroz
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_oXtWDSuWdRxmuroz
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_rrjpGYvOVbyTuJhX
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_YpUUeqCMqbpqJbHI
		#while_upFKwFPCHzCPzxjE
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_upFKwFPCHzCPzxjE
	#if_YpUUeqCMqbpqJbHI
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_okjalecpkNHvNHdI
		#while_FLVjVEEtqvktTJhA
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_FLVjVEEtqvktTJhA
	#if_okjalecpkNHvNHdI
quit

#input
	ifnot runArg1|=|"craft" jump #if_jfVxxWuMKXNoOMAG
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_ASUnycbDyEuXXtET
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_RUlSPVNbnOpYEpRz
				msg &cInvalid item name or ID
				quit
			#if_RUlSPVNbnOpYEpRz
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_JkZNzyIJkGXAEBtd
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_JkZNzyIJkGXAEBtd
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_ASUnycbDyEuXXtET
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_IFRlzXEsWQiJwnYr
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_IFRlzXEsWQiJwnYr
		set i 0
		#while_xEljuOKzqlLMDsEa
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_CCMSeDwfgkKUnQLr
				...
				#while_LESGoFPqXmmgqidB
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_LESGoFPqXmmgqidB
				msg {ingrediantList}
			#if_CCMSeDwfgkKUnQLr
			setadd i 1
		if i|<|{recipes.Length} jump #while_xEljuOKzqlLMDsEa
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_jfVxxWuMKXNoOMAG
	set i 0
	msg &eResources:
	#while_oECNnqTqhygxGXnI
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_oECNnqTqhygxGXnI
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_xViaIxlKTIHwkgjb
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_xViaIxlKTIHwkgjb
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_bDooIfVYHTMmxYyi
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_bDooIfVYHTMmxYyi
	#while_jWqgUVtJBosJAGWy
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_jWqgUVtJBosJAGWy
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_XkXcOQXSRyPtmAdA
		set {runArg1} {runArg2}
		quit
	#ifnot_XkXcOQXSRyPtmAdA
	set i 0
	#while_EdjjpaitBusGruak
		ifnot blocks[{i}].name|=|runArg2 jump #if_XTJHpZQkopBaCkVD
			set {runArg1} {i}
			quit
		#if_XTJHpZQkopBaCkVD
		setadd i 1
	if i|<|{blocks.Length} jump #while_EdjjpaitBusGruak
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_YcKNfMJHRKJHmfex
		ifnot recipes[{i}].output.id|=|bid jump #if_odXwJjcXfjvLSVVb
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_MRyepdoauTsEHnie
				set {pname} {i}
				quit
			#if_MRyepdoauTsEHnie
		#if_odXwJjcXfjvLSVVb
		setadd i 1
	if i|<|{recipes.Length} jump #while_YcKNfMJHRKJHmfex
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_oomDkdPJnHxSbdHi
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_FQAjgYUnXFrawqNt
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_FQAjgYUnXFrawqNt
	#ifnot_oomDkdPJnHxSbdHi
quit
