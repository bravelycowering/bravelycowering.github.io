...
// start scope
	set l_WJSBKv_PrevPlayerCoords 
	set l_YqUkQm_prevhp 
	set l_qTsyeP_hpbar 
	set l_MnMxcA_myblock 
	#tick
		call #getblock|l_MnMxcA_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_MnMxcA_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_MnMxcA_myblock}].extinguishFire jump #if_unHlHx
			ifnot fireticks|>|0 jump #if_EhQlcL
				gui barSize 0
				set fireticks 0
			#if_EhQlcL
		#if_unHlHx
		ifnot blocks[{l_MnMxcA_myblock}].damage|=|"" call #damage|{blocks[{l_MnMxcA_myblock}].damage}|{blocks[{l_MnMxcA_myblock}].damageType}
		ifnot PlayerCoords|=|l_WJSBKv_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_WJSBKv_PrevPlayerCoords set usingStonecutter false
		set l_WJSBKv_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_YqUkQm_prevhp jump #ifnot_cDoMGe
			set l_YqUkQm_prevhp {hp}
			call #makebar|l_qTsyeP_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_qTsyeP_hpbar}
		#ifnot_cDoMGe
		...
		ifnot iframes|>|0 jump #if_QzPNGd
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_QzPNGd
		ifnot fireticks|>|0 jump #if_QmzfrX
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_aHgEmM
...
#mine
	...
	ifnot minetimer|>|0 jump #if_zkOxhn
...
#give
	ifnot isTool({runArg1}) jump #if_EOLJdK
		set {runArg1} {runArg2}
		quit
	#if_EOLJdK
...
#giveall
	...
	#while_EmNSZw
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_EmNSZw
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_WJAAmh
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_WJAAmh
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_qRhdDR
...
#setblock
	if allowMapChanges jump #ifnot_rGCIFr
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_XFFYqW
		#while_kDoazA
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_kDoazA
	#if_XFFYqW
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_jlaflY
		#while_OjMiCo
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_OjMiCo
	#if_jlaflY
quit

#input
	ifnot runArg1|=|"craft" jump #if_VpPeLp
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_RCXDCo
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_bLFlTZ
				msg &cInvalid item name or ID
				quit
			#if_bLFlTZ
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_xIbDzH
		...
		if usingWorkbench jump #ifnot_APfUEj
		...
		#while_lEqHTL
			...
			set ingrediantList
			ifnot canAfford jump #if_TeviFl
				...
				#while_wOyQSX
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_wOyQSX
				msg {ingrediantList}
			#if_TeviFl
			setadd i 1
		if i|<|{recipes.Length} jump #while_lEqHTL
	...
	#while_ePxVxj
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_ePxVxj
	msg &eTools:
...
#doCraft
	...
	#while_jASMLf
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_jASMLf
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_ZJhjDb
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_ZJhjDb
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_yniiXS
	...
	#while_BfFFxH
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_BfFFxH
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_KmFDzW
		set {runArg1} {runArg2}
		quit
	#ifnot_KmFDzW
	set i 0
	#while_fcGfYB
		ifnot blocks[{i}].name|=|runArg2 jump #if_OYAGnv
			set {runArg1} {i}
			quit
		#if_OYAGnv
		setadd i 1
	if i|<|{blocks.Length} jump #while_fcGfYB
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_cPvgPO
		ifnot recipes[{i}].output.id|=|bid jump #if_FrPuEL
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_HpxOQp
				set {pname} {i}
				quit
			#if_HpxOQp
		#if_FrPuEL
		setadd i 1
	if i|<|{recipes.Length} jump #while_cPvgPO
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_oHwCyC
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_zFLUzL
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_bQYhwF
		call #take|80|1
		call #give|70|1
	#if_bQYhwF
quit
