...
// start scope
	set l_ohkGBR_PrevPlayerCoords 
	set l_YhmEwp_prevhp 
	set l_jhFwTP_hpbar 
	set l_wsOnVV_firebar 
	set l_EPUuLC_myblock 
	set l_nWdvSc_firetickmod 
	set l_PMtEEB_temp 
	#tick
		call #getblock|l_EPUuLC_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_EPUuLC_myblock}].catchFire jump #if_ikuxBv
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_ikuxBv
		ifnot blocks[{l_EPUuLC_myblock}].extinguishFire jump #if_PdNELt
			ifnot fireticks|>|0 jump #if_BuILkH
				gui barSize 0
				set fireticks 0
			#if_BuILkH
		#if_PdNELt
		ifnot blocks[{l_EPUuLC_myblock}].damage|=|"" call #damage|{blocks[{l_EPUuLC_myblock}].damage}|{blocks[{l_EPUuLC_myblock}].damageType}
		ifnot PlayerCoords|=|l_ohkGBR_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_ohkGBR_PrevPlayerCoords set usingStonecutter false
		set l_ohkGBR_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_YhmEwp_prevhp jump #ifnot_MfeDKr
			set l_YhmEwp_prevhp {hp}
			call #makebar|l_jhFwTP_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_jhFwTP_hpbar}
		#ifnot_MfeDKr
		...
		ifnot iframes|>|0 jump #if_yCAWiK
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_yCAWiK
		ifnot fireticks|>|0 jump #if_pMTNui
			setsub fireticks 1
			set l_nWdvSc_firetickmod {fireticks}
			setmod l_nWdvSc_firetickmod 10
			ifnot l_nWdvSc_firetickmod|=|0 jump #if_yvMxqT
				ifnot fireticks|>|0 jump #if_ihnlAf
					set l_PMtEEB_temp {fireticks}
					setdiv l_PMtEEB_temp 10
					call #makecharbar|l_wsOnVV_firebar|▐|6|{l_PMtEEB_temp}|10
					cpemsg smallannounce {l_wsOnVV_firebar}
				#if_ihnlAf
...
#mine
	...
	ifnot minetimer|>|0 jump #if_TnAiFA
...
#give
	ifnot isTool({runArg1}) jump #if_sgPWZX
		set {runArg1} {runArg2}
		quit
	#if_sgPWZX
...
#giveall
	...
	#while_bpmGow
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_bpmGow
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_hfUDMC
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_hfUDMC
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_HmTyRm
...
#itemuse
	...
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_IxZyYT
		ifnot hp|<|maxhp jump #if_fYEMqZ
...
#setblock
	if allowMapChanges jump #ifnot_ULoHej
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_NFanVf
		#while_RWRwFb
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_RWRwFb
	#if_NFanVf
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_vNfKgv
		#while_UTxxHf
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_UTxxHf
	#if_vNfKgv
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_SkJyNV
		#while_lYwmZm
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_lYwmZm
	#if_SkJyNV
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_flgeLB
		#while_bueFix
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_bueFix
	#if_flgeLB
quit

#input
	ifnot runArg1|=|"craft" jump #if_bkzxqY
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_YLXGdX
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_ZLZpub
				msg &cInvalid item name or ID
				quit
			#if_ZLZpub
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_LxeZkp
		...
		if usingWorkbench jump #ifnot_wVjRRy
		...
		#while_opahjM
			...
			set ingrediantList
			ifnot canAfford jump #if_bwCLLq
				...
				#while_FDFceZ
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_FDFceZ
				msg {ingrediantList}
			#if_bwCLLq
			setadd i 1
		if i|<|{recipes.Length} jump #while_opahjM
	...
	#while_CFQSaJ
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_CFQSaJ
	msg &eTools:
...
#doCraft
	...
	#while_XKBrzs
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_XKBrzs
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_xoXraD
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_xoXraD
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_MoFGel
	...
	#while_yFsQvo
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_yFsQvo
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_oUArxS
		set {runArg1} {runArg2}
		quit
	#ifnot_oUArxS
	set i 0
	#while_ODmypg
		ifnot blocks[{i}].name|=|runArg2 jump #if_UpeZCs
			set {runArg1} {i}
			quit
		#if_UpeZCs
		setadd i 1
	if i|<|{blocks.Length} jump #while_ODmypg
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_ANkMWG
		ifnot recipes[{i}].output.id|=|bid jump #if_iJqbxa
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_wSxMRW
				set {pname} {i}
				quit
			#if_wSxMRW
		#if_iJqbxa
		setadd i 1
	if i|<|{recipes.Length} jump #while_ANkMWG
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_NuLIGh
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_qQdVXK
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_uHILGx
		call #take|80|1
		call #give|70|1
	#if_uHILGx
quit
