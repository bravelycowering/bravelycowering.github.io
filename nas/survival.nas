...
#tick
	...
	if hp|=|prevhp jump #ifnot_xVCMRanjyRoAacdw
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_xVCMRanjyRoAacdw
	...
	ifnot iframes|>|0 jump #if_eDNodmcMMXWlLzkt
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_eDNodmcMMXWlLzkt
	ifnot fireticks|>|0 jump #if_mZxiQzSHOPuCvgho
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_ithgWnMsRxLwYUni
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_BCmtHpSpWBRXfLRt
...
#mine
	...
	ifnot minetimer|>|0 jump #if_uoSnvJSdomVpFJOW
	...
	#if_uoSnvJSdomVpFJOW
	set minepos
	if toomuch jump #ifnot_HxtTtivtSaaJNHYk
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_HxtTtivtSaaJNHYk
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_ZWKNpHNNIhNsMDzK
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_PWvpZvlgDyGoLVCi
		set {runArg1} {runArg2}
		quit
	#if_PWvpZvlgDyGoLVCi
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_UqPwgIetyNrnWYcQ
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_UqPwgIetyNrnWYcQ
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_kjUKhlPtQAMHxRLY
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_KhfeZOASqelONVhl
		#while_DkveklrbinAPGxuW
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_DkveklrbinAPGxuW
	#if_KhfeZOASqelONVhl
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_WAXkwNKHZOhVgYRT
		#while_szrskBEuBMFjPjFM
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_szrskBEuBMFjPjFM
	#if_WAXkwNKHZOhVgYRT
quit

#input
	ifnot runArg1|=|"craft" jump #if_WnTwGbozCxumhlyN
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_oRNcYtxXCkOiHQvA
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_sQZwpFDoozXBhnRl
				msg &cInvalid item name or ID
				quit
			#if_sQZwpFDoozXBhnRl
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_GtwqAOPspuvqBtFN
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_GtwqAOPspuvqBtFN
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_oRNcYtxXCkOiHQvA
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_LeluJcKHPIqoaAnh
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_LeluJcKHPIqoaAnh
		set i 0
		#while_gYGZcSeZAjUZARfL
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_xZcsmHCPuJcSUMTQ
				...
				#while_qdYkAWXCwtlKERtA
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_qdYkAWXCwtlKERtA
				msg {ingrediantList}
			#if_xZcsmHCPuJcSUMTQ
			setadd i 1
		if i|<|{recipes.Length} jump #while_gYGZcSeZAjUZARfL
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_WnTwGbozCxumhlyN
	set i 0
	msg &eResources:
	#while_dqXXhxzGXqbgueYa
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_dqXXhxzGXqbgueYa
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_dajyJXSIHBPSHNov
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_dajyJXSIHBPSHNov
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_mQjRbNwTgLlguZym
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_mQjRbNwTgLlguZym
	#while_zPdNfUCYCFrHKtOU
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_zPdNfUCYCFrHKtOU
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_zWhOGXNrOPChZKJp
		set {runArg1} {runArg2}
		quit
	#ifnot_zWhOGXNrOPChZKJp
	set i 0
	#while_nUtDwGzTmHKAXqEY
		ifnot blocks[{i}].name|=|runArg2 jump #if_MbBjIEKjEGanvJvx
			set {runArg1} {i}
			quit
		#if_MbBjIEKjEGanvJvx
		setadd i 1
	if i|<|{blocks.Length} jump #while_nUtDwGzTmHKAXqEY
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_brrpuxUVvsvdJMCI
		ifnot recipes[{i}].output.id|=|bid jump #if_vgtDIDnupRQXLYtT
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_jztgilikhVXyxuom
				set {pname} {i}
				quit
			#if_jztgilikhVXyxuom
		#if_vgtDIDnupRQXLYtT
		setadd i 1
	if i|<|{recipes.Length} jump #while_brrpuxUVvsvdJMCI
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_aLvSekeaezCEwwnJ
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_GsDGaaZCvBvZavbj
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_GsDGaaZCvBvZavbj
	#ifnot_aLvSekeaezCEwwnJ
quit
