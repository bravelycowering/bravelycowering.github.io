...
#tick
	...
	ifnot blocks[{id}].kills jump #if_ILnkUfMswIWbKAlc
		msg your died
		kill
	#if_ILnkUfMswIWbKAlc
...
#mine
	...
	ifnot minetimer|>|0 jump #if_uGYzjJDLQvQPvtZE
	...
	#if_uGYzjJDLQvQPvtZE
	set minepos
	if toomuch jump #ifnot_ImxhigdmNEkymbCl
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_ImxhigdmNEkymbCl
...
#give
	ifnot isTool({runArg1}) jump #if_mjpewcUnpQpcfPXU
		set {runArg1} {runArg2}
		quit
	#if_mjpewcUnpQpcfPXU
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_KKKJgupRtGPughOX
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_KKKJgupRtGPughOX
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_HRheolPKmoSjFpDZ
		...
		#while_FaFMMAcpqgQxQNnO
			...
			ifnot canAfford jump #if_HDuCFiLXQwzrZBZG
				...
				#while_LJhFAiAFsCEubidh
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_LJhFAiAFsCEubidh
				msg {ingrediantList}
			#if_HDuCFiLXQwzrZBZG
			setadd i 1
		if i|<|{recipes.Length} jump #while_FaFMMAcpqgQxQNnO
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_lXQSIyzXLPecijNZ
	set i 0
	msg &eResources:
	#while_MbLtYFfrUgDaYHYg
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_MbLtYFfrUgDaYHYg
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_nSwANOAYdwhNygnD
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_nSwANOAYdwhNygnD
	#while_ZDyAlzydToUvWYvx
		...
		ifnot isTool({id}) jump #if_APGLVCTBJyDtCFFe
			if {id}|>=|count quit
		#if_APGLVCTBJyDtCFFe
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_ZDyAlzydToUvWYvx
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_XnPkeaaSbXCFgvuL
		set {runArg1} {runArg2}
		quit
	#ifnot_XnPkeaaSbXCFgvuL
	set i 0
	#while_KXaxoURgIlmCuaOi
		ifnot blocks[{i}].name|=|runArg2 jump #if_IYiVDQXtphcDEDbu
			set {runArg1} {i}
			quit
		#if_IYiVDQXtphcDEDbu
		setadd i 1
	if i|<|{blocks.Length} jump #while_KXaxoURgIlmCuaOi
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_crhxaUYhcWVEQvfp
		ifnot recipes[{i}].output.id|=|bid jump #if_LnjznFyGjhJjmZjN
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_TsdKRIRdKymHRayU
				set {pname} {i}
				quit
			#if_TsdKRIRdKymHRayU
		#if_LnjznFyGjhJjmZjN
		setadd i 1
	if i|<|{recipes.Length} jump #while_crhxaUYhcWVEQvfp
quit
