...
// start scope
	set l_EvzJtN_PrevPlayerCoords 
	set l_TccDtM_prevhp 
	set l_nKPjjJ_hpbar 
	set l_xYyzxq_firebar 
	set l_NjRlmb_myblock 
	set l_dwXkOb_firetickmod 
	set l_qtgEax_temp 
	#tick
		call #getblock|l_NjRlmb_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_NjRlmb_myblock}].catchFire jump #if_zCDLAT
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_zCDLAT
		ifnot blocks[{l_NjRlmb_myblock}].extinguishFire jump #if_MjEnXf
			ifnot fireticks|>|0 jump #if_efQHym
				gui barSize 0
				set fireticks 0
			#if_efQHym
		#if_MjEnXf
		ifnot blocks[{l_NjRlmb_myblock}].damage|=|"" call #damage|{blocks[{l_NjRlmb_myblock}].damage}|{blocks[{l_NjRlmb_myblock}].damageType}
		ifnot PlayerCoords|=|l_EvzJtN_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_EvzJtN_PrevPlayerCoords set usingStonecutter false
		set l_EvzJtN_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_TccDtM_prevhp jump #ifnot_fWMKHe
			set l_TccDtM_prevhp {hp}
			call #makebar|l_nKPjjJ_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_nKPjjJ_hpbar}
		#ifnot_fWMKHe
		...
		ifnot iframes|>|0 jump #if_gCrrxl
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_gCrrxl
		ifnot fireticks|>|0 jump #if_WjNlkG
			setsub fireticks 1
			set l_dwXkOb_firetickmod {fireticks}
			setmod l_dwXkOb_firetickmod 10
			ifnot l_dwXkOb_firetickmod|=|0 jump #if_xEJBUE
				ifnot fireticks|>|0 jump #if_dbUgvS
					set l_qtgEax_temp {fireticks}
					setdiv l_qtgEax_temp 10
					call #makecharbar|l_xYyzxq_firebar|▐|6|{l_qtgEax_temp}|10
					cpemsg smallannounce {l_xYyzxq_firebar}
				#if_dbUgvS
...
#mine
	...
	ifnot minetimer|>|0 jump #if_Gdfsxq
...
#give
	ifnot isTool({runArg1}) jump #if_DrtCrM
		set {runArg1} {runArg2}
		quit
	#if_DrtCrM
...
#giveall
	...
	#while_vjEkqV
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_vjEkqV
quit

#place
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_EUGdjI
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_LbaVed
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_JOzvVv
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_RwQNjb
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_oZZuqE
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_pBwIOH
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_Jdrsax
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_rFDyZK
		#while_Uvofhm
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_Uvofhm
	#if_rFDyZK
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_hJiBSV
		#while_YLJKKB
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_YLJKKB
	#if_hJiBSV
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_ZriOxf
		#while_xwRwoF
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_xwRwoF
	#if_ZriOxf
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_AEBZOc
		#while_MTczTs
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_MTczTs
	#if_AEBZOc
quit

#input
	ifnot runArg1|=|"craft" jump #if_fNXSar
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_fXPcGf
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_QFikGJ
				msg &cInvalid item name or ID
				quit
			#if_QFikGJ
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_AfZhEU
		...
		if usingWorkbench jump #ifnot_zgbeye
		...
		#while_KbdsOZ
			...
			set ingrediantList
			ifnot canAfford jump #if_yFxVaG
				...
				#while_cCZpam
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_cCZpam
				msg {ingrediantList}
			#if_yFxVaG
			setadd i 1
		if i|<|{recipes.Length} jump #while_KbdsOZ
	...
	#while_wZqzBF
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_wZqzBF
	msg &eTools:
...
#doCraft
	...
	#while_FqvuGm
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_FqvuGm
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_clfrrI
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_clfrrI
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_WBGbak
	...
	#while_CbXHtI
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_CbXHtI
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_FeEdiC
		set {runArg1} {runArg2}
		quit
	#ifnot_FeEdiC
	set i 0
	#while_grehnd
		ifnot blocks[{i}].name|=|runArg2 jump #if_xHnnjp
			set {runArg1} {i}
			quit
		#if_xHnnjp
		setadd i 1
	if i|<|{blocks.Length} jump #while_grehnd
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_LBmrXn
		ifnot recipes[{i}].output.id|=|bid jump #if_SDfQOv
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_npTJvl
				set {pname} {i}
				quit
			#if_npTJvl
		#if_SDfQOv
		setadd i 1
	if i|<|{recipes.Length} jump #while_LBmrXn
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_QdMYlU
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_EZGgjB
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_vRYGko
		call #take|80|1
		call #give|70|1
	#if_vRYGko
quit
