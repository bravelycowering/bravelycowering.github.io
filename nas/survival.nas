...
// start scope
	set l_kjLZwZ_PrevPlayerCoords 
	set l_XTMWem_prevhp 
	set l_DgiSTU_hpbar 
	set l_DTFVsY_firebar 
	set l_WNZxYX_myblock 
	set l_gYlnWf_firetickmod 
	set l_Qbkmdm_temp 
	#tick
		call #getblock|l_WNZxYX_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_WNZxYX_myblock}].catchFire jump #if_tdVHQT
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_tdVHQT
		ifnot blocks[{l_WNZxYX_myblock}].extinguishFire jump #if_eOTZtg
			ifnot fireticks|>|0 jump #if_eWvTaL
				gui barSize 0
				set fireticks 0
			#if_eWvTaL
		#if_eOTZtg
		ifnot blocks[{l_WNZxYX_myblock}].damage|=|"" call #damage|{blocks[{l_WNZxYX_myblock}].damage}|{blocks[{l_WNZxYX_myblock}].damageType}
		ifnot PlayerCoords|=|l_kjLZwZ_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_kjLZwZ_PrevPlayerCoords set usingStonecutter false
		set l_kjLZwZ_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_XTMWem_prevhp jump #ifnot_Cyolbj
			set l_XTMWem_prevhp {hp}
			call #makebar|l_DgiSTU_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_DgiSTU_hpbar}
		#ifnot_Cyolbj
		...
		ifnot iframes|>|0 jump #if_cQRABa
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_cQRABa
		ifnot fireticks|>|0 jump #if_ZQGbft
			setsub fireticks 1
			set l_gYlnWf_firetickmod {fireticks}
			setmod l_gYlnWf_firetickmod 10
			ifnot l_gYlnWf_firetickmod|=|0 jump #if_MFzOAp
				ifnot fireticks|>|0 jump #if_HIxhhZ
					set l_Qbkmdm_temp {fireticks}
					setdiv l_Qbkmdm_temp 10
					call #makecharbar|l_DTFVsY_firebar|▐|6|{l_Qbkmdm_temp}|10
					cpemsg smallannounce {l_DTFVsY_firebar}
				#if_HIxhhZ
...
#mine
	...
	ifnot minetimer|>|0 jump #if_tIvQHM
...
#give
	ifnot isTool({runArg1}) jump #if_xiXJBc
		set {runArg1} {runArg2}
		quit
	#if_xiXJBc
...
#giveall
	...
	#while_jfPumI
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_jfPumI
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_tVKoKq
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_tVKoKq
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_bIyzFw
...
#setblock
	if allowMapChanges jump #ifnot_yRBYAn
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_wAdrpU
		#while_brcOht
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_brcOht
	#if_wAdrpU
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_lwYnBY
		#while_mJKgTi
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_mJKgTi
	#if_lwYnBY
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_QqglRO
		#while_ZuEsyF
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_ZuEsyF
	#if_QqglRO
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_nkReJW
		#while_jTCsbw
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_jTCsbw
	#if_nkReJW
quit

#input
	ifnot runArg1|=|"craft" jump #if_uWtjHk
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_eUQowz
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_QyBsBi
				msg &cInvalid item name or ID
				quit
			#if_QyBsBi
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_IACWzw
		...
		if usingWorkbench jump #ifnot_HbSXGM
		...
		#while_XiOjRN
			...
			set ingrediantList
			ifnot canAfford jump #if_ZTxnIF
				...
				#while_HVoYAU
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_HVoYAU
				msg {ingrediantList}
			#if_ZTxnIF
			setadd i 1
		if i|<|{recipes.Length} jump #while_XiOjRN
	...
	#while_EckoTV
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_EckoTV
	msg &eTools:
...
#doCraft
	...
	#while_ylfciR
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_ylfciR
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_PcjpaY
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_PcjpaY
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_BoooRR
	...
	#while_IsnOUL
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_IsnOUL
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_tbhXGN
		set {runArg1} {runArg2}
		quit
	#ifnot_tbhXGN
	set i 0
	#while_LKpYBi
		ifnot blocks[{i}].name|=|runArg2 jump #if_BCRRnp
			set {runArg1} {i}
			quit
		#if_BCRRnp
		setadd i 1
	if i|<|{blocks.Length} jump #while_LKpYBi
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_nFeetr
		ifnot recipes[{i}].output.id|=|bid jump #if_brtBRG
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_awtHhl
				set {pname} {i}
				quit
			#if_awtHhl
		#if_brtBRG
		setadd i 1
	if i|<|{recipes.Length} jump #while_nFeetr
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_bhgdTR
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_gvsyvb
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_VnDsFg
		call #take|80|1
		call #give|70|1
	#if_VnDsFg
quit
