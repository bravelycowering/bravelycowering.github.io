...
// start scope
	set l_Tmkvim_PrevPlayerCoords 
	set l_iFRoPP_prevhp 
	set l_qxRGYt_hpbar 
	set l_ZtGGHt_myblock 
	#tick
		call #getblock|l_ZtGGHt_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_ZtGGHt_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_ZtGGHt_myblock}].extinguishFire jump #if_TeBiyp
			ifnot fireticks|>|0 jump #if_duBXOj
				gui barSize 0
				set fireticks 0
			#if_duBXOj
		#if_TeBiyp
		ifnot blocks[{l_ZtGGHt_myblock}].damage|=|"" call #damage|{blocks[{l_ZtGGHt_myblock}].damage}|{blocks[{l_ZtGGHt_myblock}].damageType}
		ifnot PlayerCoords|=|l_Tmkvim_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_Tmkvim_PrevPlayerCoords set usingStonecutter false
		set l_Tmkvim_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_iFRoPP_prevhp jump #ifnot_UnLXQy
			set l_iFRoPP_prevhp {hp}
			call #makebar|l_qxRGYt_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_qxRGYt_hpbar}
		#ifnot_UnLXQy
		...
		ifnot iframes|>|0 jump #if_rQotNX
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_rQotNX
		ifnot fireticks|>|0 jump #if_fAVWBc
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_UyBzXl
...
#mine
	...
	ifnot minetimer|>|0 jump #if_YkRKEg
...
#give
	ifnot isTool({runArg1}) jump #if_HAMhOr
		set {runArg1} {runArg2}
		quit
	#if_HAMhOr
...
#giveall
	...
	#while_sBBarX
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_sBBarX
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_TnegLo
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_TnegLo
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_DXJybG
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_GMCbSf
		#while_ZhVvYP
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_ZhVvYP
	#if_GMCbSf
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_sJYEPB
		#while_rIDZRi
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_rIDZRi
	#if_sJYEPB
quit

#input
	ifnot runArg1|=|"craft" jump #if_IiuhNa
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_DQZuhl
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_bZlqTC
				msg &cInvalid item name or ID
				quit
			#if_bZlqTC
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_syXeAN
		...
		if usingWorkbench jump #ifnot_czkzGF
		...
		#while_xvlPgA
			...
			set ingrediantList
			ifnot canAfford jump #if_DUbyGK
				...
				#while_omCtPU
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_omCtPU
				msg {ingrediantList}
			#if_DUbyGK
			setadd i 1
		if i|<|{recipes.Length} jump #while_xvlPgA
	...
	#while_coGEky
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_coGEky
	msg &eTools:
...
#doCraft
	...
	#while_jmERHa
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_jmERHa
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_HbBfMX
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_HbBfMX
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_hUTSZV
	...
	#while_BdmgRj
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_BdmgRj
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_uTbhQs
		set {runArg1} {runArg2}
		quit
	#ifnot_uTbhQs
	set i 0
	#while_WchTrx
		ifnot blocks[{i}].name|=|runArg2 jump #if_LvrELV
			set {runArg1} {i}
			quit
		#if_LvrELV
		setadd i 1
	if i|<|{blocks.Length} jump #while_WchTrx
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_sqoKDX
		ifnot recipes[{i}].output.id|=|bid jump #if_sZCqzv
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_GrtXbk
				set {pname} {i}
				quit
			#if_GrtXbk
		#if_sZCqzv
		setadd i 1
	if i|<|{recipes.Length} jump #while_sqoKDX
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_wYOyuS
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_nvLnSG
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_tSbJlY
		call #take|80|1
		call #give|70|1
	#if_tSbJlY
quit
...
#use[80:70]
	ifnot inventory[70]|>|0 jump #if_kUxfLT
