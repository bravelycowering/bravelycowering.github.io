...
// start scope
	set l_fefKjd_PrevPlayerCoords 
	set l_usBMjc_prevhp 
	set l_RQRgJM_hpbar 
	set l_JOfegx_myblock 
	#tick
		call #getblock|l_JOfegx_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_JOfegx_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_JOfegx_myblock}].extinguishFire jump #if_uPQNmd
			ifnot fireticks|>|0 jump #if_VACknl
				gui barSize 0
				set fireticks 0
			#if_VACknl
		#if_uPQNmd
		ifnot blocks[{l_JOfegx_myblock}].damage|=|"" call #damage|{blocks[{l_JOfegx_myblock}].damage}|{blocks[{l_JOfegx_myblock}].damageType}
		ifnot PlayerCoords|=|l_fefKjd_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_fefKjd_PrevPlayerCoords set usingStonecutter false
		set l_fefKjd_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_usBMjc_prevhp jump #ifnot_HHokMs
			set l_usBMjc_prevhp {hp}
			call #makebar|l_RQRgJM_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_RQRgJM_hpbar}
		#ifnot_HHokMs
		...
		ifnot iframes|>|0 jump #if_nhCAKp
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_nhCAKp
		ifnot fireticks|>|0 jump #if_atjdZN
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_GLqGxA
...
#mine
	...
	ifnot minetimer|>|0 jump #if_XUtiRY
...
#give
	ifnot isTool({runArg1}) jump #if_TeUBzx
		set {runArg1} {runArg2}
		quit
	#if_TeUBzx
...
#giveall
	...
	#while_fyUfQK
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_fyUfQK
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_tSmizr
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_tSmizr
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_VqyhAW
...
#setblock
	if allowMapChanges jump #ifnot_sOkpoz
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_sDjptA
		#while_sAqymS
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_sAqymS
	#if_sDjptA
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_tvojRD
		#while_edLbfo
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_edLbfo
	#if_tvojRD
quit

#input
	ifnot runArg1|=|"craft" jump #if_WbETQd
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_MlIekf
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_OPhQxO
				msg &cInvalid item name or ID
				quit
			#if_OPhQxO
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_ICUYqn
		...
		if usingWorkbench jump #ifnot_BtKCJl
		...
		#while_NwltHY
			...
			set ingrediantList
			ifnot canAfford jump #if_ICpmLp
				...
				#while_qDpzWi
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_qDpzWi
				msg {ingrediantList}
			#if_ICpmLp
			setadd i 1
		if i|<|{recipes.Length} jump #while_NwltHY
	...
	#while_DbCGqk
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_DbCGqk
	msg &eTools:
...
#doCraft
	...
	#while_wUooXK
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_wUooXK
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_AmsJQn
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_AmsJQn
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_IhulFM
	...
	#while_fnZtag
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_fnZtag
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_zKaZYW
		set {runArg1} {runArg2}
		quit
	#ifnot_zKaZYW
	set i 0
	#while_KqKukC
		ifnot blocks[{i}].name|=|runArg2 jump #if_PScdyO
			set {runArg1} {i}
			quit
		#if_PScdyO
		setadd i 1
	if i|<|{blocks.Length} jump #while_KqKukC
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_CsOCJN
		ifnot recipes[{i}].output.id|=|bid jump #if_ERCWay
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_rtcZMj
				set {pname} {i}
				quit
			#if_rtcZMj
		#if_ERCWay
		setadd i 1
	if i|<|{recipes.Length} jump #while_CsOCJN
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_FpHizX
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_AzabMF
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_yNsEIO
		call #take|80|1
		call #give|70|1
	#if_yNsEIO
quit
