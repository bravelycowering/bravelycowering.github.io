...
// start scope
	set l_dYQHjm_PrevPlayerCoords 
	set l_LboYwQ_prevhp 
	set l_iZaWZo_hpbar 
	set l_CXgrpc_myblock 
	#tick
		call #getblock|l_CXgrpc_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_CXgrpc_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_CXgrpc_myblock}].extinguishFire jump #if_LJbknK
			msg EXTINGUISH FIRE GODDAMMIT
			ifnot fireticks|>|0 jump #if_JIGtcm
				gui barSize 0
				set fireticks 0
			#if_JIGtcm
		#if_LJbknK
		ifnot blocks[{l_CXgrpc_myblock}].damage|=|"" call #damage|{blocks[{l_CXgrpc_myblock}].damage}|{blocks[{l_CXgrpc_myblock}].damageType}
		ifnot PlayerCoords|=|l_dYQHjm_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_dYQHjm_PrevPlayerCoords set usingStonecutter false
		set l_dYQHjm_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_LboYwQ_prevhp jump #ifnot_njOjJT
			set l_LboYwQ_prevhp {hp}
			call #makebar|l_iZaWZo_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_iZaWZo_hpbar}
		#ifnot_njOjJT
		...
		ifnot iframes|>|0 jump #if_llgbaz
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_llgbaz
		ifnot fireticks|>|0 jump #if_PzLMnN
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_rPotIb
...
#mine
	...
	ifnot minetimer|>|0 jump #if_wvcwhI
...
#give
	ifnot isTool({runArg1}) jump #if_oJxLZU
		set {runArg1} {runArg2}
		quit
	#if_oJxLZU
...
#giveall
	...
	#while_ZbWVaZ
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_ZbWVaZ
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_oNLiOO
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_oNLiOO
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_xhVDyZ
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_jJjbdp
		#while_OaMlMI
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_OaMlMI
	#if_jJjbdp
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_ecqMvt
		#while_CcWSGK
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_CcWSGK
	#if_ecqMvt
quit

#input
	ifnot runArg1|=|"craft" jump #if_mvyfpj
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_IJVlXp
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_rQAmXp
				msg &cInvalid item name or ID
				quit
			#if_rQAmXp
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_MrGewE
		...
		if usingWorkbench jump #ifnot_pVcxLB
		...
		#while_jWTBIm
			...
			set ingrediantList
			ifnot canAfford jump #if_JdpvuE
				...
				#while_TVaFCr
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_TVaFCr
				msg {ingrediantList}
			#if_JdpvuE
			setadd i 1
		if i|<|{recipes.Length} jump #while_jWTBIm
	...
	#while_GmcegW
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_GmcegW
	msg &eTools:
...
#doCraft
	...
	#while_xJNLdO
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_xJNLdO
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_LajxpE
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_LajxpE
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_IMMPpd
	...
	#while_MlOxfA
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_MlOxfA
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_KDrIru
		set {runArg1} {runArg2}
		quit
	#ifnot_KDrIru
	set i 0
	#while_lqkSLc
		ifnot blocks[{i}].name|=|runArg2 jump #if_VpYNoO
			set {runArg1} {i}
			quit
		#if_VpYNoO
		setadd i 1
	if i|<|{blocks.Length} jump #while_lqkSLc
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_AppRRo
		ifnot recipes[{i}].output.id|=|bid jump #if_lZVdWi
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_RAzxro
				set {pname} {i}
				quit
			#if_RAzxro
		#if_lZVdWi
		setadd i 1
	if i|<|{recipes.Length} jump #while_AppRRo
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_hORNPC
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_cINfcZ
...
#use[68]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_CdQxDm
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_rsmLsP
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
