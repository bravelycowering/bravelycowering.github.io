...
#tick
	...
	if hp|=|prevhp jump #ifnot_GgMRrUQVAqQMaaik
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_GgMRrUQVAqQMaaik
	...
	ifnot iframes|>|0 jump #if_PzNxvJYnzWiFIJgI
		setsub iframes 1
...
#damage
	...
	set iframes 4
	cs me ow
	ifnot hp|<=|0 jump #if_qXRNKWdWPCTnaRDy
...
#mine
	...
	ifnot minetimer|>|0 jump #if_eNYgrDmqdpSyfcPo
	...
	#if_eNYgrDmqdpSyfcPo
	set minepos
	if toomuch jump #ifnot_uXspKUTZaNffQInu
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_uXspKUTZaNffQInu
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_dNBZmzNLudGrxjnA
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_ceFmUUhcDklkHfuO
		set {runArg1} {runArg2}
		quit
	#if_ceFmUUhcDklkHfuO
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_bruNQUpmBOpJLxjP
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_bruNQUpmBOpJLxjP
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_IffzUTNogNFbsRNY
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_iziEeukTorzeWoEJ
		#while_zJJWrIlFLndkiXDP
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_zJJWrIlFLndkiXDP
	#if_iziEeukTorzeWoEJ
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_oryviaeFrGlcsqUH
		#while_lLsqYPcPQZxLXibd
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_lLsqYPcPQZxLXibd
	#if_oryviaeFrGlcsqUH
quit

#input
	ifnot runArg1|=|"craft" jump #if_gcstlevELCLittqs
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_ibDJnBYPjUXpHlSL
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_mEzGZgxvjHJizBRa
				msg &cInvalid item name or ID
				quit
			#if_mEzGZgxvjHJizBRa
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_IAtTUTbhYVheoNOD
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_IAtTUTbhYVheoNOD
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_ibDJnBYPjUXpHlSL
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_lUMjQrtiDHNGbNJR
			ifnot usingStonecutter jump #if_gPkchkKpyPpVvRYGmsg &eStonecutter Recipes:
			else msg &eRecipes:
		#if_gPkchkKpyPpVvRYG
		set i 0
		#while_aqdVnFuirtCKrRbk
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_UXIdgYrLdjYvkLEB
				...
				#while_wvfsIEEAyuRBHelL
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_wvfsIEEAyuRBHelL
				msg {ingrediantList}
			#if_UXIdgYrLdjYvkLEB
			setadd i 1
		if i|<|{recipes.Length} jump #while_aqdVnFuirtCKrRbk
		msg &eType &a/in craft [name]&e to craft something
		quit
	#ifnot_lUMjQrtiDHNGbNJR
	set i 0
	msg &eResources:
	#while_nikCupYikSfEavmr
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_nikCupYikSfEavmr
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_iMscQbsmybwacdXP
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_iMscQbsmybwacdXP
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_HkwPWRZTZlvsonEX
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_HkwPWRZTZlvsonEX
	#while_UKXzwoFynhpizAMr
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_UKXzwoFynhpizAMr
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_pLyIUxqYeyRKHiUK
		set {runArg1} {runArg2}
		quit
	#ifnot_pLyIUxqYeyRKHiUK
	set i 0
	#while_HlyxWriSOwvIJbdJ
		ifnot blocks[{i}].name|=|runArg2 jump #if_CFAbIFkxREBkGvdJ
			set {runArg1} {i}
			quit
		#if_CFAbIFkxREBkGvdJ
		setadd i 1
	if i|<|{blocks.Length} jump #while_HlyxWriSOwvIJbdJ
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_RgdaJWDHDMmXlZuL
		ifnot recipes[{i}].output.id|=|bid jump #if_PHVNFZSNREKjeeyr
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_gTaTLFOgbdUvUiXM
				set {pname} {i}
				quit
			#if_gTaTLFOgbdUvUiXM
		#if_PHVNFZSNREKjeeyr
		setadd i 1
	if i|<|{recipes.Length} jump #while_RgdaJWDHDMmXlZuL
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_SDNdYRsmnFKbSPnI
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_tmTSLHKRFrTPHfau
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_tmTSLHKRFrTPHfau
	#ifnot_SDNdYRsmnFKbSPnI
quit
