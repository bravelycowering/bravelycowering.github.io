...
#tick
	...
	if hp|=|prevhp jump #ifnot_IkfPRFwaXmVkfhFd
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_IkfPRFwaXmVkfhFd
	...
	ifnot iframes|>|0 jump #if_eHUbTbQQCpEfPyad
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_eHUbTbQQCpEfPyad
	ifnot fireticks|>|0 jump #if_bpetMJrPVzRCIdWv
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_ezEockRhagscaaHs
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_mvnatUvQVSxyesfU
...
#mine
	...
	ifnot minetimer|>|0 jump #if_dUVkBJuVfiBuBNOD
	...
	#if_dUVkBJuVfiBuBNOD
	set minepos
	if toomuch jump #ifnot_EmSZnqsJNPxZBleX
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_EmSZnqsJNPxZBleX
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_OwMSCDlFaqJyzPtX
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_hFwJBPXDUQXzrSoW
		set {runArg1} {runArg2}
		quit
	#if_hFwJBPXDUQXzrSoW
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_dWHKBPIASTGkNjWb
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_dWHKBPIASTGkNjWb
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_RfiwXJWWcIqNTgGg
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_YBnimejOUWEYnuYE
		#while_hIyjTorggTSRHlvF
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_hIyjTorggTSRHlvF
	#if_YBnimejOUWEYnuYE
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_fHTWtFtiLhloOWgi
		#while_ygBijwPGvNYqvXwu
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_ygBijwPGvNYqvXwu
	#if_fHTWtFtiLhloOWgi
quit

#input
	ifnot runArg1|=|"craft" jump #if_UIqLJerFrwPtobBJ
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_zICnknoaCsYsHrev
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_eEYeLMRQGvBqQZIJ
				msg &cInvalid item name or ID
				quit
			#if_eEYeLMRQGvBqQZIJ
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_bKicYEcDDxdvbJfe
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_bKicYEcDDxdvbJfe
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_zICnknoaCsYsHrev
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_dHiDMCcQPCplSjaS
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_dHiDMCcQPCplSjaS
		set i 0
		#while_UkGTGPjNRBRenQVS
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_qQjRwKqrnXjBqtnr
				...
				#while_PSBNPnnfalZnRiqa
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_PSBNPnnfalZnRiqa
				msg {ingrediantList}
			#if_qQjRwKqrnXjBqtnr
			setadd i 1
		if i|<|{recipes.Length} jump #while_UkGTGPjNRBRenQVS
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_UIqLJerFrwPtobBJ
	set i 0
	msg &eResources:
	#while_VtfXJVLWoVlTlMBt
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_VtfXJVLWoVlTlMBt
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_yLBjxVfUurVQiyCj
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_yLBjxVfUurVQiyCj
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_SKzgQpchIFauAWUY
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_SKzgQpchIFauAWUY
	#while_UXqmKRgKJXTPXYvm
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_UXqmKRgKJXTPXYvm
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_kjoNkWHDEqULFTJR
		set {runArg1} {runArg2}
		quit
	#ifnot_kjoNkWHDEqULFTJR
	set i 0
	#while_JKTDgeAmpSssXjVV
		ifnot blocks[{i}].name|=|runArg2 jump #if_ieafbYUNNVZXfrYx
			set {runArg1} {i}
			quit
		#if_ieafbYUNNVZXfrYx
		setadd i 1
	if i|<|{blocks.Length} jump #while_JKTDgeAmpSssXjVV
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_diTOtugCrfKfiqFr
		ifnot recipes[{i}].output.id|=|bid jump #if_UIVhsKEPeMJdBNYT
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_zcdZxKoUIONXjmif
				set {pname} {i}
				quit
			#if_zcdZxKoUIONXjmif
		#if_UIVhsKEPeMJdBNYT
		setadd i 1
	if i|<|{recipes.Length} jump #while_diTOtugCrfKfiqFr
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_fWJNIMclafIUSTxh
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_mCjjmIwiiUrtuUoT
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_mCjjmIwiiUrtuUoT
	#ifnot_fWJNIMclafIUSTxh
quit
