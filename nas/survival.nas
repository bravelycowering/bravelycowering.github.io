...
set recipes[10].output.id shovel
...
set recipes[11].output.id shovel
...
set recipes[12].output.id shovel
...
set recipes[13].output.id shovel
...
set recipes[14].output.id shovel
...
#tick
	...
	ifnot blocks[{id}].kills jump #if_CPEOHfqACCyZMkUR
		msg your died
		kill
	#if_CPEOHfqACCyZMkUR
...
#mine
	...
	ifnot minetimer|>|0 jump #if_QpFgVoFYQAJmAKEE
	...
	#if_QpFgVoFYQAJmAKEE
	set minepos
	if toomuch jump #ifnot_FhsrxfiZfbPqPZyK
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_FhsrxfiZfbPqPZyK
...
#give
	ifnot isTool({runArg1}) jump #if_WceTcScBaKJnZrYT
		set {runArg1} {runArg2}
		quit
	#if_WceTcScBaKJnZrYT
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_NjDMhSROHqfmixsE
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_NjDMhSROHqfmixsE
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_uauydYWlYFAWyFfv
		...
		#while_RlRnJovBFCLomXkh
			...
			ifnot canAfford jump #if_PjbdTcSMEVhSgunh
				...
				#while_sTLEkofbsSuyqLRr
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_sTLEkofbsSuyqLRr
				msg {ingrediantList}
			#if_PjbdTcSMEVhSgunh
			setadd i 1
		if i|<|{recipes.Length} jump #while_RlRnJovBFCLomXkh
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_OSpxsqKdTxLHSeou
	set i 0
	msg &eResources:
	#while_tbVimGeALKrENrSr
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_tbVimGeALKrENrSr
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_xAxYlOSVaGHooCze
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_xAxYlOSVaGHooCze
	#while_kPXLjgwlytvHgiwE
		...
		ifnot isTool({id}) jump #if_IGiDSIXxXdpHgFUP
			if {id}|>=|count quit
		#if_IGiDSIXxXdpHgFUP
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_kPXLjgwlytvHgiwE
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_dJRsmJgqdxWJCGih
		set {runArg1} {runArg2}
		quit
	#ifnot_dJRsmJgqdxWJCGih
	set i 0
	#while_VvWQjvFqaWHFvEyF
		ifnot blocks[{i}].name|=|runArg2 jump #if_yjPGQGaYDyZBnKof
			set {runArg1} {i}
			quit
		#if_yjPGQGaYDyZBnKof
		setadd i 1
	if i|<|{blocks.Length} jump #while_VvWQjvFqaWHFvEyF
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_ZyumDznIJmrBQAHN
		ifnot recipes[{i}].output.id|=|bid jump #if_XfcMbkIBhhLGPqku
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_ySbcFSeISYGawMOl
				set {pname} {i}
				quit
			#if_ySbcFSeISYGawMOl
		#if_XfcMbkIBhhLGPqku
		setadd i 1
	if i|<|{recipes.Length} jump #while_ZyumDznIJmrBQAHN
quit
