...
// start scope
	set l_eKKYOS_PrevPlayerCoords 
	set l_Akuoeb_prevhp 
	set l_LCEVNE_hpbar 
	set l_McsVrJ_firebar 
	set l_rCIJox_myblock 
	set l_XkZvpz_temp 
	set l_mAFTps_firetickmod 
	#tick
		call #getblock|l_rCIJox_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_rCIJox_myblock}].catchFire set fireticks 101
		ifnot blocks[{l_rCIJox_myblock}].extinguishFire jump #if_RqvvMB
			ifnot fireticks|>|0 jump #if_RMsQut
				gui barSize 0
				set fireticks 0
			#if_RMsQut
		#if_RqvvMB
		ifnot blocks[{l_rCIJox_myblock}].damage|=|"" call #damage|{blocks[{l_rCIJox_myblock}].damage}|{blocks[{l_rCIJox_myblock}].damageType}
		ifnot PlayerCoords|=|l_eKKYOS_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_eKKYOS_PrevPlayerCoords set usingStonecutter false
		set l_eKKYOS_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_Akuoeb_prevhp jump #ifnot_bulEty
			set l_Akuoeb_prevhp {hp}
			call #makebar|l_LCEVNE_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_LCEVNE_hpbar}
		#ifnot_bulEty
		...
		ifnot iframes|>|0 jump #if_eMIxzR
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_eMIxzR
		ifnot fireticks|>|0 jump #if_ZYYErH
			setsub fireticks 1
			set l_mAFTps_firetickmod {fireticks}
			setmod l_mAFTps_firetickmod 10
			ifnot l_mAFTps_firetickmod|=|0 jump #if_Vlwufg
				call #damage|2|burn
				ifnot fireticks|>|0 jump #if_ZPmTNr
					set l_XkZvpz_temp {fireticks}
					setdiv l_XkZvpz_temp 10
					call #makecharbar|l_McsVrJ_firebar|▐|6|{l_XkZvpz_temp}|10
					cpemsg smallannounce {l_McsVrJ_firebar}
				#if_ZPmTNr
...
#mine
	...
	ifnot minetimer|>|0 jump #if_pRDadw
...
#give
	ifnot isTool({runArg1}) jump #if_GpqsuX
		set {runArg1} {runArg2}
		quit
	#if_GpqsuX
...
#giveall
	...
	#while_orLRFB
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_orLRFB
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_tbjNQb
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_tbjNQb
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_neCnzA
...
#setblock
	if allowMapChanges jump #ifnot_fMYEnj
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_ncGgJk
		#while_DyFqRp
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_DyFqRp
	#if_ncGgJk
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_jnauVP
		#while_kkLPQA
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_kkLPQA
	#if_jnauVP
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_imtfzD
		#while_qfnoOd
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_qfnoOd
	#if_imtfzD
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_HxvWtq
		#while_suHERK
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_suHERK
	#if_HxvWtq
quit

#input
	ifnot runArg1|=|"craft" jump #if_pKxbOF
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_FwfjnC
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_aYnMrm
				msg &cInvalid item name or ID
				quit
			#if_aYnMrm
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_quTANB
		...
		if usingWorkbench jump #ifnot_leoREH
		...
		#while_wpIeBg
			...
			set ingrediantList
			ifnot canAfford jump #if_uWAtrF
				...
				#while_VYgwUq
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_VYgwUq
				msg {ingrediantList}
			#if_uWAtrF
			setadd i 1
		if i|<|{recipes.Length} jump #while_wpIeBg
	...
	#while_HeqwiA
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_HeqwiA
	msg &eTools:
...
#doCraft
	...
	#while_rWFliA
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_rWFliA
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_QZadRQ
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_QZadRQ
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_ShejGX
	...
	#while_clRnbI
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_clRnbI
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_QcFKGr
		set {runArg1} {runArg2}
		quit
	#ifnot_QcFKGr
	set i 0
	#while_GrwrnS
		ifnot blocks[{i}].name|=|runArg2 jump #if_IYispR
			set {runArg1} {i}
			quit
		#if_IYispR
		setadd i 1
	if i|<|{blocks.Length} jump #while_GrwrnS
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_hNDvjO
		ifnot recipes[{i}].output.id|=|bid jump #if_mFRkJY
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_vqGOuW
				set {pname} {i}
				quit
			#if_vqGOuW
		#if_mFRkJY
		setadd i 1
	if i|<|{recipes.Length} jump #while_hNDvjO
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_aKBXQh
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_MTxbzY
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_VCrnOu
		call #take|80|1
		call #give|70|1
	#if_VCrnOu
quit
