...
// start scope
	set l_MUYgTojoMxpaQwOn_PrevPlayerCoords 
	set l_uwsaSmWhzTposRBR_prevhp 
	set l_GgpxkMGrOYzEViAi_hpbar 
	set l_MZeqwJtJreIQEAOb_myblock 
	#tick
		call #getblock|l_MZeqwJtJreIQEAOb_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_MZeqwJtJreIQEAOb_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_MZeqwJtJreIQEAOb_myblock}].extinguishFire jump #if_woUaoJQWsNxQDsnd
			ifnot fireticks|>|0 jump #if_VtONReKmLyloEuhM
				gui barSize 0
				set fireticks 0
			#if_VtONReKmLyloEuhM
		#if_woUaoJQWsNxQDsnd
		ifnot blocks[{l_MZeqwJtJreIQEAOb_myblock}].damage|=|"" call #damage|{blocks[{l_MZeqwJtJreIQEAOb_myblock}].damage}|{blocks[{l_MZeqwJtJreIQEAOb_myblock}].damageType}
		ifnot PlayerCoords|=|l_MUYgTojoMxpaQwOn_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_MUYgTojoMxpaQwOn_PrevPlayerCoords set usingStonecutter false
		set l_MUYgTojoMxpaQwOn_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_uwsaSmWhzTposRBR_prevhp jump #ifnot_IQRYJKFSRBvTyIIn
			set l_uwsaSmWhzTposRBR_prevhp {hp}
			call #makebar|l_GgpxkMGrOYzEViAi_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_GgpxkMGrOYzEViAi_hpbar}
		#ifnot_IQRYJKFSRBvTyIIn
		...
		ifnot iframes|>|0 jump #if_acFNOaHPpeDrRYUe
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_acFNOaHPpeDrRYUe
		ifnot fireticks|>|0 jump #if_vFanoRIPmmvnxVGh
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_chhnsyxtlabkDYHM
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_VhxJWHpgUpBbUObl
...
#mine
	...
	ifnot minetimer|>|0 jump #if_CtnfSyzSOWCySSyR
	...
	#if_CtnfSyzSOWCySSyR
	set minepos
	if toomuch jump #ifnot_MFWxsuNEVobbdblf
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_MFWxsuNEVobbdblf
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_vzEEwMWZzxznJDOx
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_DFOFZXKdMEEzLyJA
		set {runArg1} {runArg2}
		quit
	#if_DFOFZXKdMEEzLyJA
...
#giveall
	...
	set spade 8
	set i 0
	#while_MoZcoOQwSpjgEJSV
		set inventory[{i}] 9999
		setadd i 1
	if i|<|{blocks.Length} jump #while_MoZcoOQwSpjgEJSV
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_ohSbZuQzpoHkbbLr
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_ohSbZuQzpoHkbbLr
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_BVNwOYIZcJkhVQUX
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_tTIicmlhIXkvItZC
		#while_VhwhdaIAtMyAPwdo
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_VhwhdaIAtMyAPwdo
	#if_tTIicmlhIXkvItZC
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_GEpBXsHcYqgcMkiC
		#while_tBVqstgddZnfNWta
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_tBVqstgddZnfNWta
	#if_GEpBXsHcYqgcMkiC
quit

#input
	ifnot runArg1|=|"craft" jump #if_BicitCebfjRdZfJD
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_FJdLzMQrVVoDMACK
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_cjBDCgYDhjtdxaBK
				msg &cInvalid item name or ID
				quit
			#if_cjBDCgYDhjtdxaBK
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_EhmDuNsrKPwJwgYc
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_EhmDuNsrKPwJwgYc
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_FJdLzMQrVVoDMACK
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_gVPkBxLYZJZzEnNN
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_gVPkBxLYZJZzEnNN
		set i 0
		#while_kNqMBuNXRvSSbJHx
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_czoIydJnJVQXcZQs
				...
				#while_rZtdmTveWEQIuZAs
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_rZtdmTveWEQIuZAs
				msg {ingrediantList}
			#if_czoIydJnJVQXcZQs
			setadd i 1
		if i|<|{recipes.Length} jump #while_kNqMBuNXRvSSbJHx
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_BicitCebfjRdZfJD
	set i 0
	msg &eResources:
	#while_rQwqyGwtoLxTyULb
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_rQwqyGwtoLxTyULb
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_VMjDVmMQNAfXXGFI
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_VMjDVmMQNAfXXGFI
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_UZRvaANTjGULWtuA
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_UZRvaANTjGULWtuA
	#while_NgqOHpAWmLGbWBhU
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_NgqOHpAWmLGbWBhU
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_PqwEHrhkhilXqsXs
		set {runArg1} {runArg2}
		quit
	#ifnot_PqwEHrhkhilXqsXs
	set i 0
	#while_SzjaEmasVBTLLDOn
		ifnot blocks[{i}].name|=|runArg2 jump #if_KAznKdsCEaOavMSs
			set {runArg1} {i}
			quit
		#if_KAznKdsCEaOavMSs
		setadd i 1
	if i|<|{blocks.Length} jump #while_SzjaEmasVBTLLDOn
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_GsSnTXQCozTnwdrZ
		ifnot recipes[{i}].output.id|=|bid jump #if_sHFDwjdOlCsuEHfJ
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_STjOxoqJxBPuDinB
				set {pname} {i}
				quit
			#if_STjOxoqJxBPuDinB
		#if_sHFDwjdOlCsuEHfJ
		setadd i 1
	if i|<|{recipes.Length} jump #while_GsSnTXQCozTnwdrZ
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_gunOZfiFioXfocpk
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_dqfnrylNpLwtvDGL
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_dqfnrylNpLwtvDGL
	#ifnot_gunOZfiFioXfocpk
...
#use[68]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_tzrYHWHxuHeVYMeY
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_YtXzeBXcBgiMIauD
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
		#if_YtXzeBXcBgiMIauD
	#ifnot_tzrYHWHxuHeVYMeY
	setdeathspawn {PlayerCoords} {PlayerYaw} {PlayerPitch}
	set spawnblock {runArg1} {runArg2} {runArg3}
	msg &fRespawn point set
quit
