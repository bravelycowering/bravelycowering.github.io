...
// start scope
	set l_cgWWdF_PrevPlayerCoords 
	set l_NFGumj_prevhp 
	set l_OfaQGD_hpbar 
	set l_pFWiDq_firebar 
	set l_xTtQKp_myblock 
	set l_ybgzyP_firetickmod 
	set l_DxBqqV_temp 
	#tick
		call #getblock|l_xTtQKp_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_xTtQKp_myblock}].catchFire jump #if_xjrubJ
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_xjrubJ
		ifnot blocks[{l_xTtQKp_myblock}].extinguishFire jump #if_vLQpQb
			ifnot fireticks|>|0 jump #if_zPVuTV
				gui barSize 0
				set fireticks 0
			#if_zPVuTV
		#if_vLQpQb
		ifnot blocks[{l_xTtQKp_myblock}].damage|=|"" call #damage|{blocks[{l_xTtQKp_myblock}].damage}|{blocks[{l_xTtQKp_myblock}].damageType}
		ifnot PlayerCoords|=|l_cgWWdF_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_cgWWdF_PrevPlayerCoords set usingStonecutter false
		set l_cgWWdF_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_NFGumj_prevhp jump #ifnot_MWORhi
			set l_NFGumj_prevhp {hp}
			call #makebar|l_OfaQGD_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_OfaQGD_hpbar}
		#ifnot_MWORhi
		...
		ifnot iframes|>|0 jump #if_RYABtw
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_RYABtw
		ifnot fireticks|>|0 jump #if_bByAmT
			setsub fireticks 1
			set l_ybgzyP_firetickmod {fireticks}
			setmod l_ybgzyP_firetickmod 10
			ifnot l_ybgzyP_firetickmod|=|0 jump #if_zYQDFX
				ifnot fireticks|>|0 jump #if_vJztMI
					set l_DxBqqV_temp {fireticks}
					setdiv l_DxBqqV_temp 10
					call #makecharbar|l_pFWiDq_firebar|▐|6|{l_DxBqqV_temp}|10
					cpemsg smallannounce {l_pFWiDq_firebar}
				#if_vJztMI
...
#mine
	...
	ifnot minetimer|>|0 jump #if_kpndkg
...
#give
	ifnot isTool({runArg1}) jump #if_Yuvqky
		set {runArg1} {runArg2}
		quit
	#if_Yuvqky
...
#giveall
	...
	#while_KwUjzG
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_KwUjzG
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_kZvooS
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_PjWAGF
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_yGvHLe
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_TzXHCq
	...
	call #getblock|id|{x}|{y}|{z}
	if blocks[{id}].mergeInto|=|"" jump #ifnot_RAvToj
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_eWWJei
			call #take|{playerHeldBlock}|1
			jump #setblock|{blocks[{id}].mergeInto}|{x}|{y}|{z}
			quit
		#if_eWWJei
	...
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_GhAWVl
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_IvAzKY
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_yZqsEU
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_QPAFeD
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_uLccgQ
		#while_VVzTNt
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_VVzTNt
	#if_uLccgQ
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_yXZXfI
		#while_TPUQMf
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_TPUQMf
	#if_yXZXfI
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_wfOtSZ
		#while_vpNWvu
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_vpNWvu
	#if_wfOtSZ
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_zHKcIV
		#while_jRSqAW
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_jRSqAW
	#if_zHKcIV
quit

#input
	if runArg1|=|"changes" jump #changelog
	ifnot runArg1|=|"craft" jump #if_jqozWU
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_nFuvqc
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_gzSTPX
				msg &cInvalid item name or ID
				quit
			#if_gzSTPX
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_qiJXTu
		...
		if usingWorkbench jump #ifnot_fogOxF
		...
		#while_ImfiHT
			...
			set ingrediantList
			ifnot canAfford|>|0 jump #if_NrMiAp
				...
				#while_DbXdvH
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_DbXdvH
				msg {ingrediantList}
			#if_NrMiAp
			setadd i 1
		if i|<|{recipes.Length} jump #while_ImfiHT
	...
	#while_GvQlrv
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_GvQlrv
	msg &eTools:
...
#doCraft
	...
	#while_jqFxwN
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_jqFxwN
...
#checkRecipeAfford
	set j 0
	set {runArg2} 999
	if recipes[{runArg1}].condition|=|"" jump #ifnot_pYkmRs
		ifnot {recipes[{runArg1}].condition} set {runArg2} 0
	#ifnot_pYkmRs
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_ZXWOnb
	...
	#while_gBMQxm
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_gBMQxm
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_JibUmQ
		set {runArg1} {runArg2}
		quit
	#ifnot_JibUmQ
	set i 0
	#while_XDRXkj
		ifnot blocks[{i}].name|=|runArg2 jump #if_UzNLqw
			set {runArg1} {i}
			quit
		#if_UzNLqw
		setadd i 1
	if i|<|{blocks.Length} jump #while_XDRXkj
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_OZrSFq
		ifnot recipes[{i}].output.id|=|bid jump #if_hhyYlv
			call #checkRecipeAfford|{i}|canAfford
			ifnot canAfford|>=|c jump #if_FopTYk
				set {pname} {i}
				quit
			#if_FopTYk
		#if_hhyYlv
		setadd i 1
	if i|<|{recipes.Length} jump #while_OZrSFq
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_quOsEM
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_bskCsL
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_nZDiha
		call #take|80|1
		call #give|70|1
	#if_nZDiha
quit
