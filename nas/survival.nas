...
// start scope
	set l_PfSAsG_PrevPlayerCoords 
	set l_HxzOWW_prevhp 
	set l_FOIFxa_hpbar 
	set l_RQAVIi_firebar 
	set l_ZIehji_myblock 
	set l_USAbzM_firetickmod 
	set l_UNckch_temp 
	#tick
		call #getblock|l_ZIehji_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_ZIehji_myblock}].catchFire jump #if_QnhdcN
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_QnhdcN
		ifnot blocks[{l_ZIehji_myblock}].extinguishFire jump #if_rLDPlQ
			ifnot fireticks|>|0 jump #if_zHgWML
				gui barSize 0
				set fireticks 0
			#if_zHgWML
		#if_rLDPlQ
		ifnot blocks[{l_ZIehji_myblock}].damage|=|"" call #damage|{blocks[{l_ZIehji_myblock}].damage}|{blocks[{l_ZIehji_myblock}].damageType}
		ifnot PlayerCoords|=|l_PfSAsG_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_PfSAsG_PrevPlayerCoords set usingStonecutter false
		set l_PfSAsG_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_HxzOWW_prevhp jump #ifnot_NqRzRH
			set l_HxzOWW_prevhp {hp}
			call #makebar|l_FOIFxa_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_FOIFxa_hpbar}
		#ifnot_NqRzRH
		...
		ifnot iframes|>|0 jump #if_sLvesB
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_sLvesB
		ifnot fireticks|>|0 jump #if_WvGkfg
			setsub fireticks 1
			set l_USAbzM_firetickmod {fireticks}
			setmod l_USAbzM_firetickmod 10
			ifnot l_USAbzM_firetickmod|=|0 jump #if_HDGFSY
				ifnot fireticks|>|0 jump #if_XxLgYY
					set l_UNckch_temp {fireticks}
					setdiv l_UNckch_temp 10
					call #makecharbar|l_RQAVIi_firebar|▐|6|{l_UNckch_temp}|10
					cpemsg smallannounce {l_RQAVIi_firebar}
				#if_XxLgYY
...
#mine
	...
	ifnot minetimer|>|0 jump #if_HnSmMP
...
#give
	ifnot isTool({runArg1}) jump #if_lKuUUt
		set {runArg1} {runArg2}
		quit
	#if_lKuUUt
...
#giveall
	...
	#while_KuXsYs
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_KuXsYs
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_dwaElT
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_mJtLaW
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_xzzcdR
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_BABVth
	...
	call #getblock|id|{x}|{y}|{z}
	if blocks[{id}].mergeInto|=|"" jump #ifnot_BthNyF
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_tRUtfD
			call #take|{playerHeldBlock}|1
			jump #setblock|{blocks[{id}].mergeInto}|{x}|{y}|{z}
			quit
		#if_tRUtfD
	...
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_zucBUR
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_RSVZpd
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_mAWnqj
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_CgqeUG
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_VfsIyF
		#while_EJmIzA
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_EJmIzA
	#if_VfsIyF
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_yAxlSJ
		#while_YjSuNi
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_YjSuNi
	#if_yAxlSJ
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_qaoPoV
		#while_JyvMkk
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_JyvMkk
	#if_qaoPoV
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_cZwXDe
		#while_HbijOA
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_HbijOA
	#if_cZwXDe
quit

#input
	if runArg1|=|"changes" jump #changelog
	ifnot runArg1|=|"craft" jump #if_zQxvIW
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_xwoghs
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_ExjAGn
				msg &cInvalid item name or ID
				quit
			#if_ExjAGn
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_UhbQVJ
		...
		if usingWorkbench jump #ifnot_lkDCTz
		...
		#while_zkMXCw
			...
			set ingrediantList
			ifnot canAfford|>|0 jump #if_CDtIlL
				...
				#while_pGanTT
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_pGanTT
				msg {ingrediantList}
			#if_CDtIlL
			setadd i 1
		if i|<|{recipes.Length} jump #while_zkMXCw
	...
	#while_qkSyaF
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_qkSyaF
	msg &eTools:
...
#doCraft
	...
	#while_UrpIvZ
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_UrpIvZ
...
#checkRecipeAfford
	set j 0
	set {runArg2} 999
	if recipes[{runArg1}].condition|=|"" jump #ifnot_xkUNzB
		ifnot {recipes[{runArg1}].condition} set {runArg2} 0
	#ifnot_xkUNzB
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_TeaYFu
	...
	#while_XIoVBb
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_XIoVBb
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_knJZRt
		set {runArg1} {runArg2}
		quit
	#ifnot_knJZRt
	set i 0
	#while_mIpZDG
		ifnot blocks[{i}].name|=|runArg2 jump #if_NndQUt
			set {runArg1} {i}
			quit
		#if_NndQUt
		setadd i 1
	if i|<|{blocks.Length} jump #while_mIpZDG
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_RTmKVK
		ifnot recipes[{i}].output.id|=|bid jump #if_MbyPJu
			call #checkRecipeAfford|{i}|canAfford
			ifnot canAfford|>=|c jump #if_dbIZzo
				set {pname} {i}
				quit
			#if_dbIZzo
		#if_MbyPJu
		setadd i 1
	if i|<|{recipes.Length} jump #while_RTmKVK
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_JQFUzb
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_wQXWuU
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_bOiAKr
		call #take|80|1
		call #give|70|1
	#if_bOiAKr
quit
