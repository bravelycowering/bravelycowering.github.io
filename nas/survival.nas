...
// start scope
	set l_jozbQu_PrevPlayerCoords 
	set l_PtRJho_prevhp 
	set l_tzThVw_hpbar 
	set l_MyIhkJ_myblock 
	#tick
		call #getblock|l_MyIhkJ_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_MyIhkJ_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_MyIhkJ_myblock}].extinguishFire jump #if_uQYugs
			ifnot fireticks|>|0 jump #if_edEVhl
				gui barSize 0
				set fireticks 0
			#if_edEVhl
		#if_uQYugs
		ifnot blocks[{l_MyIhkJ_myblock}].damage|=|"" call #damage|{blocks[{l_MyIhkJ_myblock}].damage}|{blocks[{l_MyIhkJ_myblock}].damageType}
		ifnot PlayerCoords|=|l_jozbQu_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_jozbQu_PrevPlayerCoords set usingStonecutter false
		set l_jozbQu_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_PtRJho_prevhp jump #ifnot_CoycxV
			set l_PtRJho_prevhp {hp}
			call #makebar|l_tzThVw_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_tzThVw_hpbar}
		#ifnot_CoycxV
		...
		ifnot iframes|>|0 jump #if_vttZWI
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_vttZWI
		ifnot fireticks|>|0 jump #if_hXIINy
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_uMqvBn
...
#mine
	...
	ifnot minetimer|>|0 jump #if_OPBJMD
...
#give
	ifnot isTool({runArg1}) jump #if_sOlMld
		set {runArg1} {runArg2}
		quit
	#if_sOlMld
...
#giveall
	...
	#while_MpgQgs
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_MpgQgs
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_UhTxuy
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_UhTxuy
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_lKKFdq
...
#setblock
	if allowMapChanges jump #ifnot_AyGjnP
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_QHXWtm
		#while_qvsNIN
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_qvsNIN
	#if_QHXWtm
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_IcvqXW
		#while_eXpAbp
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_eXpAbp
	#if_IcvqXW
quit

#input
	ifnot runArg1|=|"craft" jump #if_GdPSkS
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_EowcLp
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_werLvR
				msg &cInvalid item name or ID
				quit
			#if_werLvR
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_ibFIrY
		...
		if usingWorkbench jump #ifnot_RvrNVZ
		...
		#while_ljWMJy
			...
			set ingrediantList
			ifnot canAfford jump #if_TweXry
				...
				#while_jhsuOk
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_jhsuOk
				msg {ingrediantList}
			#if_TweXry
			setadd i 1
		if i|<|{recipes.Length} jump #while_ljWMJy
	...
	#while_eAFSKW
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_eAFSKW
	msg &eTools:
...
#doCraft
	...
	#while_iZaUon
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_iZaUon
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_szIdMW
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_szIdMW
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_yTsQgb
	...
	#while_pWXyxt
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_pWXyxt
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_IYtirF
		set {runArg1} {runArg2}
		quit
	#ifnot_IYtirF
	set i 0
	#while_eKRSEv
		ifnot blocks[{i}].name|=|runArg2 jump #if_FWttrG
			set {runArg1} {i}
			quit
		#if_FWttrG
		setadd i 1
	if i|<|{blocks.Length} jump #while_eKRSEv
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_qmaIOT
		ifnot recipes[{i}].output.id|=|bid jump #if_Tnuxav
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_guLbGM
				set {pname} {i}
				quit
			#if_guLbGM
		#if_Tnuxav
		setadd i 1
	if i|<|{recipes.Length} jump #while_qmaIOT
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_dwcWEN
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_FqhrAF
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_DcTCiH
		call #take|80|1
		call #give|70|1
	#if_DcTCiH
quit
