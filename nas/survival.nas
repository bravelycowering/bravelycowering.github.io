...
// start scope
	set l_fcYQqu_PrevPlayerCoords 
	set l_QrqLsL_prevhp 
	set l_OVnqko_hpbar 
	set l_Cbbrcs_myblock 
	#tick
		call #getblock|l_Cbbrcs_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_Cbbrcs_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_Cbbrcs_myblock}].extinguishFire jump #if_uWJDXZ
			ifnot fireticks|>|0 jump #if_MkNlja
				gui barSize 0
				set fireticks 0
			#if_MkNlja
		#if_uWJDXZ
		ifnot blocks[{l_Cbbrcs_myblock}].damage|=|"" call #damage|{blocks[{l_Cbbrcs_myblock}].damage}|{blocks[{l_Cbbrcs_myblock}].damageType}
		ifnot PlayerCoords|=|l_fcYQqu_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_fcYQqu_PrevPlayerCoords set usingStonecutter false
		set l_fcYQqu_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_QrqLsL_prevhp jump #ifnot_DuGldL
			set l_QrqLsL_prevhp {hp}
			call #makebar|l_OVnqko_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_OVnqko_hpbar}
		#ifnot_DuGldL
		...
		ifnot iframes|>|0 jump #if_uWZOhe
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_uWZOhe
		ifnot fireticks|>|0 jump #if_tYdpxV
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_pRAVJW
...
#mine
	...
	ifnot minetimer|>|0 jump #if_HPEBWi
...
#give
	ifnot isTool({runArg1}) jump #if_NmfwWM
		set {runArg1} {runArg2}
		quit
	#if_NmfwWM
...
#giveall
	...
	#while_csvRKz
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_csvRKz
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_jNsSVE
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_jNsSVE
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_jcdGct
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_LIfRpF
		#while_lDYzeW
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_lDYzeW
	#if_LIfRpF
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_ZZLSnU
		#while_qWoPYI
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_qWoPYI
	#if_ZZLSnU
quit

#input
	ifnot runArg1|=|"craft" jump #if_OxgwSN
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_uClMKa
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_epySQF
				msg &cInvalid item name or ID
				quit
			#if_epySQF
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_bvKhZe
		...
		if usingWorkbench jump #ifnot_FDZFNm
		...
		#while_joaOKD
			...
			set ingrediantList
			ifnot canAfford jump #if_qZXXCk
				...
				#while_JGmcSN
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_JGmcSN
				msg {ingrediantList}
			#if_qZXXCk
			setadd i 1
		if i|<|{recipes.Length} jump #while_joaOKD
	...
	#while_mMPxuj
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_mMPxuj
	msg &eTools:
...
#doCraft
	...
	#while_rGpnuz
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_rGpnuz
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_wgUGEx
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_wgUGEx
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_goQsfH
	...
	#while_bSwque
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_bSwque
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_FTRHPM
		set {runArg1} {runArg2}
		quit
	#ifnot_FTRHPM
	set i 0
	#while_GDuEvA
		ifnot blocks[{i}].name|=|runArg2 jump #if_kRyuTb
			set {runArg1} {i}
			quit
		#if_kRyuTb
		setadd i 1
	if i|<|{blocks.Length} jump #while_GDuEvA
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_UTNeTN
		ifnot recipes[{i}].output.id|=|bid jump #if_AUQuyM
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_CLAqss
				set {pname} {i}
				quit
			#if_CLAqss
		#if_AUQuyM
		setadd i 1
	if i|<|{recipes.Length} jump #while_UTNeTN
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_cKrEYr
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_otxBxw
...
#use[68]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_yZEPEV
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_Aosfeg
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
