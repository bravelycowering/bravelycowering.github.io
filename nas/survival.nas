...
// start scope
	set l_HmDlVP_PrevPlayerCoords 
	set l_gGvxUV_prevhp 
	set l_VgYTno_hpbar 
	set l_pnZrov_myblock 
	#tick
		call #getblock|l_pnZrov_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_pnZrov_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_pnZrov_myblock}].extinguishFire jump #if_LcTTPR
			ifnot fireticks|>|0 jump #if_JxpRaN
				gui barSize 0
				set fireticks 0
			#if_JxpRaN
		#if_LcTTPR
		ifnot blocks[{l_pnZrov_myblock}].damage|=|"" call #damage|{blocks[{l_pnZrov_myblock}].damage}|{blocks[{l_pnZrov_myblock}].damageType}
		ifnot PlayerCoords|=|l_HmDlVP_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_HmDlVP_PrevPlayerCoords set usingStonecutter false
		set l_HmDlVP_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_gGvxUV_prevhp jump #ifnot_xgWUse
			set l_gGvxUV_prevhp {hp}
			call #makebar|l_VgYTno_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_VgYTno_hpbar}
		#ifnot_xgWUse
		...
		ifnot iframes|>|0 jump #if_Comlas
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_Comlas
		ifnot fireticks|>|0 jump #if_ozlxdH
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_CzQMrU
...
#mine
	...
	ifnot minetimer|>|0 jump #if_uDYgag
...
#give
	ifnot isTool({runArg1}) jump #if_RqmElg
		set {runArg1} {runArg2}
		quit
	#if_RqmElg
...
#giveall
	...
	#while_wLFGQm
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_wLFGQm
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_yEiTdx
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_yEiTdx
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_itVhKG
...
#setblock
	if allowMapChanges jump #ifnot_TskCWZ
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_ZqrGqS
		#while_dpmlOq
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_dpmlOq
	#if_ZqrGqS
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_fRmuKg
		#while_LCHZiO
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_LCHZiO
	#if_fRmuKg
quit

#input
	ifnot runArg1|=|"craft" jump #if_uXXVYD
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_SWhUbG
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_pfRNFr
				msg &cInvalid item name or ID
				quit
			#if_pfRNFr
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_qVOVXJ
		...
		if usingWorkbench jump #ifnot_GkpUhL
		...
		#while_rlxwJz
			...
			set ingrediantList
			ifnot canAfford jump #if_hEaLEQ
				...
				#while_KCARYu
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_KCARYu
				msg {ingrediantList}
			#if_hEaLEQ
			setadd i 1
		if i|<|{recipes.Length} jump #while_rlxwJz
	...
	#while_gxEgXG
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_gxEgXG
	msg &eTools:
...
#doCraft
	...
	#while_JxMrrt
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_JxMrrt
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_diVwfb
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_diVwfb
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_HTXXlk
	...
	#while_gLtmcS
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_gLtmcS
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_dhJPzd
		set {runArg1} {runArg2}
		quit
	#ifnot_dhJPzd
	set i 0
	#while_FueQvI
		ifnot blocks[{i}].name|=|runArg2 jump #if_kLbtmL
			set {runArg1} {i}
			quit
		#if_kLbtmL
		setadd i 1
	if i|<|{blocks.Length} jump #while_FueQvI
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_aNqQmi
		ifnot recipes[{i}].output.id|=|bid jump #if_dwHtyF
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_EHPXkM
				set {pname} {i}
				quit
			#if_EHPXkM
		#if_dwHtyF
		setadd i 1
	if i|<|{recipes.Length} jump #while_aNqQmi
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_aGkrtC
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_SkaviP
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_TLXLko
		call #take|80|1
		call #give|70|1
	#if_TLXLko
quit
