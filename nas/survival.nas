...
#tick
	...
	if blocks[{id}].setFireTicks|=|"" jump #ifnot_TdZSmelUYdPYeMPz
		ifnot fireticks|>|0 jump #if_zTvGdNbzIoBkhGcR
			if {blocks[{id}].setFireTicks}|=|0 gui barSize 0
		#if_zTvGdNbzIoBkhGcR
		set fireticks {blocks[{id}].setFireTicks}
	#ifnot_TdZSmelUYdPYeMPz
	...
	if hp|=|prevhp jump #ifnot_tBfxbXtKJsuPIJrI
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_tBfxbXtKJsuPIJrI
	...
	ifnot iframes|>|0 jump #if_ElfsCkAHgIzcuKDu
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_ElfsCkAHgIzcuKDu
	ifnot fireticks|>|0 jump #if_FfkgUXPNnzVnYxuX
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_KBkQXumrgEuTIKbW
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_kCydFYHGBKvoxqgd
...
#mine
	...
	ifnot minetimer|>|0 jump #if_msksGQjvJXyDbOFo
	...
	#if_msksGQjvJXyDbOFo
	set minepos
	if toomuch jump #ifnot_cgWpkUlOrJkrfGuH
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_cgWpkUlOrJkrfGuH
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_kjkJWsYUnFMIotxx
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_UnxcblsGwLpKHPbE
		set {runArg1} {runArg2}
		quit
	#if_UnxcblsGwLpKHPbE
...
#giveall
	#while_VwxVnXIUPLgAQNDL
		set inventory[{i}] 9999
	if i|<|{blocks.Length} jump #while_VwxVnXIUPLgAQNDL
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_XnAkmmaKLjIuRDVa
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_XnAkmmaKLjIuRDVa
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_iiiRwAksDdnvfUzN
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_XmhMwiGHFtxYafkz
		#while_TLUyTywQAgDbZRFc
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_TLUyTywQAgDbZRFc
	#if_XmhMwiGHFtxYafkz
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_fZdZHbuKaUmOvRGI
		#while_tKFjZTkhxWDUohOR
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_tKFjZTkhxWDUohOR
	#if_fZdZHbuKaUmOvRGI
quit

#input
	ifnot runArg1|=|"craft" jump #if_deUQbTEGjAqFJzXv
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_GiSHBhBMSTXqPXiq
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_xazvKLrdPCMOEovu
				msg &cInvalid item name or ID
				quit
			#if_xazvKLrdPCMOEovu
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_kyKzzeSMAnuxmnvd
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_kyKzzeSMAnuxmnvd
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_GiSHBhBMSTXqPXiq
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_AheGCxVWuWcMfNxM
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_AheGCxVWuWcMfNxM
		set i 0
		#while_nSuntJZEXrUxxodP
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_LWDqkIWwzSmldKVO
				...
				#while_QRdGipvjtokGhOYB
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_QRdGipvjtokGhOYB
				msg {ingrediantList}
			#if_LWDqkIWwzSmldKVO
			setadd i 1
		if i|<|{recipes.Length} jump #while_nSuntJZEXrUxxodP
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_deUQbTEGjAqFJzXv
	set i 0
	msg &eResources:
	#while_vRpTjoygyHZCEfll
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_vRpTjoygyHZCEfll
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_tPFQTqjbtESuOhOk
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_tPFQTqjbtESuOhOk
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_yJClTFXwJRDtPyYi
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_yJClTFXwJRDtPyYi
	#while_ktCLIJAicobXuYCI
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_ktCLIJAicobXuYCI
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_acnLqaOKIuYfoKGt
		set {runArg1} {runArg2}
		quit
	#ifnot_acnLqaOKIuYfoKGt
	set i 0
	#while_TfxqsZyyKRAEweTI
		ifnot blocks[{i}].name|=|runArg2 jump #if_PCGSOhWPPQnWGtgJ
			set {runArg1} {i}
			quit
		#if_PCGSOhWPPQnWGtgJ
		setadd i 1
	if i|<|{blocks.Length} jump #while_TfxqsZyyKRAEweTI
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_GRWPHchZqWDaMCFn
		ifnot recipes[{i}].output.id|=|bid jump #if_eYxUMorGbsPUzDGf
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_kHgxHqldNddFGKZz
				set {pname} {i}
				quit
			#if_kHgxHqldNddFGKZz
		#if_eYxUMorGbsPUzDGf
		setadd i 1
	if i|<|{recipes.Length} jump #while_GRWPHchZqWDaMCFn
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_SvHAGpOWeULQlmZY
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_nMZsIbcHIkjMGKVx
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_nMZsIbcHIkjMGKVx
	#ifnot_SvHAGpOWeULQlmZY
...
#use[68]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_TMHkFNXvrNbnxmom
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_lRSEAcKeTSPpSaLL
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
		#if_lRSEAcKeTSPpSaLL
	#ifnot_TMHkFNXvrNbnxmom
	setdeathspawn {PlayerCoords} {PlayerYaw} {PlayerPitch}
	set spawnblock {runArg1} {runArg2} {runArg3}
	msg &fRespawn point set
quit
