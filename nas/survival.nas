...
#tick
	...
	if hp|=|prevhp jump #ifnot_cLQfLIPRAJWNQTOt
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_cLQfLIPRAJWNQTOt
	...
	ifnot iframes|>|0 jump #if_RwTGVZTYHRdPMIlu
		setsub iframes 1
...
#damage
	...
	set iframes 4
	cs me ow
	ifnot hp|<=|0 jump #if_MFwzgOirAyJHvGxC
...
#mine
	...
	ifnot minetimer|>|0 jump #if_DdOVXaLbwGKqQFFj
	...
	#if_DdOVXaLbwGKqQFFj
	set minepos
	if toomuch jump #ifnot_hYCBJMRwkDfUdFOW
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_hYCBJMRwkDfUdFOW
...
#give
	ifnot isTool({runArg1}) jump #if_UiVVtAuWOhpWXspD
		set {runArg1} {runArg2}
		quit
	#if_UiVVtAuWOhpWXspD
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_WPgHJtbZhVxIddJh
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_WPgHJtbZhVxIddJh
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_qBMDjFHXzsuBgglc
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_piiGjjROciVMDyDz
		#while_IwynTQGXeRcohxTk
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_IwynTQGXeRcohxTk
	#if_piiGjjROciVMDyDz
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_sQwjKkqwSHKrkKtx
		#while_pnLexltarrtWcEqf
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_pnLexltarrtWcEqf
	#if_sQwjKkqwSHKrkKtx
quit

#input
	ifnot runArg1|=|"craft" jump #if_vcIlHxxjdaHoRYOZ
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_XmjoPcHSUrNHBbvj
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_tmuhRrPCSqiqkNIs
				msg &cInvalid item name or ID
				quit
			#if_tmuhRrPCSqiqkNIs
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_MNykEbPHKIFQrNrf
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_MNykEbPHKIFQrNrf
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_XmjoPcHSUrNHBbvj
		if usingWorkbench msg &eWorkbench Recipes:
		else msg &eRecipes:
		set i 0
		#while_PHcWisihGCnZkhfK
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_KIAqDyHxrAqxFPam
				...
				#while_VjOsOLixpiqssfdW
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_VjOsOLixpiqssfdW
				msg {ingrediantList}
			#if_KIAqDyHxrAqxFPam
			setadd i 1
		if i|<|{recipes.Length} jump #while_PHcWisihGCnZkhfK
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_vcIlHxxjdaHoRYOZ
	set i 0
	msg &eResources:
	#while_MrInZhLbAjrEHGMO
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_MrInZhLbAjrEHGMO
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_cpIEQaaLWpsdfsjN
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_cpIEQaaLWpsdfsjN
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_ztryXxFHdVkaaykd
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_ztryXxFHdVkaaykd
	#while_AIeiDVjgYDnUkjgW
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_AIeiDVjgYDnUkjgW
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_ItIqHtIqCBlkGWQn
		set {runArg1} {runArg2}
		quit
	#ifnot_ItIqHtIqCBlkGWQn
	set i 0
	#while_hdmwonqcBBCEcmLP
		ifnot blocks[{i}].name|=|runArg2 jump #if_FVQsNfaTnKhTHsxL
			set {runArg1} {i}
			quit
		#if_FVQsNfaTnKhTHsxL
		setadd i 1
	if i|<|{blocks.Length} jump #while_hdmwonqcBBCEcmLP
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_BhjwBGGyyYbwLhuw
		ifnot recipes[{i}].output.id|=|bid jump #if_cAWIKzcWfrSVgejg
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_vExkPLwLxmqSyjpn
				set {pname} {i}
				quit
			#if_vExkPLwLxmqSyjpn
		#if_cAWIKzcWfrSVgejg
		setadd i 1
	if i|<|{recipes.Length} jump #while_BhjwBGGyyYbwLhuw
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_wxOTTbYaEJlIxxsa
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_EzxRIeQZMhASdCIG
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_EzxRIeQZMhASdCIG
	#ifnot_wxOTTbYaEJlIxxsa
quit
