...
// start scope
	set l_YYYyhU_PrevPlayerCoords 
	set l_NLFRvG_prevhp 
	set l_GXqGpl_hpbar 
	set l_vdJwlI_firebar 
	set l_sPXHgX_myblock 
	set l_yBnlav_firetickmod 
	set l_etncpS_temp 
	#tick
		call #getblock|l_sPXHgX_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_sPXHgX_myblock}].catchFire jump #if_fiXzOt
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_fiXzOt
		ifnot blocks[{l_sPXHgX_myblock}].extinguishFire jump #if_JQscYI
			ifnot fireticks|>|0 jump #if_YwftuG
				gui barSize 0
				set fireticks 0
			#if_YwftuG
		#if_JQscYI
		ifnot blocks[{l_sPXHgX_myblock}].damage|=|"" call #damage|{blocks[{l_sPXHgX_myblock}].damage}|{blocks[{l_sPXHgX_myblock}].damageType}
		ifnot PlayerCoords|=|l_YYYyhU_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_YYYyhU_PrevPlayerCoords set usingStonecutter false
		set l_YYYyhU_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_NLFRvG_prevhp jump #ifnot_yKkKiY
			set l_NLFRvG_prevhp {hp}
			call #makebar|l_GXqGpl_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_GXqGpl_hpbar}
		#ifnot_yKkKiY
		...
		ifnot iframes|>|0 jump #if_bcgZVA
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_bcgZVA
		ifnot fireticks|>|0 jump #if_vmlciU
			setsub fireticks 1
			set l_yBnlav_firetickmod {fireticks}
			setmod l_yBnlav_firetickmod 10
			ifnot l_yBnlav_firetickmod|=|0 jump #if_nVGKcH
				ifnot fireticks|>|0 jump #if_magMDR
					set l_etncpS_temp {fireticks}
					setdiv l_etncpS_temp 10
					call #makecharbar|l_vdJwlI_firebar|▐|6|{l_etncpS_temp}|10
					cpemsg smallannounce {l_vdJwlI_firebar}
				#if_magMDR
...
#mine
	...
	ifnot minetimer|>|0 jump #if_gZUqRY
...
#give
	ifnot isTool({runArg1}) jump #if_ecIiRM
		set {runArg1} {runArg2}
		quit
	#if_ecIiRM
...
#giveall
	...
	#while_xzAakO
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_xzAakO
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_EGNTHq
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_EGNTHq
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_CCutMz
...
#setblock
	if allowMapChanges jump #ifnot_vxWTen
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_WZuWxk
		#while_KPKAXo
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_KPKAXo
	#if_WZuWxk
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_aqciBB
		#while_ieQPyv
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_ieQPyv
	#if_aqciBB
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_bomzSO
		#while_fsmLyy
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_fsmLyy
	#if_bomzSO
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_gclWSF
		#while_ybsPxG
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_ybsPxG
	#if_gclWSF
quit

#input
	ifnot runArg1|=|"craft" jump #if_UWaVrM
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_egcEXO
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_Wgtwzu
				msg &cInvalid item name or ID
				quit
			#if_Wgtwzu
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_vONwRl
		...
		if usingWorkbench jump #ifnot_FgjRgo
		...
		#while_kQmoHe
			...
			set ingrediantList
			ifnot canAfford jump #if_EtHSkX
				...
				#while_xxOmQA
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_xxOmQA
				msg {ingrediantList}
			#if_EtHSkX
			setadd i 1
		if i|<|{recipes.Length} jump #while_kQmoHe
	...
	#while_QVtSLh
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_QVtSLh
	msg &eTools:
...
#doCraft
	...
	#while_FDLfzC
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_FDLfzC
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_DnMKYy
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_DnMKYy
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_VBaDuK
	...
	#while_EXSeBL
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_EXSeBL
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_scfcvP
		set {runArg1} {runArg2}
		quit
	#ifnot_scfcvP
	set i 0
	#while_WCsoaH
		ifnot blocks[{i}].name|=|runArg2 jump #if_WWJEqU
			set {runArg1} {i}
			quit
		#if_WWJEqU
		setadd i 1
	if i|<|{blocks.Length} jump #while_WCsoaH
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_eoHSzW
		ifnot recipes[{i}].output.id|=|bid jump #if_mOWWOQ
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_iUEDjU
				set {pname} {i}
				quit
			#if_iUEDjU
		#if_mOWWOQ
		setadd i 1
	if i|<|{recipes.Length} jump #while_eoHSzW
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_LnZlYE
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_sQetMx
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_ivyafd
		call #take|80|1
		call #give|70|1
	#if_ivyafd
quit
