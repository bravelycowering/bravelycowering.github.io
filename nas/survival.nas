...
// start scope
	set l_THBoaZ_PrevPlayerCoords 
	set l_KksYwd_prevhp 
	set l_VlCaPY_hpbar 
	set l_mXpRKc_myblock 
	#tick
		call #getblock|l_mXpRKc_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_mXpRKc_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_mXpRKc_myblock}].extinguishFire jump #if_IKPjMV
			ifnot fireticks|>|0 jump #if_prfLLh
				gui barSize 0
				set fireticks 0
			#if_prfLLh
		#if_IKPjMV
		ifnot blocks[{l_mXpRKc_myblock}].damage|=|"" call #damage|{blocks[{l_mXpRKc_myblock}].damage}|{blocks[{l_mXpRKc_myblock}].damageType}
		ifnot PlayerCoords|=|l_THBoaZ_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_THBoaZ_PrevPlayerCoords set usingStonecutter false
		set l_THBoaZ_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_KksYwd_prevhp jump #ifnot_XRlnqi
			set l_KksYwd_prevhp {hp}
			call #makebar|l_VlCaPY_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_VlCaPY_hpbar}
		#ifnot_XRlnqi
		...
		ifnot iframes|>|0 jump #if_YukplZ
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_YukplZ
		ifnot fireticks|>|0 jump #if_EQOOfG
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_tQxtfp
...
#mine
	...
	ifnot minetimer|>|0 jump #if_nhNKqx
...
#give
	ifnot isTool({runArg1}) jump #if_facnHM
		set {runArg1} {runArg2}
		quit
	#if_facnHM
...
#giveall
	...
	#while_AZoZKX
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_AZoZKX
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_pEMQxO
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_pEMQxO
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_WoOYbA
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_eByRhJ
		#while_bjmQhf
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_bjmQhf
	#if_eByRhJ
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_yXXUFv
		#while_udaORl
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_udaORl
	#if_yXXUFv
quit

#input
	ifnot runArg1|=|"craft" jump #if_dqDgIN
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_QqMMjq
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_YSupbL
				msg &cInvalid item name or ID
				quit
			#if_YSupbL
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_QZHniQ
		...
		if usingWorkbench jump #ifnot_LBloWj
		...
		#while_cunwQD
			...
			set ingrediantList
			ifnot canAfford jump #if_lrZTIH
				...
				#while_FVIBup
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_FVIBup
				msg {ingrediantList}
			#if_lrZTIH
			setadd i 1
		if i|<|{recipes.Length} jump #while_cunwQD
	...
	#while_YvSpQl
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_YvSpQl
	msg &eTools:
...
#doCraft
	...
	#while_jaZlPk
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_jaZlPk
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_VdZkWr
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_VdZkWr
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_oQevkr
	...
	#while_CbUvmG
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_CbUvmG
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_aSofmY
		set {runArg1} {runArg2}
		quit
	#ifnot_aSofmY
	set i 0
	#while_DYFuAs
		ifnot blocks[{i}].name|=|runArg2 jump #if_mIjqIE
			set {runArg1} {i}
			quit
		#if_mIjqIE
		setadd i 1
	if i|<|{blocks.Length} jump #while_DYFuAs
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_xrYEGX
		ifnot recipes[{i}].output.id|=|bid jump #if_zTPSNk
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_eNvzns
				set {pname} {i}
				quit
			#if_eNvzns
		#if_zTPSNk
		setadd i 1
	if i|<|{recipes.Length} jump #while_xrYEGX
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_NYEofH
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_iiMbdx
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_GmERlw
		call #take|80|1
		call #give|70|1
	#if_GmERlw
quit
