...
// start scope
	set l_pNUsSX_PrevPlayerCoords 
	set l_gvyTlU_prevhp 
	set l_XJIckX_hpbar 
	set l_LyoToI_myblock 
	#tick
		call #getblock|l_LyoToI_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_LyoToI_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_LyoToI_myblock}].extinguishFire jump #if_NUdsps
			ifnot fireticks|>|0 jump #if_kpnSQG
				gui barSize 0
				set fireticks 0
			#if_kpnSQG
		#if_NUdsps
		ifnot blocks[{l_LyoToI_myblock}].damage|=|"" call #damage|{blocks[{l_LyoToI_myblock}].damage}|{blocks[{l_LyoToI_myblock}].damageType}
		ifnot PlayerCoords|=|l_pNUsSX_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_pNUsSX_PrevPlayerCoords set usingStonecutter false
		set l_pNUsSX_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_gvyTlU_prevhp jump #ifnot_thfgAI
			set l_gvyTlU_prevhp {hp}
			call #makebar|l_XJIckX_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_XJIckX_hpbar}
		#ifnot_thfgAI
		...
		ifnot iframes|>|0 jump #if_xhYJFx
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_xhYJFx
		ifnot fireticks|>|0 jump #if_ZFcPMy
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_GJQMCZ
...
#mine
	...
	ifnot minetimer|>|0 jump #if_tdxFMg
...
#give
	ifnot isTool({runArg1}) jump #if_aGbbfr
		set {runArg1} {runArg2}
		quit
	#if_aGbbfr
...
#giveall
	...
	#while_yMWCUh
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_yMWCUh
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_yYDWWB
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_yYDWWB
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_UxLxbU
...
#setblock
	if allowMapChanges jump #ifnot_AUynFV
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_TKNLnY
		#while_NyzPtZ
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_NyzPtZ
	#if_TKNLnY
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_Kpuplr
		#while_AZXlnt
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_AZXlnt
	#if_Kpuplr
quit

#input
	ifnot runArg1|=|"craft" jump #if_KKOVAv
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_TFnjYc
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_ZhChWU
				msg &cInvalid item name or ID
				quit
			#if_ZhChWU
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_RPgaxa
		...
		if usingWorkbench jump #ifnot_Sscazs
		...
		#while_RuOWzn
			...
			set ingrediantList
			ifnot canAfford jump #if_yRnrOw
				...
				#while_jnaABO
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_jnaABO
				msg {ingrediantList}
			#if_yRnrOw
			setadd i 1
		if i|<|{recipes.Length} jump #while_RuOWzn
	...
	#while_cIbOKs
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_cIbOKs
	msg &eTools:
...
#doCraft
	...
	#while_UKnJXk
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_UKnJXk
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_dOoAgb
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_dOoAgb
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_AHkniV
	...
	#while_JnxGmZ
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_JnxGmZ
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_qwfvwr
		set {runArg1} {runArg2}
		quit
	#ifnot_qwfvwr
	set i 0
	#while_aSXuGd
		ifnot blocks[{i}].name|=|runArg2 jump #if_aNXcaM
			set {runArg1} {i}
			quit
		#if_aNXcaM
		setadd i 1
	if i|<|{blocks.Length} jump #while_aSXuGd
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_WBizQu
		ifnot recipes[{i}].output.id|=|bid jump #if_ilbqVL
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_OSjmvf
				set {pname} {i}
				quit
			#if_OSjmvf
		#if_ilbqVL
		setadd i 1
	if i|<|{recipes.Length} jump #while_WBizQu
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_XVLnuJ
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_xINibH
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_BWXeVo
		call #take|80|1
		call #give|70|1
	#if_BWXeVo
quit
