...
// start scope
	set l_vGTOBw_PrevPlayerCoords 
	set l_Ebvrkr_prevhp 
	set l_ePNqfW_hpbar 
	set l_jmOxpM_myblock 
	#tick
		call #getblock|l_jmOxpM_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_jmOxpM_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_jmOxpM_myblock}].extinguishFire jump #if_buyaKV
			ifnot fireticks|>|0 jump #if_NYWVGk
				gui barSize 0
				set fireticks 0
			#if_NYWVGk
		#if_buyaKV
		ifnot blocks[{l_jmOxpM_myblock}].damage|=|"" call #damage|{blocks[{l_jmOxpM_myblock}].damage}|{blocks[{l_jmOxpM_myblock}].damageType}
		ifnot PlayerCoords|=|l_vGTOBw_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_vGTOBw_PrevPlayerCoords set usingStonecutter false
		set l_vGTOBw_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_Ebvrkr_prevhp jump #ifnot_SjVyEh
			set l_Ebvrkr_prevhp {hp}
			call #makebar|l_ePNqfW_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_ePNqfW_hpbar}
		#ifnot_SjVyEh
		...
		ifnot iframes|>|0 jump #if_PDVDJE
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_PDVDJE
		ifnot fireticks|>|0 jump #if_uwXdAW
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_FsZLLP
...
#mine
	...
	ifnot minetimer|>|0 jump #if_ertiWQ
...
#give
	ifnot isTool({runArg1}) jump #if_yINcsh
		set {runArg1} {runArg2}
		quit
	#if_yINcsh
...
#giveall
	...
	#while_tVKDnp
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_tVKDnp
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_ateCUi
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_ateCUi
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_fmRENS
...
#setblock
	if allowMapChanges jump #ifnot_Kjodiw
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_HSVHYm
		#while_rDXzkh
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_rDXzkh
	#if_HSVHYm
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_JnIKtW
		#while_tKewHH
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_tKewHH
	#if_JnIKtW
quit

#input
	ifnot runArg1|=|"craft" jump #if_aBryMx
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_hOLsWO
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_ZsmBCP
				msg &cInvalid item name or ID
				quit
			#if_ZsmBCP
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_eQmHaq
		...
		if usingWorkbench jump #ifnot_QGsyyy
		...
		#while_UTkFeU
			...
			set ingrediantList
			ifnot canAfford jump #if_amuYkI
				...
				#while_StXCaZ
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_StXCaZ
				msg {ingrediantList}
			#if_amuYkI
			setadd i 1
		if i|<|{recipes.Length} jump #while_UTkFeU
	...
	#while_pXikxb
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_pXikxb
	msg &eTools:
...
#doCraft
	...
	#while_taknyO
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_taknyO
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_uROiYK
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_uROiYK
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_NnhIHe
	...
	#while_hzXEWM
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_hzXEWM
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_eCrZxY
		set {runArg1} {runArg2}
		quit
	#ifnot_eCrZxY
	set i 0
	#while_dOMVrG
		ifnot blocks[{i}].name|=|runArg2 jump #if_vIekyA
			set {runArg1} {i}
			quit
		#if_vIekyA
		setadd i 1
	if i|<|{blocks.Length} jump #while_dOMVrG
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_orDCZa
		ifnot recipes[{i}].output.id|=|bid jump #if_iasNoi
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_wZJFDi
				set {pname} {i}
				quit
			#if_wZJFDi
		#if_iasNoi
		setadd i 1
	if i|<|{recipes.Length} jump #while_orDCZa
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_KVKeuW
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_TPZBFS
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_sptOCX
		call #take|80|1
		call #give|70|1
	#if_sptOCX
quit
