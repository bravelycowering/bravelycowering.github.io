...
// start scope
	set l_VggdtU_PrevPlayerCoords 
	set l_sPmAUK_prevhp 
	set l_ctUzcr_hpbar 
	set l_PyScWN_firebar 
	set l_guMJoI_myblock 
	set l_HsnEcd_firetickmod 
	set l_qDyyFq_temp 
	#tick
		call #getblock|l_guMJoI_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_guMJoI_myblock}].catchFire jump #if_geVLmx
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_geVLmx
		ifnot blocks[{l_guMJoI_myblock}].extinguishFire jump #if_wfaWhd
			ifnot fireticks|>|0 jump #if_DmMgbs
				gui barSize 0
				set fireticks 0
			#if_DmMgbs
		#if_wfaWhd
		ifnot blocks[{l_guMJoI_myblock}].damage|=|"" call #damage|{blocks[{l_guMJoI_myblock}].damage}|{blocks[{l_guMJoI_myblock}].damageType}
		ifnot PlayerCoords|=|l_VggdtU_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_VggdtU_PrevPlayerCoords set usingStonecutter false
		set l_VggdtU_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_sPmAUK_prevhp jump #ifnot_QpFzXo
			set l_sPmAUK_prevhp {hp}
			call #makebar|l_ctUzcr_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_ctUzcr_hpbar}
		#ifnot_QpFzXo
		...
		ifnot iframes|>|0 jump #if_xsfuQf
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_xsfuQf
		ifnot fireticks|>|0 jump #if_hAEdsA
			setsub fireticks 1
			set l_HsnEcd_firetickmod {fireticks}
			setmod l_HsnEcd_firetickmod 10
			ifnot l_HsnEcd_firetickmod|=|0 jump #if_jBmZsi
				ifnot fireticks|>|0 jump #if_ttEGvC
					set l_qDyyFq_temp {fireticks}
					setdiv l_qDyyFq_temp 10
					call #makecharbar|l_PyScWN_firebar|▐|6|{l_qDyyFq_temp}|10
					cpemsg smallannounce {l_PyScWN_firebar}
				#if_ttEGvC
...
#mine
	...
	ifnot minetimer|>|0 jump #if_KDvTcL
...
#give
	ifnot isTool({runArg1}) jump #if_dvarvm
		set {runArg1} {runArg2}
		quit
	#if_dvarvm
...
#giveall
	...
	#while_MifNsd
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_MifNsd
quit

#place
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_KuTYlS
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_eIhFcw
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_TCQvtv
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_zaDnJA
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_YNVckw
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_WnNZZx
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_yLiYzB
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_wXuXKT
		#while_psUmnz
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_psUmnz
	#if_wXuXKT
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_lIhHea
		#while_Fcaept
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_Fcaept
	#if_lIhHea
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_YWCztP
		#while_Hcppsb
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_Hcppsb
	#if_YWCztP
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_XnIatk
		#while_wtCvKS
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_wtCvKS
	#if_XnIatk
quit

#input
	ifnot runArg1|=|"craft" jump #if_epOqEQ
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_uIaIFN
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_ICmzhh
				msg &cInvalid item name or ID
				quit
			#if_ICmzhh
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_mfDxaS
		...
		if usingWorkbench jump #ifnot_DJqybC
		...
		#while_FAwFLF
			...
			set ingrediantList
			ifnot canAfford jump #if_NsiTie
				...
				#while_UIxBgS
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_UIxBgS
				msg {ingrediantList}
			#if_NsiTie
			setadd i 1
		if i|<|{recipes.Length} jump #while_FAwFLF
	...
	#while_cnEqUZ
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_cnEqUZ
	msg &eTools:
...
#doCraft
	...
	#while_vYsVac
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_vYsVac
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_PkLhbr
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_PkLhbr
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_edtqHh
	...
	#while_xCqgfT
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_xCqgfT
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_rOwgSp
		set {runArg1} {runArg2}
		quit
	#ifnot_rOwgSp
	set i 0
	#while_faSUpr
		ifnot blocks[{i}].name|=|runArg2 jump #if_rqquLQ
			set {runArg1} {i}
			quit
		#if_rqquLQ
		setadd i 1
	if i|<|{blocks.Length} jump #while_faSUpr
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_tSpzDD
		ifnot recipes[{i}].output.id|=|bid jump #if_AQYPQC
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_ZgKBoI
				set {pname} {i}
				quit
			#if_ZgKBoI
		#if_AQYPQC
		setadd i 1
	if i|<|{recipes.Length} jump #while_tSpzDD
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_RXmMUu
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_thIPfz
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_zreyze
		call #take|80|1
		call #give|70|1
	#if_zreyze
quit
