...
#tick
	...
	if hp|=|prevhp jump #ifnot_fWVLyhmMXXhAOIgx
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_fWVLyhmMXXhAOIgx
	...
	ifnot iframes|>|0 jump #if_hgASeegkfDVzouZO
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_hgASeegkfDVzouZO
	ifnot fireticks|>|0 jump #if_voRkbzjaPjfdtDKL
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_wqTdNPRQzeIYPRPm
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_YDoMzbJPzQpYegKL
...
#mine
	...
	ifnot minetimer|>|0 jump #if_FqWtDGpgbofsTaDH
	...
	#if_FqWtDGpgbofsTaDH
	set minepos
	if toomuch jump #ifnot_CTzpjlmtRJwLHNKA
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_CTzpjlmtRJwLHNKA
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_ThHojinDZisjSzAh
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_puDvbVUMluxQOupm
		set {runArg1} {runArg2}
		quit
	#if_puDvbVUMluxQOupm
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_wWHSqEREVFLORbXJ
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_wWHSqEREVFLORbXJ
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_ZrdXpNtUwZEyKsJl
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_vhFjGYSvjzWbPavT
		#while_wuZtqKssfCUvYNPg
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_wuZtqKssfCUvYNPg
	#if_vhFjGYSvjzWbPavT
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_VQGRkLfuxTpNFxcN
		#while_fsRmnpQvUyDFyhae
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_fsRmnpQvUyDFyhae
	#if_VQGRkLfuxTpNFxcN
quit

#input
	ifnot runArg1|=|"craft" jump #if_WSvFLaoxRttNJFQa
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_ZRHBDErYDqKfGbeu
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_NTXqKTGOkegGmYCf
				msg &cInvalid item name or ID
				quit
			#if_NTXqKTGOkegGmYCf
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_MKGtzcURAQTzcfPP
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_MKGtzcURAQTzcfPP
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_ZRHBDErYDqKfGbeu
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_bGxAIOGwNSnqIhYx
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_bGxAIOGwNSnqIhYx
		set i 0
		#while_mAIvsLGIaedXDIFl
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_EzfPTnCJBZDiQFmK
				...
				#while_BrIFgJtBpdZcaoIk
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_BrIFgJtBpdZcaoIk
				msg {ingrediantList}
			#if_EzfPTnCJBZDiQFmK
			setadd i 1
		if i|<|{recipes.Length} jump #while_mAIvsLGIaedXDIFl
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_WSvFLaoxRttNJFQa
	set i 0
	msg &eResources:
	#while_rzXVFqSXDpywEgTw
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_rzXVFqSXDpywEgTw
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_qCinwJtqwSuhyXWx
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_qCinwJtqwSuhyXWx
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_frsjrPpZnjyHhyrl
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_frsjrPpZnjyHhyrl
	#while_vXKRcYHfsZeeTmdT
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_vXKRcYHfsZeeTmdT
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_GVDHytgMPeZWWeqE
		set {runArg1} {runArg2}
		quit
	#ifnot_GVDHytgMPeZWWeqE
	set i 0
	#while_CTxNqSGsypoSGmCW
		ifnot blocks[{i}].name|=|runArg2 jump #if_SxbgCTsCCbunCFsZ
			set {runArg1} {i}
			quit
		#if_SxbgCTsCCbunCFsZ
		setadd i 1
	if i|<|{blocks.Length} jump #while_CTxNqSGsypoSGmCW
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_KbOcxYigXZJmGXwG
		ifnot recipes[{i}].output.id|=|bid jump #if_TeERwgwhokwwIyNK
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_ZbPgSjcnbkWDhhMr
				set {pname} {i}
				quit
			#if_ZbPgSjcnbkWDhhMr
		#if_TeERwgwhokwwIyNK
		setadd i 1
	if i|<|{recipes.Length} jump #while_KbOcxYigXZJmGXwG
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_YYhtJJGJbydQQaHe
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_cfFQmHsKaDYRXOYB
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_cfFQmHsKaDYRXOYB
	#ifnot_YYhtJJGJbydQQaHe
quit
