...
// start scope
	set l_ViBDoh_PrevPlayerCoords 
	set l_sorNcI_prevhp 
	set l_XwZyqE_hpbar 
	set l_hiwuHu_myblock 
	#tick
		call #getblock|l_hiwuHu_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_hiwuHu_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_hiwuHu_myblock}].extinguishFire jump #if_ldUsAQ
			ifnot fireticks|>|0 jump #if_LgaYFE
				gui barSize 0
				set fireticks 0
			#if_LgaYFE
		#if_ldUsAQ
		ifnot blocks[{l_hiwuHu_myblock}].damage|=|"" call #damage|{blocks[{l_hiwuHu_myblock}].damage}|{blocks[{l_hiwuHu_myblock}].damageType}
		ifnot PlayerCoords|=|l_ViBDoh_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_ViBDoh_PrevPlayerCoords set usingStonecutter false
		set l_ViBDoh_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_sorNcI_prevhp jump #ifnot_RSqJSo
			set l_sorNcI_prevhp {hp}
			call #makebar|l_XwZyqE_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_XwZyqE_hpbar}
		#ifnot_RSqJSo
		...
		ifnot iframes|>|0 jump #if_zQEfRT
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_zQEfRT
		ifnot fireticks|>|0 jump #if_Rpvanv
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_hvslGV
...
#mine
	...
	ifnot minetimer|>|0 jump #if_wJDlLS
...
#give
	ifnot isTool({runArg1}) jump #if_syZbEE
		set {runArg1} {runArg2}
		quit
	#if_syZbEE
...
#giveall
	...
	#while_ETmxgz
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_ETmxgz
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_HSsjrv
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_HSsjrv
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_ezGvTX
...
#setblock
	if allowMapChanges jump #ifnot_pHLXkV
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_GrjoNK
		#while_irVZZF
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_irVZZF
	#if_GrjoNK
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_XyywRE
		#while_yWmOwN
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_yWmOwN
	#if_XyywRE
quit

#input
	ifnot runArg1|=|"craft" jump #if_BDmqiE
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_LwURbs
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_lDWFSr
				msg &cInvalid item name or ID
				quit
			#if_lDWFSr
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_zyKdBV
		...
		if usingWorkbench jump #ifnot_QTAFZP
		...
		#while_aMixZn
			...
			set ingrediantList
			ifnot canAfford jump #if_oWZyZO
				...
				#while_qUdXqQ
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_qUdXqQ
				msg {ingrediantList}
			#if_oWZyZO
			setadd i 1
		if i|<|{recipes.Length} jump #while_aMixZn
	...
	#while_LXwLax
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_LXwLax
	msg &eTools:
...
#doCraft
	...
	#while_qztOBO
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_qztOBO
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_kjPhkH
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_kjPhkH
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_rJeYhJ
	...
	#while_LgwOYn
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_LgwOYn
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_HTNTtm
		set {runArg1} {runArg2}
		quit
	#ifnot_HTNTtm
	set i 0
	#while_fjjJuZ
		ifnot blocks[{i}].name|=|runArg2 jump #if_jTYesG
			set {runArg1} {i}
			quit
		#if_jTYesG
		setadd i 1
	if i|<|{blocks.Length} jump #while_fjjJuZ
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_bliXhj
		ifnot recipes[{i}].output.id|=|bid jump #if_wgsLvy
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_ggrMtN
				set {pname} {i}
				quit
			#if_ggrMtN
		#if_wgsLvy
		setadd i 1
	if i|<|{recipes.Length} jump #while_bliXhj
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_bQzGrv
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_UXeygB
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_SvsbXm
		call #take|80|1
		call #give|70|1
	#if_SvsbXm
quit
