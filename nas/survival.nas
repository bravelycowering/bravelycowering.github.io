...
// start scope
	set l_axaPHx_PrevPlayerCoords 
	set l_rxfvnG_prevhp 
	set l_totJjT_hpbar 
	set l_zWStXz_myblock 
	#tick
		call #getblock|l_zWStXz_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_zWStXz_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_zWStXz_myblock}].extinguishFire jump #if_AIxlvT
			ifnot fireticks|>|0 jump #if_ycnXsx
				gui barSize 0
				set fireticks 0
			#if_ycnXsx
		#if_AIxlvT
		ifnot blocks[{l_zWStXz_myblock}].damage|=|"" call #damage|{blocks[{l_zWStXz_myblock}].damage}|{blocks[{l_zWStXz_myblock}].damageType}
		ifnot PlayerCoords|=|l_axaPHx_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_axaPHx_PrevPlayerCoords set usingStonecutter false
		set l_axaPHx_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_rxfvnG_prevhp jump #ifnot_zpAkWn
			set l_rxfvnG_prevhp {hp}
			call #makebar|l_totJjT_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_totJjT_hpbar}
		#ifnot_zpAkWn
		...
		ifnot iframes|>|0 jump #if_KjjNzl
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_KjjNzl
		ifnot fireticks|>|0 jump #if_KlmyfV
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_PZirPX
...
#mine
	...
	ifnot minetimer|>|0 jump #if_akDcwL
...
#give
	ifnot isTool({runArg1}) jump #if_LFzqwJ
		set {runArg1} {runArg2}
		quit
	#if_LFzqwJ
...
#giveall
	...
	#while_CsNRVS
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_CsNRVS
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_xeJuxG
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_xeJuxG
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_ixIqNy
...
#setblock
	if allowMapChanges jump #ifnot_AJWTKh
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_jlFzKe
		#while_kkyjSE
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_kkyjSE
	#if_jlFzKe
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_YpbisF
		#while_nuRcNr
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_nuRcNr
	#if_YpbisF
quit

#input
	ifnot runArg1|=|"craft" jump #if_aYvAIV
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_DoVXIJ
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_bWzNmo
				msg &cInvalid item name or ID
				quit
			#if_bWzNmo
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_XAcNWs
		...
		if usingWorkbench jump #ifnot_MsCoqb
		...
		#while_LMOpaN
			...
			set ingrediantList
			ifnot canAfford jump #if_JhSCGS
				...
				#while_GKRyyF
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_GKRyyF
				msg {ingrediantList}
			#if_JhSCGS
			setadd i 1
		if i|<|{recipes.Length} jump #while_LMOpaN
	...
	#while_LOUBbD
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_LOUBbD
	msg &eTools:
...
#doCraft
	...
	#while_hDBkSf
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_hDBkSf
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_djznvm
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_djznvm
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_mHJkXR
	...
	#while_yVFXHr
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_yVFXHr
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_IuuMZY
		set {runArg1} {runArg2}
		quit
	#ifnot_IuuMZY
	set i 0
	#while_xSDjPg
		ifnot blocks[{i}].name|=|runArg2 jump #if_ClpIOP
			set {runArg1} {i}
			quit
		#if_ClpIOP
		setadd i 1
	if i|<|{blocks.Length} jump #while_xSDjPg
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_qQsiaS
		ifnot recipes[{i}].output.id|=|bid jump #if_aSzaJV
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_EBHDxS
				set {pname} {i}
				quit
			#if_EBHDxS
		#if_aSzaJV
		setadd i 1
	if i|<|{recipes.Length} jump #while_qQsiaS
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_QleaYX
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_xJqPuo
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_NVYomL
		call #take|80|1
		call #give|70|1
	#if_NVYomL
quit
