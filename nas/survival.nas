...
#tick
	...
	ifnot blocks[{id}].kills jump #if_ICYgcPBQcPsJVzYH
		msg your died
		kill
	#if_ICYgcPBQcPsJVzYH
...
#mine
	...
	ifnot minetimer|>|0 jump #if_ucUeoFiEcPUvwvMW
	...
	#if_ucUeoFiEcPUvwvMW
	set minepos
	if toomuch jump #ifnot_qKGaIpjtCiTrDrFm
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_qKGaIpjtCiTrDrFm
...
#give
	ifnot isTool({runArg1}) jump #if_bLmFRBnVaIqcXLHA
		set {runArg1} {runArg2}
		quit
	#if_bLmFRBnVaIqcXLHA
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_stNtJlNIcQTavsid
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_stNtJlNIcQTavsid
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_uKqaZrENxEBtRWnl
		...
		#while_jxFHuzfCULIbgDBS
			...
			ifnot canAfford jump #if_QtipnZLUOdNUDtMa
				...
				#while_mBQKtQhtGsKJtcQE
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_mBQKtQhtGsKJtcQE
				msg {ingrediantList}
			#if_QtipnZLUOdNUDtMa
			setadd i 1
		if i|<|{recipes.Length} jump #while_jxFHuzfCULIbgDBS
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_DLNJCAHsrygmQxgr
	set i 0
	msg &eResources:
	#while_hZsnwmtArihykUJd
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_hZsnwmtArihykUJd
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_lvVCEGEleGmaeemJ
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_lvVCEGEleGmaeemJ
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_KxuqqCapIuyVxAtc
		if {recipes[{runArg1}].output.count}|>=|count quit
	#if_KxuqqCapIuyVxAtc
	#while_SpcuJBCunSmkvYnF
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_SpcuJBCunSmkvYnF
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_IvTcmYfmSnIZqgCO
		set {runArg1} {runArg2}
		quit
	#ifnot_IvTcmYfmSnIZqgCO
	set i 0
	#while_gNoyLBkeRDRVaYKM
		ifnot blocks[{i}].name|=|runArg2 jump #if_ioqiRBRdSwBWCKyz
			set {runArg1} {i}
			quit
		#if_ioqiRBRdSwBWCKyz
		setadd i 1
	if i|<|{blocks.Length} jump #while_gNoyLBkeRDRVaYKM
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_POtbhQsDfeVEDfWy
		ifnot recipes[{i}].output.id|=|bid jump #if_coSFKzxMymIxfNFQ
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_sYHjefjGYLUaWXHw
				set {pname} {i}
				quit
			#if_sYHjefjGYLUaWXHw
		#if_coSFKzxMymIxfNFQ
		setadd i 1
	if i|<|{recipes.Length} jump #while_POtbhQsDfeVEDfWy
quit
