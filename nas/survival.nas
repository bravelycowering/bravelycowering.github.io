...
// start scope
	set l_oowJHa_PrevPlayerCoords 
	set l_PzAWsS_prevhp 
	set l_SZbKhp_hpbar 
	set l_KKKysC_myblock 
	#tick
		call #getblock|l_KKKysC_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_KKKysC_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_KKKysC_myblock}].extinguishFire jump #if_fvCGHt
			ifnot fireticks|>|0 jump #if_xZrcUC
				gui barSize 0
				set fireticks 0
			#if_xZrcUC
		#if_fvCGHt
		ifnot blocks[{l_KKKysC_myblock}].damage|=|"" call #damage|{blocks[{l_KKKysC_myblock}].damage}|{blocks[{l_KKKysC_myblock}].damageType}
		ifnot PlayerCoords|=|l_oowJHa_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_oowJHa_PrevPlayerCoords set usingStonecutter false
		set l_oowJHa_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_PzAWsS_prevhp jump #ifnot_pjPseE
			set l_PzAWsS_prevhp {hp}
			call #makebar|l_SZbKhp_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_SZbKhp_hpbar}
		#ifnot_pjPseE
		...
		ifnot iframes|>|0 jump #if_vSmSnp
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_vSmSnp
		ifnot fireticks|>|0 jump #if_nHTjqi
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_hvudqX
...
#mine
	...
	ifnot minetimer|>|0 jump #if_MonEvq
...
#give
	ifnot isTool({runArg1}) jump #if_zESeoi
		set {runArg1} {runArg2}
		quit
	#if_zESeoi
...
#giveall
	...
	#while_JDyfkB
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_JDyfkB
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_qePeEU
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_qePeEU
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_egWpbG
...
#setblock
	if allowMapChanges jump #ifnot_CblJCJ
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_jAgwES
		#while_ELdUeL
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_ELdUeL
	#if_jAgwES
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_NErrLn
		#while_HbQFUq
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_HbQFUq
	#if_NErrLn
quit

#input
	ifnot runArg1|=|"craft" jump #if_EtKthy
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_ULWVZz
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_cLxklO
				msg &cInvalid item name or ID
				quit
			#if_cLxklO
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_tAIeVS
		...
		if usingWorkbench jump #ifnot_eTmMKr
		...
		#while_yysjaH
			...
			set ingrediantList
			ifnot canAfford jump #if_pvyiNY
				...
				#while_TCfEVM
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_TCfEVM
				msg {ingrediantList}
			#if_pvyiNY
			setadd i 1
		if i|<|{recipes.Length} jump #while_yysjaH
	...
	#while_qlDRFd
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_qlDRFd
	msg &eTools:
...
#doCraft
	...
	#while_IRdIRk
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_IRdIRk
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_pEUZuB
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_pEUZuB
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_xObiCI
	...
	#while_QtWrCK
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_QtWrCK
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_IkRzdP
		set {runArg1} {runArg2}
		quit
	#ifnot_IkRzdP
	set i 0
	#while_VxOCTk
		ifnot blocks[{i}].name|=|runArg2 jump #if_hahhTD
			set {runArg1} {i}
			quit
		#if_hahhTD
		setadd i 1
	if i|<|{blocks.Length} jump #while_VxOCTk
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_wedjRX
		ifnot recipes[{i}].output.id|=|bid jump #if_aZQVdJ
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_jdWfNT
				set {pname} {i}
				quit
			#if_jdWfNT
		#if_aZQVdJ
		setadd i 1
	if i|<|{recipes.Length} jump #while_wedjRX
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_aOWJRT
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_iVIqAY
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_jlQXpX
		call #take|80|1
		call #give|70|1
	#if_jlQXpX
quit
