...
// start scope
	set l_dLUqGF_PrevPlayerCoords 
	set l_oVUFwI_prevhp 
	set l_toVgYO_hpbar 
	set l_NzZkbx_myblock 
	#tick
		call #getblock|l_NzZkbx_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_NzZkbx_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_NzZkbx_myblock}].extinguishFire jump #if_YQbtoJ
			ifnot fireticks|>|0 jump #if_OSbrBZ
				gui barSize 0
				set fireticks 0
			#if_OSbrBZ
		#if_YQbtoJ
		ifnot blocks[{l_NzZkbx_myblock}].damage|=|"" call #damage|{blocks[{l_NzZkbx_myblock}].damage}|{blocks[{l_NzZkbx_myblock}].damageType}
		ifnot PlayerCoords|=|l_dLUqGF_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_dLUqGF_PrevPlayerCoords set usingStonecutter false
		set l_dLUqGF_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_oVUFwI_prevhp jump #ifnot_PjZbqt
			set l_oVUFwI_prevhp {hp}
			call #makebar|l_toVgYO_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_toVgYO_hpbar}
		#ifnot_PjZbqt
		...
		ifnot iframes|>|0 jump #if_MRBYSG
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_MRBYSG
		ifnot fireticks|>|0 jump #if_xyqeYU
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_UZZWZL
...
#mine
	...
	ifnot minetimer|>|0 jump #if_hbsIJx
...
#give
	ifnot isTool({runArg1}) jump #if_UJRHLG
		set {runArg1} {runArg2}
		quit
	#if_UJRHLG
...
#giveall
	...
	#while_nceTVG
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_nceTVG
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_qFXxYG
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_qFXxYG
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_plpbYl
...
#setblock
	if allowMapChanges jump #ifnot_trHVIu
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_wnLfDX
		#while_MNeoMh
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_MNeoMh
	#if_wnLfDX
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_fzhVre
		#while_mvmTMd
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_mvmTMd
	#if_fzhVre
quit

#input
	ifnot runArg1|=|"craft" jump #if_iAXdPt
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_SaBpAG
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_isxvNr
				msg &cInvalid item name or ID
				quit
			#if_isxvNr
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_ajHkrK
		...
		if usingWorkbench jump #ifnot_fSaGrv
		...
		#while_XwwcOP
			...
			set ingrediantList
			ifnot canAfford jump #if_KPQtOP
				...
				#while_AsFUsG
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_AsFUsG
				msg {ingrediantList}
			#if_KPQtOP
			setadd i 1
		if i|<|{recipes.Length} jump #while_XwwcOP
	...
	#while_xDwhIV
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_xDwhIV
	msg &eTools:
...
#doCraft
	...
	#while_aVYbHK
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_aVYbHK
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_BBDpSY
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_BBDpSY
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_eppMeC
	...
	#while_ZRHEYD
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_ZRHEYD
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_eULPmD
		set {runArg1} {runArg2}
		quit
	#ifnot_eULPmD
	set i 0
	#while_hltmEL
		ifnot blocks[{i}].name|=|runArg2 jump #if_BCQPiO
			set {runArg1} {i}
			quit
		#if_BCQPiO
		setadd i 1
	if i|<|{blocks.Length} jump #while_hltmEL
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_JvjEzh
		ifnot recipes[{i}].output.id|=|bid jump #if_oQFsvw
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_obWnDF
				set {pname} {i}
				quit
			#if_obWnDF
		#if_oQFsvw
		setadd i 1
	if i|<|{recipes.Length} jump #while_JvjEzh
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_ALUMTw
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_AzVHDw
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_PMGMOf
		call #take|80|1
		call #give|70|1
	#if_PMGMOf
quit
