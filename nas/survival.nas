...
// start scope
	set l_WjqihP_PrevPlayerCoords 
	set l_PJSIBh_prevhp 
	set l_nUfYjZ_hpbar 
	set l_zTXDPz_firebar 
	set l_chcQAV_myblock 
	set l_MoFuaX_firetickmod 
	set l_meFPaZ_temp 
	#tick
		call #getblock|l_chcQAV_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_chcQAV_myblock}].catchFire jump #if_XGmsKu
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_XGmsKu
		ifnot blocks[{l_chcQAV_myblock}].extinguishFire jump #if_gTuACy
			ifnot fireticks|>|0 jump #if_huKPnb
				gui barSize 0
				set fireticks 0
			#if_huKPnb
		#if_gTuACy
		ifnot blocks[{l_chcQAV_myblock}].damage|=|"" call #damage|{blocks[{l_chcQAV_myblock}].damage}|{blocks[{l_chcQAV_myblock}].damageType}
		ifnot PlayerCoords|=|l_WjqihP_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_WjqihP_PrevPlayerCoords set usingStonecutter false
		set l_WjqihP_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_PJSIBh_prevhp jump #ifnot_HMHOWW
			set l_PJSIBh_prevhp {hp}
			call #makebar|l_nUfYjZ_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_nUfYjZ_hpbar}
		#ifnot_HMHOWW
		...
		ifnot iframes|>|0 jump #if_fIRUSA
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_fIRUSA
		ifnot fireticks|>|0 jump #if_snkkRk
			setsub fireticks 1
			set l_MoFuaX_firetickmod {fireticks}
			setmod l_MoFuaX_firetickmod 10
			ifnot l_MoFuaX_firetickmod|=|0 jump #if_jOwAGY
				ifnot fireticks|>|0 jump #if_cZEkEl
					set l_meFPaZ_temp {fireticks}
					setdiv l_meFPaZ_temp 10
					call #makecharbar|l_zTXDPz_firebar|▐|6|{l_meFPaZ_temp}|10
					cpemsg smallannounce {l_zTXDPz_firebar}
				#if_cZEkEl
...
#mine
	...
	ifnot minetimer|>|0 jump #if_CVFxcW
...
#give
	ifnot isTool({runArg1}) jump #if_UafeEL
		set {runArg1} {runArg2}
		quit
	#if_UafeEL
...
#giveall
	...
	#while_rWtWAn
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_rWtWAn
quit

#place
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_foQwbt
	...
	set y {runArg2}
	set z {runArg3}
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_PothCw
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_AuJBdm
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_fqWVnq
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_FxOPTv
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_LLHMVJ
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_gbZJTW
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_VECwfd
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_YpwMaE
		#while_eLeszF
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_eLeszF
	#if_YpwMaE
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_EYdAtV
		#while_LsDlXD
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_LsDlXD
	#if_EYdAtV
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_xhniJf
		#while_HPHtYj
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_HPHtYj
	#if_xhniJf
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_vqnZTA
		#while_JpWMzh
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_JpWMzh
	#if_vqnZTA
quit

#input
	ifnot runArg1|=|"craft" jump #if_HiTufl
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_frdrtn
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_QCXOxF
				msg &cInvalid item name or ID
				quit
			#if_QCXOxF
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_DLbAjA
		...
		if usingWorkbench jump #ifnot_xnKYwi
		...
		#while_hBlfam
			...
			set ingrediantList
			ifnot canAfford jump #if_yVJtCr
				...
				#while_HxnUYV
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_HxnUYV
				msg {ingrediantList}
			#if_yVJtCr
			setadd i 1
		if i|<|{recipes.Length} jump #while_hBlfam
	...
	#while_bLqHdh
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_bLqHdh
	msg &eTools:
...
#doCraft
	...
	#while_ZrGish
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_ZrGish
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_wECynz
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_wECynz
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_CWNaok
	...
	#while_JJqlVE
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_JJqlVE
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_radBZH
		set {runArg1} {runArg2}
		quit
	#ifnot_radBZH
	set i 0
	#while_mpJBtl
		ifnot blocks[{i}].name|=|runArg2 jump #if_ThWYgn
			set {runArg1} {i}
			quit
		#if_ThWYgn
		setadd i 1
	if i|<|{blocks.Length} jump #while_mpJBtl
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_rftzBw
		ifnot recipes[{i}].output.id|=|bid jump #if_mtdFCs
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_OPXSmZ
				set {pname} {i}
				quit
			#if_OPXSmZ
		#if_mtdFCs
		setadd i 1
	if i|<|{recipes.Length} jump #while_rftzBw
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_lbFICJ
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_MAJKED
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_FBtqBI
		call #take|80|1
		call #give|70|1
	#if_FBtqBI
quit
