...
// start scope
	set l_EOgRuO_PrevPlayerCoords 
	set l_DGlHHd_prevhp 
	set l_fuHPNR_hpbar 
	set l_KlvIfo_myblock 
	#tick
		call #getblock|l_KlvIfo_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_KlvIfo_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_KlvIfo_myblock}].extinguishFire jump #if_BdIGFf
			ifnot fireticks|>|0 jump #if_pOslxX
				gui barSize 0
				set fireticks 0
			#if_pOslxX
		#if_BdIGFf
		ifnot blocks[{l_KlvIfo_myblock}].damage|=|"" call #damage|{blocks[{l_KlvIfo_myblock}].damage}|{blocks[{l_KlvIfo_myblock}].damageType}
		ifnot PlayerCoords|=|l_EOgRuO_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_EOgRuO_PrevPlayerCoords set usingStonecutter false
		set l_EOgRuO_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_DGlHHd_prevhp jump #ifnot_cMPVXN
			set l_DGlHHd_prevhp {hp}
			call #makebar|l_fuHPNR_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_fuHPNR_hpbar}
		#ifnot_cMPVXN
		...
		ifnot iframes|>|0 jump #if_GsXDWI
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_GsXDWI
		ifnot fireticks|>|0 jump #if_SMeUKh
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_pUhdoM
...
#mine
	...
	ifnot minetimer|>|0 jump #if_QJEYOI
...
#give
	ifnot isTool({runArg1}) jump #if_HgWxyy
		set {runArg1} {runArg2}
		quit
	#if_HgWxyy
...
#giveall
	...
	#while_IwmgEM
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_IwmgEM
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_AQObBq
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_AQObBq
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_pZKzGR
...
#setblock
	if allowMapChanges jump #ifnot_VIddUZ
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_unJOLe
		#while_axLkms
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_axLkms
	#if_unJOLe
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_AdPvQU
		#while_BkBsEC
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_BkBsEC
	#if_AdPvQU
quit

#input
	ifnot runArg1|=|"craft" jump #if_LubEdd
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_ogWFKH
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_HRTDPZ
				msg &cInvalid item name or ID
				quit
			#if_HRTDPZ
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_WidwtJ
		...
		if usingWorkbench jump #ifnot_VBLEeC
		...
		#while_RCcoTK
			...
			set ingrediantList
			ifnot canAfford jump #if_IGsWAa
				...
				#while_SYiqRL
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_SYiqRL
				msg {ingrediantList}
			#if_IGsWAa
			setadd i 1
		if i|<|{recipes.Length} jump #while_RCcoTK
	...
	#while_qBtefK
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_qBtefK
	msg &eTools:
...
#doCraft
	...
	#while_hVaaZm
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_hVaaZm
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_TKjETe
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_TKjETe
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_QEgZQu
	...
	#while_UwWBLj
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_UwWBLj
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_tdYxVq
		set {runArg1} {runArg2}
		quit
	#ifnot_tdYxVq
	set i 0
	#while_ibkpCE
		ifnot blocks[{i}].name|=|runArg2 jump #if_EqMIiv
			set {runArg1} {i}
			quit
		#if_EqMIiv
		setadd i 1
	if i|<|{blocks.Length} jump #while_ibkpCE
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_ltAong
		ifnot recipes[{i}].output.id|=|bid jump #if_JVIvrZ
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_MouQtu
				set {pname} {i}
				quit
			#if_MouQtu
		#if_JVIvrZ
		setadd i 1
	if i|<|{recipes.Length} jump #while_ltAong
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_tDzfSH
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_RqXFrC
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_SICsLe
		call #take|80|1
		call #give|70|1
	#if_SICsLe
quit
