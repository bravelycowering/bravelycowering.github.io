...
// start scope
	set l_YZiWwu_PrevPlayerCoords 
	set l_ieAmzZ_prevhp 
	set l_IdOkdJ_hpbar 
	set l_sTayMd_myblock 
	#tick
		call #getblock|l_sTayMd_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_sTayMd_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_sTayMd_myblock}].extinguishFire jump #if_dbCxBB
			ifnot fireticks|>|0 jump #if_iWUTuU
				gui barSize 0
				set fireticks 0
			#if_iWUTuU
		#if_dbCxBB
		ifnot blocks[{l_sTayMd_myblock}].damage|=|"" call #damage|{blocks[{l_sTayMd_myblock}].damage}|{blocks[{l_sTayMd_myblock}].damageType}
		ifnot PlayerCoords|=|l_YZiWwu_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_YZiWwu_PrevPlayerCoords set usingStonecutter false
		set l_YZiWwu_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_ieAmzZ_prevhp jump #ifnot_BXFYvv
			set l_ieAmzZ_prevhp {hp}
			call #makebar|l_IdOkdJ_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_IdOkdJ_hpbar}
		#ifnot_BXFYvv
		...
		ifnot iframes|>|0 jump #if_MXDMut
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_MXDMut
		ifnot fireticks|>|0 jump #if_DoTgXV
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_NQgnGS
...
#mine
	...
	ifnot minetimer|>|0 jump #if_QWuACJ
...
#give
	ifnot isTool({runArg1}) jump #if_dSAFQz
		set {runArg1} {runArg2}
		quit
	#if_dSAFQz
...
#giveall
	...
	#while_erTUJF
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_erTUJF
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_FoXdNE
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_FoXdNE
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_bYyHiY
...
#setblock
	if allowMapChanges jump #ifnot_mxayuD
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_DBWKCl
		#while_IThith
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_IThith
	#if_DBWKCl
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_zoNEjj
		#while_TtiOHp
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_TtiOHp
	#if_zoNEjj
quit

#input
	ifnot runArg1|=|"craft" jump #if_rmfybt
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_dSSOvT
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_ZhGRWT
				msg &cInvalid item name or ID
				quit
			#if_ZhGRWT
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_XbpxIN
		...
		if usingWorkbench jump #ifnot_ljhatO
		...
		#while_NfzQRo
			...
			set ingrediantList
			ifnot canAfford jump #if_vFKFes
				...
				#while_jjerCg
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_jjerCg
				msg {ingrediantList}
			#if_vFKFes
			setadd i 1
		if i|<|{recipes.Length} jump #while_NfzQRo
	...
	#while_aOJpwI
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_aOJpwI
	msg &eTools:
...
#doCraft
	...
	#while_DUEuXJ
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_DUEuXJ
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_hOvYRQ
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_hOvYRQ
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_ZbyXqF
	...
	#while_wUoFQZ
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_wUoFQZ
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_wglgdD
		set {runArg1} {runArg2}
		quit
	#ifnot_wglgdD
	set i 0
	#while_VNrrFF
		ifnot blocks[{i}].name|=|runArg2 jump #if_SqDZJQ
			set {runArg1} {i}
			quit
		#if_SqDZJQ
		setadd i 1
	if i|<|{blocks.Length} jump #while_VNrrFF
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_VShZHi
		ifnot recipes[{i}].output.id|=|bid jump #if_TtACIq
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_hFwNeg
				set {pname} {i}
				quit
			#if_hFwNeg
		#if_TtACIq
		setadd i 1
	if i|<|{recipes.Length} jump #while_VShZHi
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_CSpLTO
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_aYUOQG
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_rksUDW
		call #take|80|1
		call #give|70|1
	#if_rksUDW
quit
