...
// start scope
	set l_PUArjH_PrevPlayerCoords 
	set l_YkoMUz_prevhp 
	set l_SjpbVJ_hpbar 
	set l_LpcowS_myblock 
	#tick
		call #getblock|l_LpcowS_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_LpcowS_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_LpcowS_myblock}].extinguishFire jump #if_WfQvXi
			ifnot fireticks|>|0 jump #if_hSGbKb
				gui barSize 0
				set fireticks 0
			#if_hSGbKb
		#if_WfQvXi
		ifnot blocks[{l_LpcowS_myblock}].damage|=|"" call #damage|{blocks[{l_LpcowS_myblock}].damage}|{blocks[{l_LpcowS_myblock}].damageType}
		ifnot PlayerCoords|=|l_PUArjH_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_PUArjH_PrevPlayerCoords set usingStonecutter false
		set l_PUArjH_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_YkoMUz_prevhp jump #ifnot_RNQwxR
			set l_YkoMUz_prevhp {hp}
			call #makebar|l_SjpbVJ_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_SjpbVJ_hpbar}
		#ifnot_RNQwxR
		...
		ifnot iframes|>|0 jump #if_jULAsZ
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_jULAsZ
		ifnot fireticks|>|0 jump #if_KwQnvv
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_PTlbey
...
#mine
	...
	ifnot minetimer|>|0 jump #if_QucbLR
...
#give
	ifnot isTool({runArg1}) jump #if_IyXqZb
		set {runArg1} {runArg2}
		quit
	#if_IyXqZb
...
#giveall
	...
	#while_gdnFzY
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_gdnFzY
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_OsCrSa
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_OsCrSa
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_MUqnns
...
#setblock
	if allowMapChanges jump #ifnot_EIwGux
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_GVCAUv
		#while_igJfqf
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_igJfqf
	#if_GVCAUv
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_VqcWkY
		#while_fjrpQC
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_fjrpQC
	#if_VqcWkY
quit

#input
	ifnot runArg1|=|"craft" jump #if_OGXRvg
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_IGzAgf
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_PtgdPP
				msg &cInvalid item name or ID
				quit
			#if_PtgdPP
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_Crcljh
		...
		if usingWorkbench jump #ifnot_YUSmRa
		...
		#while_OaHjUb
			...
			set ingrediantList
			ifnot canAfford jump #if_HSsoeu
				...
				#while_JRzSnY
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_JRzSnY
				msg {ingrediantList}
			#if_HSsoeu
			setadd i 1
		if i|<|{recipes.Length} jump #while_OaHjUb
	...
	#while_UdSxPc
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_UdSxPc
	msg &eTools:
...
#doCraft
	...
	#while_HdDvQv
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_HdDvQv
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_bDajfO
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_bDajfO
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_SEcOty
	...
	#while_vpHTKt
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_vpHTKt
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_jkkYfo
		set {runArg1} {runArg2}
		quit
	#ifnot_jkkYfo
	set i 0
	#while_CqweYD
		ifnot blocks[{i}].name|=|runArg2 jump #if_NnxNGR
			set {runArg1} {i}
			quit
		#if_NnxNGR
		setadd i 1
	if i|<|{blocks.Length} jump #while_CqweYD
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_otNukx
		ifnot recipes[{i}].output.id|=|bid jump #if_htiHdu
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_MVlDmM
				set {pname} {i}
				quit
			#if_MVlDmM
		#if_htiHdu
		setadd i 1
	if i|<|{recipes.Length} jump #while_otNukx
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_OFGmOH
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_vUkNyV
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_UPrYAF
		call #take|80|1
		call #give|70|1
	#if_UPrYAF
quit
