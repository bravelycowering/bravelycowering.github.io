...
// start scope
	set l_CalANK_PrevPlayerCoords 
	set l_oRqphb_prevhp 
	set l_YuMqfW_hpbar 
	set l_NjPHDJ_firebar 
	set l_mbYohG_myblock 
	set l_JLUKLm_firetickmod 
	set l_tewXeN_temp 
	#tick
		call #getblock|l_mbYohG_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_mbYohG_myblock}].catchFire jump #if_VmfNFp
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_VmfNFp
		ifnot blocks[{l_mbYohG_myblock}].extinguishFire jump #if_YjJftr
			ifnot fireticks|>|0 jump #if_fTWaAU
				gui barSize 0
				set fireticks 0
			#if_fTWaAU
		#if_YjJftr
		ifnot blocks[{l_mbYohG_myblock}].damage|=|"" call #damage|{blocks[{l_mbYohG_myblock}].damage}|{blocks[{l_mbYohG_myblock}].damageType}
		ifnot PlayerCoords|=|l_CalANK_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_CalANK_PrevPlayerCoords set usingStonecutter false
		set l_CalANK_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_oRqphb_prevhp jump #ifnot_AoAMkO
			set l_oRqphb_prevhp {hp}
			call #makebar|l_YuMqfW_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_YuMqfW_hpbar}
		#ifnot_AoAMkO
		...
		ifnot iframes|>|0 jump #if_NaOSVe
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_NaOSVe
		ifnot fireticks|>|0 jump #if_TZmxle
			setsub fireticks 1
			set l_JLUKLm_firetickmod {fireticks}
			setmod l_JLUKLm_firetickmod 10
			ifnot l_JLUKLm_firetickmod|=|0 jump #if_mFkKXF
				ifnot fireticks|>|0 jump #if_puKXhZ
					set l_tewXeN_temp {fireticks}
					setdiv l_tewXeN_temp 10
					call #makecharbar|l_NjPHDJ_firebar|▐|6|{l_tewXeN_temp}|10
					cpemsg smallannounce {l_NjPHDJ_firebar}
				#if_puKXhZ
...
#mine
	...
	ifnot minetimer|>|0 jump #if_TFtmUF
...
#give
	ifnot isTool({runArg1}) jump #if_tlpjMT
		set {runArg1} {runArg2}
		quit
	#if_tlpjMT
...
#giveall
	...
	#while_CFeInq
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_CFeInq
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_JWdIyY
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_JWdIyY
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_MKhlme
...
#itemuse
	...
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_pSJDkm
		ifnot hp|<|maxhp jump #if_AtBskT
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_VGAPSO
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_DuyQGL
		#while_hRNxrK
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_hRNxrK
	#if_DuyQGL
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_kjLMZA
		#while_HMtHSj
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_HMtHSj
	#if_kjLMZA
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_VQPkZp
		#while_xYukaI
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_xYukaI
	#if_VQPkZp
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_NTgDMv
		#while_WHPplf
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_WHPplf
	#if_NTgDMv
quit

#input
	ifnot runArg1|=|"craft" jump #if_OxddeH
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_vLDqdt
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_QNHLFE
				msg &cInvalid item name or ID
				quit
			#if_QNHLFE
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_iQSSOb
		...
		if usingWorkbench jump #ifnot_zSEYQN
		...
		#while_uvOoEH
			...
			set ingrediantList
			ifnot canAfford jump #if_wwQEKQ
				...
				#while_TEJsXd
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_TEJsXd
				msg {ingrediantList}
			#if_wwQEKQ
			setadd i 1
		if i|<|{recipes.Length} jump #while_uvOoEH
	...
	#while_suRPGA
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_suRPGA
	msg &eTools:
...
#doCraft
	...
	#while_pvYlCk
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_pvYlCk
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_rZIIQg
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_rZIIQg
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_zSiEAK
	...
	#while_CKAjxb
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_CKAjxb
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_MjEHLw
		set {runArg1} {runArg2}
		quit
	#ifnot_MjEHLw
	set i 0
	#while_yGolPf
		ifnot blocks[{i}].name|=|runArg2 jump #if_giMSfY
			set {runArg1} {i}
			quit
		#if_giMSfY
		setadd i 1
	if i|<|{blocks.Length} jump #while_yGolPf
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_rDGmvm
		ifnot recipes[{i}].output.id|=|bid jump #if_gNNWfL
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_uiEgDk
				set {pname} {i}
				quit
			#if_uiEgDk
		#if_gNNWfL
		setadd i 1
	if i|<|{recipes.Length} jump #while_rDGmvm
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_SysYRB
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_zsbFZb
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_oFmONo
		call #take|80|1
		call #give|70|1
	#if_oFmONo
quit
