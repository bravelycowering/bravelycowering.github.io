...
#tick
	...
	ifnot blocks[{myblock}].extinguishFire jump #if_IDWOFqtLezFoJqhh
		ifnot fireticks|>|0 jump #if_XodYrzXsHmxbUVwg
			gui barSize 0
			set fireticks 0
		#if_XodYrzXsHmxbUVwg
	#if_IDWOFqtLezFoJqhh
	...
	if hp|=|prevhp jump #ifnot_zzWgotSpTpUVaAbI
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_zzWgotSpTpUVaAbI
	...
	ifnot iframes|>|0 jump #if_OQfQfcdYGfxCZANG
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_OQfQfcdYGfxCZANG
	ifnot fireticks|>|0 jump #if_MNsCepGbEXkgoyGw
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_heOaxDiisiquSqny
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_vYvDUDFVaEdAFvuU
...
#mine
	...
	ifnot minetimer|>|0 jump #if_ZKaNzLvrYSSFjHmC
	...
	#if_ZKaNzLvrYSSFjHmC
	set minepos
	if toomuch jump #ifnot_VaQfLFefPbPdUFJU
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_VaQfLFefPbPdUFJU
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_MmlEdaHlGqXcdbzp
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_XTgRnUKTXyUySCUG
		set {runArg1} {runArg2}
		quit
	#if_XTgRnUKTXyUySCUG
...
#giveall
	...
	set spade 8
	set i 0
	#while_kipKdIlQkhPRHxUP
		set inventory[{i}] 9999
		setadd i 1
	if i|<|{blocks.Length} jump #while_kipKdIlQkhPRHxUP
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_VXTVZjkvjHrGlFAX
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_VXTVZjkvjHrGlFAX
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_QycxHpWrLAYdbBah
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_bgfuGLDlRkRFFgyR
		#while_CZUDeLRxPrAsELmC
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_CZUDeLRxPrAsELmC
	#if_bgfuGLDlRkRFFgyR
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_yCwUVxVNhXhEbJnU
		#while_QNItjxJQEkJLvlfW
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_QNItjxJQEkJLvlfW
	#if_yCwUVxVNhXhEbJnU
quit

#input
	ifnot runArg1|=|"craft" jump #if_KlCXAQLKNioASLeY
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_hPFmgdLtQNLVVQQI
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_RcLEGloeajcHGrkS
				msg &cInvalid item name or ID
				quit
			#if_RcLEGloeajcHGrkS
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_RQgkJCFxLOUJEjMA
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_RQgkJCFxLOUJEjMA
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_hPFmgdLtQNLVVQQI
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_HMmlWIPShAOWbnUM
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_HMmlWIPShAOWbnUM
		set i 0
		#while_zQxRjtBbzVXbBeJf
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_yIZtwIXQdQaIepZw
				...
				#while_NDWdJZskmNjfFMnO
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_NDWdJZskmNjfFMnO
				msg {ingrediantList}
			#if_yIZtwIXQdQaIepZw
			setadd i 1
		if i|<|{recipes.Length} jump #while_zQxRjtBbzVXbBeJf
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_KlCXAQLKNioASLeY
	set i 0
	msg &eResources:
	#while_KlLIWVGnFynRcmEl
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_KlLIWVGnFynRcmEl
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_DswMSHrpQwDLRfgT
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_DswMSHrpQwDLRfgT
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_McRPkifnFkyQydwf
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_McRPkifnFkyQydwf
	#while_LMoRLcnJTUNrHmhj
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_LMoRLcnJTUNrHmhj
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_TFADGPbzrCgyDdkU
		set {runArg1} {runArg2}
		quit
	#ifnot_TFADGPbzrCgyDdkU
	set i 0
	#while_OgXNHqoLPUVXTish
		ifnot blocks[{i}].name|=|runArg2 jump #if_gforeBtCVfosXufR
			set {runArg1} {i}
			quit
		#if_gforeBtCVfosXufR
		setadd i 1
	if i|<|{blocks.Length} jump #while_OgXNHqoLPUVXTish
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_yMZTMOuiezIgkcpF
		ifnot recipes[{i}].output.id|=|bid jump #if_fWPMlTACCgeTKPVm
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_JHNmdyukVykQqVda
				set {pname} {i}
				quit
			#if_JHNmdyukVykQqVda
		#if_fWPMlTACCgeTKPVm
		setadd i 1
	if i|<|{recipes.Length} jump #while_yMZTMOuiezIgkcpF
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_xTICAFLPGoUqMOfh
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_nRgpKkMwauQNAGtr
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_nRgpKkMwauQNAGtr
	#ifnot_xTICAFLPGoUqMOfh
...
#use[68]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_DtbtZrwTvGuRleMn
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_pZOXorwYzWxShOKJ
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
		#if_pZOXorwYzWxShOKJ
	#ifnot_DtbtZrwTvGuRleMn
	setdeathspawn {PlayerCoords} {PlayerYaw} {PlayerPitch}
	set spawnblock {runArg1} {runArg2} {runArg3}
	msg &fRespawn point set
quit
