...
// start scope
	set l_YEZKCm_PrevPlayerCoords 
	set l_uUvVyS_prevhp 
	set l_gpBUUy_hpbar 
	set l_chZVXN_firebar 
	set l_iGiRjN_myblock 
	set l_wfMrpS_firetickmod 
	set l_yvYnRt_temp 
	#tick
		call #getblock|l_iGiRjN_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_iGiRjN_myblock}].catchFire jump #if_uYjIGl
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_uYjIGl
		ifnot blocks[{l_iGiRjN_myblock}].extinguishFire jump #if_AlRguM
			ifnot fireticks|>|0 jump #if_wHuljr
				gui barSize 0
				set fireticks 0
			#if_wHuljr
		#if_AlRguM
		ifnot blocks[{l_iGiRjN_myblock}].damage|=|"" call #damage|{blocks[{l_iGiRjN_myblock}].damage}|{blocks[{l_iGiRjN_myblock}].damageType}
		ifnot PlayerCoords|=|l_YEZKCm_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_YEZKCm_PrevPlayerCoords set usingStonecutter false
		set l_YEZKCm_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_uUvVyS_prevhp jump #ifnot_OLlnPC
			set l_uUvVyS_prevhp {hp}
			call #makebar|l_gpBUUy_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_gpBUUy_hpbar}
		#ifnot_OLlnPC
		...
		ifnot iframes|>|0 jump #if_XVVtFe
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_XVVtFe
		ifnot fireticks|>|0 jump #if_xCrlfT
			setsub fireticks 1
			set l_wfMrpS_firetickmod {fireticks}
			setmod l_wfMrpS_firetickmod 10
			ifnot l_wfMrpS_firetickmod|=|0 jump #if_PZNWnE
				ifnot fireticks|>|0 jump #if_biPiPO
					set l_yvYnRt_temp {fireticks}
					setdiv l_yvYnRt_temp 10
					call #makecharbar|l_chZVXN_firebar|▐|6|{l_yvYnRt_temp}|10
					cpemsg smallannounce {l_chZVXN_firebar}
				#if_biPiPO
...
#mine
	...
	ifnot minetimer|>|0 jump #if_GNGxBG
...
#give
	ifnot isTool({runArg1}) jump #if_aSDTUd
		set {runArg1} {runArg2}
		quit
	#if_aSDTUd
...
#giveall
	...
	#while_yBVPUL
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_yBVPUL
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_pTHjXQ
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_iHMxeE
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_ODLbCH
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_bHhrtI
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_AbLOoD
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_jPuivc
			call #take|{playerHeldBlock}|1
			jump #setblock|{blocks[{id}].mergeInto}|{x}|{y}|{z}
			quit
		#if_jPuivc
	#ifnot_AbLOoD
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_neJkFK
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_bJXfua
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_QWVyWs
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_KZTrHo
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_UBQIml
		#while_wNSVTY
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_wNSVTY
	#if_UBQIml
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_fJMHTL
		#while_UDJijl
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_UDJijl
	#if_fJMHTL
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_aEyaNb
		#while_jLhkWr
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_jLhkWr
	#if_aEyaNb
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_OBxzag
		#while_DzvMrK
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_DzvMrK
	#if_OBxzag
quit

#input
	ifnot runArg1|=|"craft" jump #if_PaQHzP
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_InIAht
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_OiUADr
				msg &cInvalid item name or ID
				quit
			#if_OiUADr
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_DPkmiE
		...
		if usingWorkbench jump #ifnot_gevRCY
		...
		#while_PCaWjl
			...
			set ingrediantList
			ifnot canAfford|>|0 jump #if_eHnbss
				...
				#while_jSrjDb
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_jSrjDb
				msg {ingrediantList}
			#if_eHnbss
			setadd i 1
		if i|<|{recipes.Length} jump #while_PCaWjl
	...
	#while_MjNmUb
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_MjNmUb
	msg &eTools:
...
#doCraft
	...
	#while_EskEDo
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_EskEDo
...
#checkRecipeAfford
	set j 0
	set {runArg2} 999
	if recipes[{runArg1}].condition|=|"" jump #ifnot_GWFixA
		ifnot {recipes[{runArg1}].condition} set {runArg2} 0
	#ifnot_GWFixA
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_mQtdex
	...
	#while_dkAPvw
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_dkAPvw
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_OrXTDL
		set {runArg1} {runArg2}
		quit
	#ifnot_OrXTDL
	set i 0
	#while_FrZqOf
		ifnot blocks[{i}].name|=|runArg2 jump #if_wpjQSv
			set {runArg1} {i}
			quit
		#if_wpjQSv
		setadd i 1
	if i|<|{blocks.Length} jump #while_FrZqOf
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_vxGHqT
		ifnot recipes[{i}].output.id|=|bid jump #if_PlZmRh
			call #checkRecipeAfford|{i}|canAfford
			ifnot canAfford|>=|c jump #if_bYmBoE
				set {pname} {i}
				quit
			#if_bYmBoE
		#if_PlZmRh
		setadd i 1
	if i|<|{recipes.Length} jump #while_vxGHqT
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_wpqxEu
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_rkrjaU
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_vJEhmY
		call #take|80|1
		call #give|70|1
	#if_vJEhmY
quit
