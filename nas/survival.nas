...
#tick
	...
	if blocks[{id}].setFireTicks|=|"" jump #ifnot_tQnEIyzleZlOCAah
		ifnot fireticks|>|0 jump #if_FDpwpqsolyLEZXJC
			if {blocks[{id}].setFireTicks}|=|0 gui barSize 0
		#if_FDpwpqsolyLEZXJC
		set fireticks {blocks[{id}].setFireTicks}
	#ifnot_tQnEIyzleZlOCAah
	...
	if hp|=|prevhp jump #ifnot_xGAymwXIbuAaEMyM
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_xGAymwXIbuAaEMyM
	...
	ifnot iframes|>|0 jump #if_zXqwrZCMBzWTsXBZ
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_zXqwrZCMBzWTsXBZ
	ifnot fireticks|>|0 jump #if_qECLCTyClodoFxrj
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_gsmspeVNWkEjUYiI
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_BTntORDRVJQnYeJX
...
#mine
	...
	ifnot minetimer|>|0 jump #if_KdmaHGjRjvJFdGDp
	...
	#if_KdmaHGjRjvJFdGDp
	set minepos
	if toomuch jump #ifnot_yxHASaFYXcaUtQll
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_yxHASaFYXcaUtQll
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_lgrPlsWFHJngsdjB
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_QaEZAJjJyObTkuLl
		set {runArg1} {runArg2}
		quit
	#if_QaEZAJjJyObTkuLl
...
#giveall
	set i 0
	#while_dYTKyPEbBUsLLywY
		set inventory[{i}] 9999
		setadd i 1
	if i|<|{blocks.Length} jump #while_dYTKyPEbBUsLLywY
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_yBZiKKZpHgLyAqoz
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_yBZiKKZpHgLyAqoz
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_ZZxgdvxnNIhPkYOr
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_YUEktwoTuaboxpAR
		#while_reghvoLBoIbtVYiz
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_reghvoLBoIbtVYiz
	#if_YUEktwoTuaboxpAR
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_UFpgCDdyvzknvjlO
		#while_YLGVvmnmHIJWMspJ
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_YLGVvmnmHIJWMspJ
	#if_UFpgCDdyvzknvjlO
quit

#input
	ifnot runArg1|=|"craft" jump #if_UhaUPxzZVsvXCRUN
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_VxscDtyegXiayyAK
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_duwyNktFTzMbijdT
				msg &cInvalid item name or ID
				quit
			#if_duwyNktFTzMbijdT
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_IiNNiCozownwdvWs
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_IiNNiCozownwdvWs
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_VxscDtyegXiayyAK
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_dlJzNMPGFrVtvsdg
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_dlJzNMPGFrVtvsdg
		set i 0
		#while_PjsXahExftDemxCm
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_CpfLqcYsVXzYSzWG
				...
				#while_BsXGkyafHCtVOhnO
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_BsXGkyafHCtVOhnO
				msg {ingrediantList}
			#if_CpfLqcYsVXzYSzWG
			setadd i 1
		if i|<|{recipes.Length} jump #while_PjsXahExftDemxCm
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_UhaUPxzZVsvXCRUN
	set i 0
	msg &eResources:
	#while_WLTGnDallSxKTypA
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_WLTGnDallSxKTypA
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_piFTsKhkmxNrbQQJ
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_piFTsKhkmxNrbQQJ
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_WsNvsjvJUmLmSuVI
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_WsNvsjvJUmLmSuVI
	#while_rtexfYCQZjQFETPf
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_rtexfYCQZjQFETPf
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_VpYBUXXsaiZnQcyW
		set {runArg1} {runArg2}
		quit
	#ifnot_VpYBUXXsaiZnQcyW
	set i 0
	#while_ZtYxCKULygHPendt
		ifnot blocks[{i}].name|=|runArg2 jump #if_nejRgOIhDJPHPYcx
			set {runArg1} {i}
			quit
		#if_nejRgOIhDJPHPYcx
		setadd i 1
	if i|<|{blocks.Length} jump #while_ZtYxCKULygHPendt
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_SIuPiToPmooiUMnX
		ifnot recipes[{i}].output.id|=|bid jump #if_VmHLpmBzWHNtJPaQ
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_QtEAyQOElvsjcIDb
				set {pname} {i}
				quit
			#if_QtEAyQOElvsjcIDb
		#if_VmHLpmBzWHNtJPaQ
		setadd i 1
	if i|<|{recipes.Length} jump #while_SIuPiToPmooiUMnX
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_IeZvUBpUjudRkrHR
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_IylULzLHmuZIqoUl
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_IylULzLHmuZIqoUl
	#ifnot_IeZvUBpUjudRkrHR
...
#use[68]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_iPfRIKSFhykdTBDZ
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_cOvQHRNAQrrEGZCa
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
			quit
		#if_cOvQHRNAQrrEGZCa
	#ifnot_iPfRIKSFhykdTBDZ
	setdeathspawn {PlayerCoords} {PlayerYaw} {PlayerPitch}
	set spawnblock {runArg1} {runArg2} {runArg3}
	msg &fRespawn point set
quit
