...
// start scope
	set l_PwfYpi_PrevPlayerCoords 
	set l_OLCOZc_prevhp 
	set l_XKlYOV_hpbar 
	set l_hpQjYI_firebar 
	set l_NyJQYm_myblock 
	set l_NSxGFS_firetickmod 
	set l_JXcams_temp 
	#tick
		call #getblock|l_NyJQYm_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_NyJQYm_myblock}].catchFire jump #if_iUpPVX
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_iUpPVX
		ifnot blocks[{l_NyJQYm_myblock}].extinguishFire jump #if_rvZcyf
			ifnot fireticks|>|0 jump #if_FtlZup
				gui barSize 0
				set fireticks 0
			#if_FtlZup
		#if_rvZcyf
		ifnot blocks[{l_NyJQYm_myblock}].damage|=|"" call #damage|{blocks[{l_NyJQYm_myblock}].damage}|{blocks[{l_NyJQYm_myblock}].damageType}
		ifnot PlayerCoords|=|l_PwfYpi_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_PwfYpi_PrevPlayerCoords set usingStonecutter false
		set l_PwfYpi_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_OLCOZc_prevhp jump #ifnot_nXOJjW
			set l_OLCOZc_prevhp {hp}
			call #makebar|l_XKlYOV_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_XKlYOV_hpbar}
		#ifnot_nXOJjW
		...
		ifnot iframes|>|0 jump #if_dFUZzc
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_dFUZzc
		ifnot fireticks|>|0 jump #if_KeIOCS
			setsub fireticks 1
			set l_NSxGFS_firetickmod {fireticks}
			setmod l_NSxGFS_firetickmod 10
			ifnot l_NSxGFS_firetickmod|=|0 jump #if_LfCLMd
				ifnot fireticks|>|0 jump #if_NfKtXl
					set l_JXcams_temp {fireticks}
					setdiv l_JXcams_temp 10
					call #makecharbar|l_hpQjYI_firebar|▐|6|{l_JXcams_temp}|10
					cpemsg smallannounce {l_hpQjYI_firebar}
				#if_NfKtXl
...
#mine
	...
	ifnot minetimer|>|0 jump #if_bGGkZg
...
#give
	ifnot isTool({runArg1}) jump #if_bWZuBu
		set {runArg1} {runArg2}
		quit
	#if_bWZuBu
...
#giveall
	...
	#while_wnbNnG
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_wnbNnG
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_dRypDN
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_KdXmQB
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_PiHQmD
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_NgwqUi
	...
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_qIwjkN
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_vXuAdL
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_xukuzd
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_aysBeG
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_GvHLVl
		#while_oityOH
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_oityOH
	#if_GvHLVl
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_zKsDrf
		#while_JJtTiy
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_JJtTiy
	#if_zKsDrf
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_bCYcJL
		#while_nHXKOE
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_nHXKOE
	#if_bCYcJL
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_paBVzF
		#while_nbrPpN
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_nbrPpN
	#if_paBVzF
quit

#input
	ifnot runArg1|=|"craft" jump #if_TFyjyq
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_lSsYFL
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_QuieZZ
				msg &cInvalid item name or ID
				quit
			#if_QuieZZ
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_AVsMTx
		...
		if usingWorkbench jump #ifnot_FNhQod
		...
		#while_QHBKKe
			...
			set ingrediantList
			ifnot canAfford|>|0 jump #if_mxLxRw
				...
				#while_PUuYsk
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_PUuYsk
				msg {ingrediantList}
			#if_mxLxRw
			setadd i 1
		if i|<|{recipes.Length} jump #while_QHBKKe
	...
	#while_MdzHrX
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_MdzHrX
	msg &eTools:
...
#doCraft
	...
	#while_XIcgVM
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_XIcgVM
...
#checkRecipeAfford
	set j 0
	set {runArg2} 999
	if recipes[{runArg1}].condition|=|"" jump #ifnot_GcWZyI
		ifnot {recipes[{runArg1}].condition} set {runArg2} 0
	#ifnot_GcWZyI
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_gmkxSE
	...
	#while_TGskgC
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_TGskgC
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_GmVRGt
		set {runArg1} {runArg2}
		quit
	#ifnot_GmVRGt
	set i 0
	#while_hzeUYb
		ifnot blocks[{i}].name|=|runArg2 jump #if_yjhhNy
			set {runArg1} {i}
			quit
		#if_yjhhNy
		setadd i 1
	if i|<|{blocks.Length} jump #while_hzeUYb
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_yjxMru
		ifnot recipes[{i}].output.id|=|bid jump #if_cimYGR
			call #checkRecipeAfford|{i}|canAfford
			ifnot canAfford|>=|c jump #if_tjJhOV
				set {pname} {i}
				quit
			#if_tjJhOV
		#if_cimYGR
		setadd i 1
	if i|<|{recipes.Length} jump #while_yjxMru
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_TPceHU
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_JBhWOU
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_MVnSGL
		call #take|80|1
		call #give|70|1
	#if_MVnSGL
quit
