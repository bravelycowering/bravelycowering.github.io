...
#tick
	...
	ifnot blocks[{id}].kills jump #if_HgTFRZwBOpjFtrPu
		msg your died
		kill
	#if_HgTFRZwBOpjFtrPu
...
#mine
	...
	ifnot minetimer|>|0 jump #if_evWfyTwrJhsslLpv
	...
	#if_evWfyTwrJhsslLpv
	set minepos
	if toomuch jump #ifnot_kXIuAHaSxfbZjTjs
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_kXIuAHaSxfbZjTjs
...
#give
	ifnot isTool({runArg1}) jump #if_CwRghmHGVslmpYQT
		set {runArg1} {runArg2}
		quit
	#if_CwRghmHGVslmpYQT
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_qbdOCxlIPPINkJSO
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_qbdOCxlIPPINkJSO
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_lumGHGlJXXAgupHe
		...
		#while_fPBlGxrKePeTOGYJ
			...
			ifnot canAfford jump #if_xCFOWUqryotUlSpX
				...
				#while_tWSNQGxHcCEjizLR
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_tWSNQGxHcCEjizLR
				msg {ingrediantList}
			#if_xCFOWUqryotUlSpX
			setadd i 1
		if i|<|{recipes.Length} jump #while_fPBlGxrKePeTOGYJ
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_PnfEjQMnLyYjksXQ
	set i 0
	msg &eResources:
	#while_uUsvhFgktkJvgdqO
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_uUsvhFgktkJvgdqO
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_ZLBoBTcJgqAhyxDe
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_ZLBoBTcJgqAhyxDe
	#while_acETkeqgUwriLufo
		...
		ifnot isTool({id}) jump #if_BGEUSVNpEZaKKsSX
			if {id}|>=|count quit
		#if_BGEUSVNpEZaKKsSX
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_acETkeqgUwriLufo
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_PtQlYfWoGYAAXaXg
		set {runArg1} {runArg2}
		quit
	#ifnot_PtQlYfWoGYAAXaXg
	set i 0
	#while_oGJmJCuFDDGlagtb
		ifnot blocks[{i}].name|=|runArg2 jump #if_tjNoekyNigcBscLj
			set {runArg1} {i}
			quit
		#if_tjNoekyNigcBscLj
		setadd i 1
	if i|<|{blocks.Length} jump #while_oGJmJCuFDDGlagtb
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_CMmcDGHfLxaStkKu
		ifnot recipes[{i}].output.id|=|bid jump #if_ZpoFcUFLvGMULpgt
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_dxgPAdqlLXiKdQbP
				set {pname} {i}
				quit
			#if_dxgPAdqlLXiKdQbP
		#if_ZpoFcUFLvGMULpgt
		setadd i 1
	if i|<|{recipes.Length} jump #while_CMmcDGHfLxaStkKu
quit
