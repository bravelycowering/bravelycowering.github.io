...
// start scope
	set l_JFdHuc_PrevPlayerCoords 
	set l_czaIpr_prevhp 
	set l_UGxbzm_hpbar 
	set l_bEnBpe_myblock 
	#tick
		call #getblock|l_bEnBpe_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_bEnBpe_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_bEnBpe_myblock}].extinguishFire jump #if_WlNbLK
			ifnot fireticks|>|0 jump #if_otVmtP
				gui barSize 0
				set fireticks 0
			#if_otVmtP
		#if_WlNbLK
		ifnot blocks[{l_bEnBpe_myblock}].damage|=|"" call #damage|{blocks[{l_bEnBpe_myblock}].damage}|{blocks[{l_bEnBpe_myblock}].damageType}
		ifnot PlayerCoords|=|l_JFdHuc_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_JFdHuc_PrevPlayerCoords set usingStonecutter false
		set l_JFdHuc_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_czaIpr_prevhp jump #ifnot_XmEsFp
			set l_czaIpr_prevhp {hp}
			call #makebar|l_UGxbzm_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_UGxbzm_hpbar}
		#ifnot_XmEsFp
		...
		ifnot iframes|>|0 jump #if_TyHGDr
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_TyHGDr
		ifnot fireticks|>|0 jump #if_LrYLTW
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_cxxIVV
...
#mine
	...
	ifnot minetimer|>|0 jump #if_quxSGn
...
#give
	ifnot isTool({runArg1}) jump #if_xZuQOh
		set {runArg1} {runArg2}
		quit
	#if_xZuQOh
...
#giveall
	...
	#while_iQeSfL
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_iQeSfL
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_rpZXJF
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_rpZXJF
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_CXKouP
...
#setblock
	if allowMapChanges jump #ifnot_CnIYLP
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_DffiOG
		#while_TVHisH
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_TVHisH
	#if_DffiOG
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_TQEAAH
		#while_XPCLGY
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_XPCLGY
	#if_TQEAAH
quit

#input
	ifnot runArg1|=|"craft" jump #if_oGasuN
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_NFPpBX
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_ObBVSs
				msg &cInvalid item name or ID
				quit
			#if_ObBVSs
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_IRYvtZ
		...
		if usingWorkbench jump #ifnot_xkhxFs
		...
		#while_JOjZCU
			...
			set ingrediantList
			ifnot canAfford jump #if_VSkFRp
				...
				#while_WbGcOw
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_WbGcOw
				msg {ingrediantList}
			#if_VSkFRp
			setadd i 1
		if i|<|{recipes.Length} jump #while_JOjZCU
	...
	#while_QGUznk
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_QGUznk
	msg &eTools:
...
#doCraft
	...
	#while_LSTAGK
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_LSTAGK
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_MrfKtT
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_MrfKtT
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_ePzplL
	...
	#while_WVhgzX
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_WVhgzX
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_qcxcjM
		set {runArg1} {runArg2}
		quit
	#ifnot_qcxcjM
	set i 0
	#while_WSBATP
		ifnot blocks[{i}].name|=|runArg2 jump #if_bXxtgl
			set {runArg1} {i}
			quit
		#if_bXxtgl
		setadd i 1
	if i|<|{blocks.Length} jump #while_WSBATP
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_XISezh
		ifnot recipes[{i}].output.id|=|bid jump #if_OHfvYA
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_XmNHmc
				set {pname} {i}
				quit
			#if_XmNHmc
		#if_OHfvYA
		setadd i 1
	if i|<|{recipes.Length} jump #while_XISezh
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_GzcGJJ
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_hyNmjN
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_bclFDm
		call #take|80|1
		call #give|70|1
	#if_bclFDm
quit
