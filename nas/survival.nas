...
// start scope
	set l_RArcUE_PrevPlayerCoords 
	set l_btheQZ_prevhp 
	set l_NpNNyQ_hpbar 
	set l_JSyMdQ_firebar 
	set l_EPuhaW_myblock 
	set l_siwwyO_firetickmod 
	set l_gChGkR_temp 
	#tick
		call #getblock|l_EPuhaW_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_EPuhaW_myblock}].catchFire jump #if_vioSfa
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_vioSfa
		ifnot blocks[{l_EPuhaW_myblock}].extinguishFire jump #if_uccTfg
			ifnot fireticks|>|0 jump #if_SvBRzJ
				gui barSize 0
				set fireticks 0
			#if_SvBRzJ
		#if_uccTfg
		ifnot blocks[{l_EPuhaW_myblock}].damage|=|"" call #damage|{blocks[{l_EPuhaW_myblock}].damage}|{blocks[{l_EPuhaW_myblock}].damageType}
		ifnot PlayerCoords|=|l_RArcUE_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_RArcUE_PrevPlayerCoords set usingStonecutter false
		set l_RArcUE_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_btheQZ_prevhp jump #ifnot_kOgtOs
			set l_btheQZ_prevhp {hp}
			call #makebar|l_NpNNyQ_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_NpNNyQ_hpbar}
		#ifnot_kOgtOs
		...
		ifnot iframes|>|0 jump #if_UFAdGt
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_UFAdGt
		ifnot fireticks|>|0 jump #if_ifXxXd
			setsub fireticks 1
			set l_siwwyO_firetickmod {fireticks}
			setmod l_siwwyO_firetickmod 10
			ifnot l_siwwyO_firetickmod|=|0 jump #if_tuUguR
				ifnot fireticks|>|0 jump #if_gVQbTS
					set l_gChGkR_temp {fireticks}
					setdiv l_gChGkR_temp 10
					call #makecharbar|l_JSyMdQ_firebar|▐|6|{l_gChGkR_temp}|10
					cpemsg smallannounce {l_JSyMdQ_firebar}
				#if_gVQbTS
...
#mine
	...
	ifnot minetimer|>|0 jump #if_RdUUeD
...
#give
	ifnot isTool({runArg1}) jump #if_EUkbsB
		set {runArg1} {runArg2}
		quit
	#if_EUkbsB
...
#giveall
	...
	#while_gacddp
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_gacddp
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_KdKHlh
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_KdKHlh
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_rTWfyU
...
#setblock
	if allowMapChanges jump #ifnot_OqhBUL
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_UXUepr
		#while_ZsrQaN
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_ZsrQaN
	#if_UXUepr
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_fHBtaA
		#while_lCgeHm
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_lCgeHm
	#if_fHBtaA
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_ygEuUp
		#while_XHRLgL
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_XHRLgL
	#if_ygEuUp
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_zkUTOR
		#while_VQrQcn
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_VQrQcn
	#if_zkUTOR
quit

#input
	ifnot runArg1|=|"craft" jump #if_MpZlNC
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_IhVjiJ
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_OwAYKn
				msg &cInvalid item name or ID
				quit
			#if_OwAYKn
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_FYCWjF
		...
		if usingWorkbench jump #ifnot_IAqbQh
		...
		#while_RjvvWI
			...
			set ingrediantList
			ifnot canAfford jump #if_owtaOX
				...
				#while_rYbVhG
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_rYbVhG
				msg {ingrediantList}
			#if_owtaOX
			setadd i 1
		if i|<|{recipes.Length} jump #while_RjvvWI
	...
	#while_qpKjNK
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_qpKjNK
	msg &eTools:
...
#doCraft
	...
	#while_ZEQjIB
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_ZEQjIB
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_tbcPpB
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_tbcPpB
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_UUshNW
	...
	#while_HbSxgr
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_HbSxgr
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_GhrOuj
		set {runArg1} {runArg2}
		quit
	#ifnot_GhrOuj
	set i 0
	#while_OxCEAh
		ifnot blocks[{i}].name|=|runArg2 jump #if_MsWTiz
			set {runArg1} {i}
			quit
		#if_MsWTiz
		setadd i 1
	if i|<|{blocks.Length} jump #while_OxCEAh
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_XfExuR
		ifnot recipes[{i}].output.id|=|bid jump #if_mnuNLn
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_LFrpMS
				set {pname} {i}
				quit
			#if_LFrpMS
		#if_mnuNLn
		setadd i 1
	if i|<|{recipes.Length} jump #while_XfExuR
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_lEMJRK
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_RumtoL
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_zhnNXg
		call #take|80|1
		call #give|70|1
	#if_zhnNXg
quit
