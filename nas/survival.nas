...
#tick
	...
	if hp|=|prevhp jump #ifnot_AJjcpTLaWNzevqbH
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_AJjcpTLaWNzevqbH
	...
	ifnot iframes|>|0 jump #if_ViCWqnrrlofmQfQl
		setsub iframes 1
...
#damage
	...
	set iframes 4
	cs me ow
	ifnot hp|<=|0 jump #if_rHbEBlXremnqDYFN
...
#mine
	...
	ifnot minetimer|>|0 jump #if_tTWIbUaYamIDxQes
	...
	#if_tTWIbUaYamIDxQes
	set minepos
	if toomuch jump #ifnot_HtZxiDiEQLcpoaio
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_HtZxiDiEQLcpoaio
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_nobsaztHFzLFXlmf
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_mvhxRFzuEcymGSHI
		set {runArg1} {runArg2}
		quit
	#if_mvhxRFzuEcymGSHI
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_JbTjjFomAmOQMZUI
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_JbTjjFomAmOQMZUI
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_oLRbzhqcDychaKna
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_rIoLjGbDzzKXOYmn
		#while_VpRZJNGWtpTKcVWU
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_VpRZJNGWtpTKcVWU
	#if_rIoLjGbDzzKXOYmn
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_KJmieiwubdZtjfPS
		#while_xHHtxfptRgUrNEUo
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_xHHtxfptRgUrNEUo
	#if_KJmieiwubdZtjfPS
quit

#input
	ifnot runArg1|=|"craft" jump #if_KEFVExEnhNzXyqAo
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_LzsDqUkZDpWYdTZf
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_lIPIsytaUqZcGZlS
				msg &cInvalid item name or ID
				quit
			#if_lIPIsytaUqZcGZlS
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_oszyGENlkBbBjFJh
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_oszyGENlkBbBjFJh
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_LzsDqUkZDpWYdTZf
		if usingWorkbench msg &eWorkbench Recipes:
		else msg &eRecipes:
		set i 0
		#while_STIfyqJiHnKJzXLq
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_SmBnERxHsoNWiGyi
				...
				#while_yZtcPEbhqHSBNjDT
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_yZtcPEbhqHSBNjDT
				msg {ingrediantList}
			#if_SmBnERxHsoNWiGyi
			setadd i 1
		if i|<|{recipes.Length} jump #while_STIfyqJiHnKJzXLq
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_KEFVExEnhNzXyqAo
	set i 0
	msg &eResources:
	#while_fPUhmQScoqsrXLcu
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_fPUhmQScoqsrXLcu
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_gOxHOQgqEKJqpHNv
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_gOxHOQgqEKJqpHNv
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_yRinLWRdaKKWDmNh
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_yRinLWRdaKKWDmNh
	#while_yeStTnkozvGIyVZJ
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_yeStTnkozvGIyVZJ
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_oHgWskItxbqCOGDR
		set {runArg1} {runArg2}
		quit
	#ifnot_oHgWskItxbqCOGDR
	set i 0
	#while_DUTxngpYUJPegeEu
		ifnot blocks[{i}].name|=|runArg2 jump #if_NXAQSDQagVkDYnWV
			set {runArg1} {i}
			quit
		#if_NXAQSDQagVkDYnWV
		setadd i 1
	if i|<|{blocks.Length} jump #while_DUTxngpYUJPegeEu
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_kKJqoaGFHTmzliEQ
		ifnot recipes[{i}].output.id|=|bid jump #if_RtZORRaTZMuWtZKI
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_aStEyqsKfWnVCMTW
				set {pname} {i}
				quit
			#if_aStEyqsKfWnVCMTW
		#if_RtZORRaTZMuWtZKI
		setadd i 1
	if i|<|{recipes.Length} jump #while_kKJqoaGFHTmzliEQ
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_QPxicMXpdGQwykun
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_cGbWaffzmLPfIust
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_cGbWaffzmLPfIust
	#ifnot_QPxicMXpdGQwykun
quit
...
set recipes[17].output.id 80
set recipes[17].ingredients[0].count 1
set recipes[17].ingredients[0].id 66
set recipes[17].ingredients[1].count 1
set recipes[17].ingredients[1].id 57
set recipes[17].ingredients.Length 2
set recipes[18].output.count 4
set recipes[18].output.id 5
set recipes[18].ingredients[0].count 1
set recipes[18].ingredients[0].id 17
