...
// start scope
	set l_YDLMlL_PrevPlayerCoords 
	set l_efsnUD_prevhp 
	set l_DxiAeq_hpbar 
	set l_hzeHTa_myblock 
	#tick
		call #getblock|l_hzeHTa_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_hzeHTa_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_hzeHTa_myblock}].extinguishFire jump #if_MCVLxa
			ifnot fireticks|>|0 jump #if_SiXrtt
				gui barSize 0
				set fireticks 0
			#if_SiXrtt
		#if_MCVLxa
		ifnot blocks[{l_hzeHTa_myblock}].damage|=|"" call #damage|{blocks[{l_hzeHTa_myblock}].damage}|{blocks[{l_hzeHTa_myblock}].damageType}
		ifnot PlayerCoords|=|l_YDLMlL_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_YDLMlL_PrevPlayerCoords set usingStonecutter false
		set l_YDLMlL_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_efsnUD_prevhp jump #ifnot_GInSIL
			set l_efsnUD_prevhp {hp}
			call #makebar|l_DxiAeq_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_DxiAeq_hpbar}
		#ifnot_GInSIL
		...
		ifnot iframes|>|0 jump #if_JTZYTO
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_JTZYTO
		ifnot fireticks|>|0 jump #if_ReEXEh
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_SgRVeC
...
#mine
	...
	ifnot minetimer|>|0 jump #if_vwaRCT
...
#give
	ifnot isTool({runArg1}) jump #if_oAwEkj
		set {runArg1} {runArg2}
		quit
	#if_oAwEkj
...
#giveall
	...
	#while_PSOqRX
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_PSOqRX
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_UjrsWm
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_UjrsWm
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_arlDLg
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_PXudaB
		#while_UDUuAO
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_UDUuAO
	#if_PXudaB
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_twhNsS
		#while_ykoUPB
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_ykoUPB
	#if_twhNsS
quit

#input
	ifnot runArg1|=|"craft" jump #if_XYEMas
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_LnxuAa
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_RzcxEt
				msg &cInvalid item name or ID
				quit
			#if_RzcxEt
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_otSxUO
		...
		if usingWorkbench jump #ifnot_uEQTrC
		...
		#while_UfZpIk
			...
			set ingrediantList
			ifnot canAfford jump #if_fiVVTY
				...
				#while_IWPRPR
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_IWPRPR
				msg {ingrediantList}
			#if_fiVVTY
			setadd i 1
		if i|<|{recipes.Length} jump #while_UfZpIk
	...
	#while_eDGMOI
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_eDGMOI
	msg &eTools:
...
#doCraft
	...
	#while_MWCDQd
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_MWCDQd
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_OeJAOg
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_OeJAOg
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_poFTPJ
	...
	#while_rAHINx
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_rAHINx
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_cvSdmV
		set {runArg1} {runArg2}
		quit
	#ifnot_cvSdmV
	set i 0
	#while_JrAJxS
		ifnot blocks[{i}].name|=|runArg2 jump #if_BVgiiQ
			set {runArg1} {i}
			quit
		#if_BVgiiQ
		setadd i 1
	if i|<|{blocks.Length} jump #while_JrAJxS
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_yPiDjR
		ifnot recipes[{i}].output.id|=|bid jump #if_lqIfoH
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_MtXKJT
				set {pname} {i}
				quit
			#if_MtXKJT
		#if_lqIfoH
		setadd i 1
	if i|<|{recipes.Length} jump #while_yPiDjR
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_oIXqLS
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_BPGtJn
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_SBhCUk
		call #take|80|1
		call #give|70|1
	#if_SBhCUk
quit
