...
// start scope
	set l_qMmXVn_PrevPlayerCoords 
	set l_bWAZdh_prevhp 
	set l_mKuDDl_hpbar 
	set l_ORmYdc_firebar 
	set l_ridmZG_myblock 
	set l_WVLIRp_firetickmod 
	set l_hvUpUF_temp 
	#tick
		call #getblock|l_ridmZG_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_ridmZG_myblock}].catchFire jump #if_gtqBXv
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_gtqBXv
		ifnot blocks[{l_ridmZG_myblock}].extinguishFire jump #if_UcVaFP
			ifnot fireticks|>|0 jump #if_UzBNQb
				gui barSize 0
				set fireticks 0
			#if_UzBNQb
		#if_UcVaFP
		ifnot blocks[{l_ridmZG_myblock}].damage|=|"" call #damage|{blocks[{l_ridmZG_myblock}].damage}|{blocks[{l_ridmZG_myblock}].damageType}
		ifnot PlayerCoords|=|l_qMmXVn_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_qMmXVn_PrevPlayerCoords set usingStonecutter false
		set l_qMmXVn_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_bWAZdh_prevhp jump #ifnot_ygrcJp
			set l_bWAZdh_prevhp {hp}
			call #makebar|l_mKuDDl_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_mKuDDl_hpbar}
		#ifnot_ygrcJp
		...
		ifnot iframes|>|0 jump #if_jPiewA
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_jPiewA
		ifnot fireticks|>|0 jump #if_WHJtEU
			setsub fireticks 1
			set l_WVLIRp_firetickmod {fireticks}
			setmod l_WVLIRp_firetickmod 10
			ifnot l_WVLIRp_firetickmod|=|0 jump #if_WjLGYN
				ifnot fireticks|>|0 jump #if_edaNYi
					set l_hvUpUF_temp {fireticks}
					setdiv l_hvUpUF_temp 10
					call #makecharbar|l_ORmYdc_firebar|▐|6|{l_hvUpUF_temp}|10
					cpemsg smallannounce {l_ORmYdc_firebar}
				#if_edaNYi
...
#mine
	...
	ifnot minetimer|>|0 jump #if_iiKBNa
...
#destroyblock
	...
	set toomuch {runArg4}
	if toomuch jump #ifnot_ydcDCg
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_ydcDCg
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_FLSLJY
		set spawnblock
		setdeathspawn {worldSpawn} 0 0
	#if_FLSLJY
	jump #setblock|{empty}|{x}|{y}|{z}
...
#give
	ifnot isTool({runArg1}) jump #if_aDBBeB
		set {runArg1} {runArg2}
		quit
	#if_aDBBeB
...
#giveall
	...
	#while_wdoGGJ
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_wdoGGJ
quit

#place
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_WgJtUR
	...
	set y {runArg2}
	set z {runArg3}
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_cTMyJL
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_cUcJyH
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_OHjIBq
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_fKxcVc
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_JFrGSA
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_paArgF
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_ZsYfej
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_jHjGvj
		#while_ZpHagb
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_ZpHagb
	#if_jHjGvj
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_oOwZAU
		#while_VRzzJm
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_VRzzJm
	#if_oOwZAU
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_jSLPUm
		#while_mOOKhJ
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_mOOKhJ
	#if_jSLPUm
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_eotRrQ
		#while_DkETrG
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_DkETrG
	#if_eotRrQ
quit

#input
	ifnot runArg1|=|"craft" jump #if_aIUncp
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_SfObVf
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_yfQoGD
				msg &cInvalid item name or ID
				quit
			#if_yfQoGD
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_sVphvN
		...
		if usingWorkbench jump #ifnot_VrQwrA
		...
		#while_ycLOSR
			...
			set ingrediantList
			ifnot canAfford jump #if_aTPmBN
				...
				#while_twGWxL
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_twGWxL
				msg {ingrediantList}
			#if_aTPmBN
			setadd i 1
		if i|<|{recipes.Length} jump #while_ycLOSR
	...
	#while_Pzdwso
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_Pzdwso
	msg &eTools:
...
#doCraft
	...
	#while_LLjJKa
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_LLjJKa
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_BehmkX
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_BehmkX
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_afIHUb
	...
	#while_xytMbR
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_xytMbR
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_cHnIlM
		set {runArg1} {runArg2}
		quit
	#ifnot_cHnIlM
	set i 0
	#while_DKRCCp
		ifnot blocks[{i}].name|=|runArg2 jump #if_GfoWqS
			set {runArg1} {i}
			quit
		#if_GfoWqS
		setadd i 1
	if i|<|{blocks.Length} jump #while_DKRCCp
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_nhJyWs
		ifnot recipes[{i}].output.id|=|bid jump #if_ppfZIP
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_zEesqp
				set {pname} {i}
				quit
			#if_zEesqp
		#if_ppfZIP
		setadd i 1
	if i|<|{recipes.Length} jump #while_nhJyWs
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_zEdTIx
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_RVLQIK
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_YTELiv
		call #take|80|1
		call #give|70|1
	#if_YTELiv
quit
