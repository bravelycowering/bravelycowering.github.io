...
#tick
	...
	ifnot blocks[{id}].kills jump #if_EVjzKouhnoZHtSdH
		set iframes 4
		msg your died
		kill
	#if_EVjzKouhnoZHtSdH
	ifnot iframes|>|0 jump #if_NODLKTLpPyYzJYru
		setsub iframes 1
	...
	if hp|=|prevhp jump #ifnot_bzpOCqDSXSwGDEoj
		set prevhp hp
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_bzpOCqDSXSwGDEoj
...
#mine
	...
	ifnot minetimer|>|0 jump #if_GYEmzPtsnyFtosxD
	...
	#if_GYEmzPtsnyFtosxD
	set minepos
	if toomuch jump #ifnot_rbituGhlTFjcgxlo
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_rbituGhlTFjcgxlo
...
#give
	ifnot isTool({runArg1}) jump #if_zmcJOwYTCBgoqcnF
		set {runArg1} {runArg2}
		quit
	#if_zmcJOwYTCBgoqcnF
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_cGKwlBRcmZkycymR
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_cGKwlBRcmZkycymR
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_uRKcFGoIGsfuvBOV
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_ZaNLTUwhQPSfFczP
		#while_yejQwZGscxXnwrxl
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_yejQwZGscxXnwrxl
	#if_ZaNLTUwhQPSfFczP
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_gpHaJhJWRKayGNmP
		#while_oZODJjvwxiezSFjc
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_oZODJjvwxiezSFjc
	#if_gpHaJhJWRKayGNmP
quit

#input
	ifnot runArg1|=|"craft" jump #if_dZFMHRRfntCUoUCZ
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_BDXHmgwIaKRxOAvh
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_kYxDySxrDmFPgCqv
				msg &cInvalid item name or ID
				quit
			#if_kYxDySxrDmFPgCqv
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_lFfQdogbGQnWTPbd
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_lFfQdogbGQnWTPbd
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_BDXHmgwIaKRxOAvh
		if usingWorkbench msg &eWorkbench Recipes:
		else msg &eRecipes:
		set i 0
		#while_RGVmrygbXTspakCl
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_salObdWpyszObgZf
				...
				#while_vXjTQUbHMHHMxlyU
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_vXjTQUbHMHHMxlyU
				msg {ingrediantList}
			#if_salObdWpyszObgZf
			setadd i 1
		if i|<|{recipes.Length} jump #while_RGVmrygbXTspakCl
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_dZFMHRRfntCUoUCZ
	set i 0
	msg &eResources:
	#while_xAcrVqfHvTtvESiz
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_xAcrVqfHvTtvESiz
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_sXyKxUDyDazWkPcD
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_sXyKxUDyDazWkPcD
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_uWGgQTJWRbYYEwnR
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_uWGgQTJWRbYYEwnR
	#while_enzMEkBzqorwMctr
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_enzMEkBzqorwMctr
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_aVRLCfhZOEOSzJXl
		set {runArg1} {runArg2}
		quit
	#ifnot_aVRLCfhZOEOSzJXl
	set i 0
	#while_kOgfmvokRVhgZRpz
		ifnot blocks[{i}].name|=|runArg2 jump #if_BuQndSQcFMOZIRNz
			set {runArg1} {i}
			quit
		#if_BuQndSQcFMOZIRNz
		setadd i 1
	if i|<|{blocks.Length} jump #while_kOgfmvokRVhgZRpz
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_iMlosyjkIExFCiGD
		ifnot recipes[{i}].output.id|=|bid jump #if_cSEgEfjbEqGugUcS
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_hzyaZZFQWiGmMVnv
				set {pname} {i}
				quit
			#if_hzyaZZFQWiGmMVnv
		#if_cSEgEfjbEqGugUcS
		setadd i 1
	if i|<|{recipes.Length} jump #while_iMlosyjkIExFCiGD
quit
