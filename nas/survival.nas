...
#tick
	...
	if hp|=|prevhp jump #ifnot_mdmzVSHmcVbSADkf
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_mdmzVSHmcVbSADkf
	...
	ifnot iframes|>|0 jump #if_BDNUssTxCMcWOdhC
		setsub iframes 1
		ifnot iframes|<|2 gui barColor #ff0000 0.25
		if iframes|<|2 gui barSize 0
		else gui barSize 1
	#if_BDNUssTxCMcWOdhC
	ifnot fireticks|>|0 jump #if_ulcUvxwUqeZmYxCI
		setsub fireticks 1
		ifnot iframes|<|2 jump #if_HOZBfsBoiPyOUUzV
...
#damage
	...
	set iframes 4
	cs me ow:select(7)
	ifnot hp|<=|0 jump #if_pKwFnzzQBDnxBdVP
...
#mine
	...
	ifnot minetimer|>|0 jump #if_MjVLYGVAZovbRrub
	...
	#if_MjVLYGVAZovbRrub
	set minepos
	if toomuch jump #ifnot_yhHPWfAexIGiFdlj
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_yhHPWfAexIGiFdlj
	if blocks[{id}].remainder|=|"" set empty 0
	else set empty {blocks[{id}].remainder}
	ifnot spawnblock|=|coords jump #if_meeyomlsCsPddKDQ
		set spawnblock
...
#give
	ifnot isTool({runArg1}) jump #if_taZGkpDiRnhETDFD
		set {runArg1} {runArg2}
		quit
	#if_taZGkpDiRnhETDFD
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_szRvToifYVCByJaQ
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_szRvToifYVCByJaQ
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_VZUyYWpZRDIqGJii
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_znUJdZTjGlVkYWEN
		#while_OZWVBdebZKRVxbBj
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_OZWVBdebZKRVxbBj
	#if_znUJdZTjGlVkYWEN
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_WKherhGUdmJDrAKL
		#while_sUpRKhRRbmgDNYXw
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_sUpRKhRRbmgDNYXw
	#if_WKherhGUdmJDrAKL
quit

#input
	ifnot runArg1|=|"craft" jump #if_iZipvlSEESvBMFND
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_vTDWLhrCZQoJxSeg
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_SzdTMCdJqibGpJaY
				msg &cInvalid item name or ID
				quit
			#if_SzdTMCdJqibGpJaY
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_JrqtAjkyJyBjmuEl
				msg &cYou cannot craft {blocks[{blockID}].name}!
				quit
			#if_JrqtAjkyJyBjmuEl
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_vTDWLhrCZQoJxSeg
		if usingWorkbench msg &eWorkbench Recipes:
		if usingWorkbench jump #ifnot_PNVVmZoFhRYvfaEX
			if usingStonecutter msg &eStonecutter Recipes:
			else msg &eRecipes:
		#ifnot_PNVVmZoFhRYvfaEX
		set i 0
		#while_eSuyZiPtwmxvXRWj
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_pKNNroNfiPruhyFr
				...
				#while_NZxvCkXySBuaBjUK
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_NZxvCkXySBuaBjUK
				msg {ingrediantList}
			#if_pKNNroNfiPruhyFr
			setadd i 1
		if i|<|{recipes.Length} jump #while_eSuyZiPtwmxvXRWj
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_iZipvlSEESvBMFND
	set i 0
	msg &eResources:
	#while_cXYlLeAoqrqZEPFG
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_cXYlLeAoqrqZEPFG
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_ITnMhPPWnItLcOAE
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_ITnMhPPWnItLcOAE
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_JyFrqeSQLLMaegiW
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_JyFrqeSQLLMaegiW
	#while_tIzommiPUCvPoCJZ
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_tIzommiPUCvPoCJZ
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_bPQOGaLYzECCPkjN
		set {runArg1} {runArg2}
		quit
	#ifnot_bPQOGaLYzECCPkjN
	set i 0
	#while_HmLuNmEvCFUcEXRV
		ifnot blocks[{i}].name|=|runArg2 jump #if_BHiGRirrPUWBhDge
			set {runArg1} {i}
			quit
		#if_BHiGRirrPUWBhDge
		setadd i 1
	if i|<|{blocks.Length} jump #while_HmLuNmEvCFUcEXRV
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_KUYgFJWbJqNlSaJI
		ifnot recipes[{i}].output.id|=|bid jump #if_VkhQgcgRiNENcwfc
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_wlLjfKvvXhINItee
				set {pname} {i}
				quit
			#if_wlLjfKvvXhINItee
		#if_VkhQgcgRiNENcwfc
		setadd i 1
	if i|<|{recipes.Length} jump #while_KUYgFJWbJqNlSaJI
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_LjcHULfBsDryHcTk
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_joXhzFehEegIwuYU
			call #setblock|68|{runArg1}|{runArg2}|{runArg3}
			call #take|{PlayerHeldBlock}|1
			call #give|{blocks[{PlayerHeldBlock}].campfireLighter}|1
		#if_joXhzFehEegIwuYU
	#ifnot_LjcHULfBsDryHcTk
quit
