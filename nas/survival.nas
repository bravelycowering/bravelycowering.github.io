...
// start scope
	set l_nCWxuQ_PrevPlayerCoords 
	set l_wpetdF_prevhp 
	set l_aVMwSm_hpbar 
	set l_QSBuwF_myblock 
	#tick
		call #getblock|l_QSBuwF_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		if blocks[{l_QSBuwF_myblock}].catchFire set fireticks 100
		ifnot blocks[{l_QSBuwF_myblock}].extinguishFire jump #if_CGJYDz
			ifnot fireticks|>|0 jump #if_lEkoUV
				gui barSize 0
				set fireticks 0
			#if_lEkoUV
		#if_CGJYDz
		ifnot blocks[{l_QSBuwF_myblock}].damage|=|"" call #damage|{blocks[{l_QSBuwF_myblock}].damage}|{blocks[{l_QSBuwF_myblock}].damageType}
		ifnot PlayerCoords|=|l_nCWxuQ_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_nCWxuQ_PrevPlayerCoords set usingStonecutter false
		set l_nCWxuQ_PrevPlayerCoords {PlayerCoords}
		delay 100
		// cpemsg top1 &c{actionCount}/60000
		if hp|=|l_wpetdF_prevhp jump #ifnot_WsPvQV
			set l_wpetdF_prevhp {hp}
			call #makebar|l_aVMwSm_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_aVMwSm_hpbar}
		#ifnot_WsPvQV
		...
		ifnot iframes|>|0 jump #if_ItaxzB
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_ItaxzB
		ifnot fireticks|>|0 jump #if_iYCexo
			setsub fireticks 1
			ifnot iframes|<|2 jump #if_yplCVz
...
#mine
	...
	ifnot minetimer|>|0 jump #if_lToeqd
...
#give
	ifnot isTool({runArg1}) jump #if_yGaPbs
		set {runArg1} {runArg2}
		quit
	#if_yGaPbs
...
#giveall
	...
	#while_RgzYlt
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_RgzYlt
quit
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_PFuZoK
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_PFuZoK
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_lvJjsz
...
#setblock
	if allowMapChanges jump #ifnot_vxeYYa
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_vtJioK
		#while_wHAIfv
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_wHAIfv
	#if_vtJioK
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_ULDwkS
		#while_ecQIeU
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_ecQIeU
	#if_ULDwkS
quit

#input
	ifnot runArg1|=|"craft" jump #if_hZRVzf
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_IWkeUx
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_ZPmhkE
				msg &cInvalid item name or ID
				quit
			#if_ZPmhkE
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_WeTJHo
		...
		if usingWorkbench jump #ifnot_HuCpMl
		...
		#while_yBmGTC
			...
			set ingrediantList
			ifnot canAfford jump #if_GaQzdz
				...
				#while_lzSIoj
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_lzSIoj
				msg {ingrediantList}
			#if_GaQzdz
			setadd i 1
		if i|<|{recipes.Length} jump #while_yBmGTC
	...
	#while_fuzOit
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_fuzOit
	msg &eTools:
...
#doCraft
	...
	#while_qefxOQ
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_qefxOQ
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_crRXoP
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_crRXoP
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_trAnDD
	...
	#while_EwxJEJ
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_EwxJEJ
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_jyzbQa
		set {runArg1} {runArg2}
		quit
	#ifnot_jyzbQa
	set i 0
	#while_IsCKSu
		ifnot blocks[{i}].name|=|runArg2 jump #if_THCCDc
			set {runArg1} {i}
			quit
		#if_THCCDc
		setadd i 1
	if i|<|{blocks.Length} jump #while_IsCKSu
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_NIIoVw
		ifnot recipes[{i}].output.id|=|bid jump #if_Tzeqzy
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_gRPmrT
				set {pname} {i}
				quit
			#if_gRPmrT
		#if_Tzeqzy
		setadd i 1
	if i|<|{recipes.Length} jump #while_NIIoVw
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_qQggAu
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_EDxvMC
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_Jkdcsz
		call #take|80|1
		call #give|70|1
	#if_Jkdcsz
quit
