...
#tick
	...
	if hp|=|prevhp jump #ifnot_lWlUxVLCNqGxHWaT
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_lWlUxVLCNqGxHWaT
	...
	ifnot iframes|>|0 jump #if_cbBHZmhPWWEkpVQf
		setsub iframes 1
...
#damage
	...
	set iframes 4
	cs me ow
	ifnot hp|<=|0 jump #if_pyljDMuvJUBNaTie
...
#mine
	...
	ifnot minetimer|>|0 jump #if_VujwvnmxsdJMrcUX
	...
	#if_VujwvnmxsdJMrcUX
	set minepos
	if toomuch jump #ifnot_PzghKPUZATxiXPNE
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_PzghKPUZATxiXPNE
...
#give
	ifnot isTool({runArg1}) jump #if_pWWkjTUMonHkiMjJ
		set {runArg1} {runArg2}
		quit
	#if_pWWkjTUMonHkiMjJ
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_mDwrHELRxSGAeqBs
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_mDwrHELRxSGAeqBs
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_hZtspJrVnjgRFycz
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_IJKoCbhLyQFSaVLi
		#while_LdnNRQGHBYwZIBrx
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_LdnNRQGHBYwZIBrx
	#if_IJKoCbhLyQFSaVLi
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_ygKbKyptukZpauxw
		#while_uDBexAMQSDYQcKTy
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_uDBexAMQSDYQcKTy
	#if_ygKbKyptukZpauxw
quit

#input
	ifnot runArg1|=|"craft" jump #if_PNKZNxdOXLKHuPUk
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_EIINrOpuvnnMPYtZ
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_RwdjkpbuZUpmvqIG
				msg &cInvalid item name or ID
				quit
			#if_RwdjkpbuZUpmvqIG
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_KLfbbkaKQzleaMeB
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_KLfbbkaKQzleaMeB
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_EIINrOpuvnnMPYtZ
		if usingWorkbench msg &eWorkbench Recipes:
		else msg &eRecipes:
		set i 0
		#while_WDBwCZYCUEkYbbhr
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_dQbJWQsfQQeaOLSb
				...
				#while_WMWkzHtjKMribaPD
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_WMWkzHtjKMribaPD
				msg {ingrediantList}
			#if_dQbJWQsfQQeaOLSb
			setadd i 1
		if i|<|{recipes.Length} jump #while_WDBwCZYCUEkYbbhr
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_PNKZNxdOXLKHuPUk
	set i 0
	msg &eResources:
	#while_uUdrpANZQedGoafS
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_uUdrpANZQedGoafS
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_PVaYGpmcDbTBNnmw
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_PVaYGpmcDbTBNnmw
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_kGOMSumSijXTkVmb
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_kGOMSumSijXTkVmb
	#while_reyMJGjPkfeLhoMr
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_reyMJGjPkfeLhoMr
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_oPGDSEAIxKBpUIbl
		set {runArg1} {runArg2}
		quit
	#ifnot_oPGDSEAIxKBpUIbl
	set i 0
	#while_JsmWYkgxdQQCuXto
		ifnot blocks[{i}].name|=|runArg2 jump #if_PabqjXtTFqxUocnb
			set {runArg1} {i}
			quit
		#if_PabqjXtTFqxUocnb
		setadd i 1
	if i|<|{blocks.Length} jump #while_JsmWYkgxdQQCuXto
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_AUfcXcPxwpIYlhvm
		ifnot recipes[{i}].output.id|=|bid jump #if_qMoUNPWBUiMVmZdB
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_tXRmsNbwekiDwYgF
				set {pname} {i}
				quit
			#if_tXRmsNbwekiDwYgF
		#if_qMoUNPWBUiMVmZdB
		setadd i 1
	if i|<|{recipes.Length} jump #while_AUfcXcPxwpIYlhvm
quit
