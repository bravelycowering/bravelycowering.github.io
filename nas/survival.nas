...
// start scope
	set l_fAktGz_PrevPlayerCoords 
	set l_gUyslR_prevhp 
	set l_mhDMiE_hpbar 
	set l_XSJcjQ_firebar 
	set l_RkRUoQ_myblock 
	set l_XiGhwD_firetickmod 
	set l_KizJMq_temp 
	#tick
		call #getblock|l_RkRUoQ_myblock|{PlayerX}|{PlayerY}|{PlayerZ}
		ifnot blocks[{l_RkRUoQ_myblock}].catchFire jump #if_bcKSfp
			set fireticks 100
			cpemsg smallannounce &6▐▐▐▐▐▐▐▐▐▐
		#if_bcKSfp
		ifnot blocks[{l_RkRUoQ_myblock}].extinguishFire jump #if_mVKKUj
			ifnot fireticks|>|0 jump #if_xcHdoN
				gui barSize 0
				set fireticks 0
			#if_xcHdoN
		#if_mVKKUj
		ifnot blocks[{l_RkRUoQ_myblock}].damage|=|"" call #damage|{blocks[{l_RkRUoQ_myblock}].damage}|{blocks[{l_RkRUoQ_myblock}].damageType}
		ifnot PlayerCoords|=|l_fAktGz_PrevPlayerCoords set usingWorkbench false
		ifnot PlayerCoords|=|l_fAktGz_PrevPlayerCoords set usingStonecutter false
		set l_fAktGz_PrevPlayerCoords {PlayerCoords}
		delay 100
		if debug cpemsg top1 &c{actionCount}/60000
		if hp|=|l_gUyslR_prevhp jump #ifnot_UfzNqh
			set l_gUyslR_prevhp {hp}
			call #makebar|l_mhDMiE_hpbar|c|{hp}|{maxhp}
			cpemsg bot1 &c♥ {l_mhDMiE_hpbar}
		#ifnot_UfzNqh
		...
		ifnot iframes|>|0 jump #if_serQQk
			setsub iframes 1
			ifnot iframes|<|2 gui barColor #ff0000 0.25
			if iframes|<|2 gui barSize 0
			else gui barSize 1
		#if_serQQk
		ifnot fireticks|>|0 jump #if_ZhWbEc
			setsub fireticks 1
			set l_XiGhwD_firetickmod {fireticks}
			setmod l_XiGhwD_firetickmod 10
			ifnot l_XiGhwD_firetickmod|=|0 jump #if_DqymVk
				ifnot fireticks|>|0 jump #if_QkZYDC
					set l_KizJMq_temp {fireticks}
					setdiv l_KizJMq_temp 10
					call #makecharbar|l_XSJcjQ_firebar|▐|6|{l_KizJMq_temp}|10
					cpemsg smallannounce {l_XSJcjQ_firebar}
				#if_QkZYDC
...
#mine
	...
	ifnot minetimer|>|0 jump #if_OUzlGv
...
#give
	ifnot isTool({runArg1}) jump #if_IpkGGW
		set {runArg1} {runArg2}
		quit
	#if_IpkGGW
...
#giveall
	...
	#while_pgMIgb
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_pgMIgb
quit

#place
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_ElPOmQ
	...
	if blocks[{id}].mergeInto|=|"" jump #ifnot_OJucxu
		ifnot PlayerHeldBlock|=|blocks[{id}].merger jump #if_BuoDqT
			ifnot blocks[{id}].mergeFace|=|click.face jump #if_CwZXwB
	...
	ifnot blocks[{PlayerHeldBlock}].grounded jump #if_fjlgWa
		setsub y 1
		call #getblock|id|{x}|{y}|{z}
...
#itemuse
	if blocks[{PlayerHeldBlock}].food|=|"" jump #ifnot_uCHdWR
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
		ifnot inventory[{PlayerHeldBlock}]|>|0 quit
		ifnot hp|<|maxhp jump #if_rtxgQl
			call #take|{playerHeldBlock}|1
...
#setblock
	if allowMapChanges jump #ifnot_haFCDj
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_BYJQuh
		#while_ASHzhf
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_ASHzhf
	#if_BYJQuh
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_KBdxAt
		#while_xyZLcR
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_xyZLcR
	#if_KBdxAt
quit
...
// package, char, color, amount, max
	set i 0
	set {runArg1} &{runArg3}
	ifnot i|<|{runArg4} jump #if_XXJEZb
		#while_fLrXvV
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg4} jump #while_fLrXvV
	#if_XXJEZb
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg5} jump #if_WvlEMX
		#while_SXTccc
			set {runArg1} {{runArg1}}{runArg2}
			setadd i 1
		if i|<|{runArg5} jump #while_SXTccc
	#if_WvlEMX
quit

#input
	ifnot runArg1|=|"craft" jump #if_jkTUTV
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_KalZbT
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_TfWHoT
				msg &cInvalid item name or ID
				quit
			#if_TfWHoT
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_ZSiZxh
		...
		if usingWorkbench jump #ifnot_eaRDPa
		...
		#while_urIbEm
			...
			set ingrediantList
			ifnot canAfford jump #if_pyjUIo
				...
				#while_nmyItu
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_nmyItu
				msg {ingrediantList}
			#if_pyjUIo
			setadd i 1
		if i|<|{recipes.Length} jump #while_urIbEm
	...
	#while_lXvfnv
		...
		setadd i 1
	if i|<|{blocks.Length} jump #while_lXvfnv
	msg &eTools:
...
#doCraft
	...
	#while_komEgc
		...
		call #take|{id}|{count}
		setadd j 1
	if j|<|{recipes[{recipeID}].ingredients.Length} jump #while_komEgc
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_ouarzB
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_ouarzB
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_thPbPq
	...
	#while_sWkPdz
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_sWkPdz
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_Xewxuo
		set {runArg1} {runArg2}
		quit
	#ifnot_Xewxuo
	set i 0
	#while_sEDwPv
		ifnot blocks[{i}].name|=|runArg2 jump #if_RlqZBY
			set {runArg1} {i}
			quit
		#if_RlqZBY
		setadd i 1
	if i|<|{blocks.Length} jump #while_sEDwPv
quit
...
#getRecipeByOutput
	...
	set i 0
	#while_heNovA
		ifnot recipes[{i}].output.id|=|bid jump #if_tanNAe
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_Cuismf
				set {pname} {i}
				quit
			#if_Cuismf
		#if_tanNAe
		setadd i 1
	if i|<|{recipes.Length} jump #while_heNovA
quit
...
#use[67]
	if blocks[{PlayerHeldBlock}].campfireLighter|=|"" jump #ifnot_KdWAAr
		ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_OauxXX
...
#use[70:80]
#use[68:80]
	ifnot inventory[80]|>|0 jump #if_zKIgXu
		call #take|80|1
		call #give|70|1
	#if_zKIgXu
quit
