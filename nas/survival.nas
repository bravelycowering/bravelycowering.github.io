...
#tick
	...
	if hp|=|prevhp jump #ifnot_nxlZVotTNbWqwjRV
		set prevhp {hp}
		call #makebar|hpbar|c|{hp}|{maxhp}
		cpemsg bot1 &c♥ {hpbar}
	#ifnot_nxlZVotTNbWqwjRV
	...
	ifnot iframes|>|0 jump #if_vrVhOPTsMNSobBRE
		setsub iframes 1
...
#damage
	...
	set iframes 4
	cs me ow
	ifnot hp|<|0 jump #if_HSneKGdRxgQMeQTf
...
#mine
	...
	ifnot minetimer|>|0 jump #if_UMtVvLdijEahxZPm
	...
	#if_UMtVvLdijEahxZPm
	set minepos
	if toomuch jump #ifnot_hbPvOBfqSdoDmXhO
		if label #loot[{id}] call #loot[{id}]
		else call #give|{id}|1
	#ifnot_hbPvOBfqSdoDmXhO
...
#give
	ifnot isTool({runArg1}) jump #if_TLmghrWXmaScsubv
		set {runArg1} {runArg2}
		quit
	#if_TLmghrWXmaScsubv
...
#place
	...
	if blocks[{PlayerHeldBlock}].replaceable jump #ifnot_ZwqGIMMaLuKeeasN
		ifnot inventory[{PlayerHeldBlock}]|>|0 msg &cYou don't have any &f{blocks[{PlayerHeldBlock}].name}!
	#ifnot_ZwqGIMMaLuKeeasN
	ifnot inventory[{PlayerHeldBlock}]|>|0 jump #if_DSdSXZQcjJcSRGcR
...
// package, color, amount, max
	set i 0
	set {runArg1} &{runArg2}
	ifnot i|<|{runArg3} jump #if_QmNBgEQSgePTgtqN
		#while_cNPwTbkJYSRiWlaL
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg3} jump #while_cNPwTbkJYSRiWlaL
	#if_QmNBgEQSgePTgtqN
	set {runArg1} {{runArg1}}&0
	ifnot i|<|{runArg4} jump #if_TMwhkcXoaHGKQPKo
		#while_WqitVJXzxoVilqKI
			set {runArg1} {{runArg1}}|
			setadd i 1
		if i|<|{runArg4} jump #while_WqitVJXzxoVilqKI
	#if_TMwhkcXoaHGKQPKo
quit

#input
	ifnot runArg1|=|"craft" jump #if_GVJMhQqIuGQONgdT
		set craftArgs {runArg2}
		if craftArgs|=|"" jump #ifnot_HYaGamOUAoZYKvxu
			set craftArgs[1] 1
			setsplit craftArgs *
			if isTool({craftArgs[0]}) set craftArgs[1] 1
			call #getBlockByName|blockID|{craftArgs[0]}
			ifnot blockID|=|"" jump #if_lfjbdtyNEszoslAf
				msg &cInvalid item name or ID
				quit
			#if_lfjbdtyNEszoslAf
			call #getRecipeByOutput|recipeID|{blockID}|{craftArgs[1]}
			ifnot recipeID|=|"" jump #if_BsCRdnPbQoHIkqYa
				msg &cYou do not have the materials to craft {blocks[{blockID}].name}!
				quit
			#if_BsCRdnPbQoHIkqYa
			call #doCraft|{recipeID}|{craftArgs[1]}
			quit
		#ifnot_HYaGamOUAoZYKvxu
		if usingWorkbench msg &eWorkbench Recipes:
		else msg &eRecipes:
		set i 0
		#while_NNHkDEWoLakPWhes
			call #checkRecipeAfford|{i}|canAfford|1
			set ingrediantList
			ifnot canAfford jump #if_PpwVXkRuqBaSqerb
				...
				#while_UNgMoJObfURXsPvT
					...
					setadd j 1
				if j|<|{recipes[{i}].ingredients.Length} jump #while_UNgMoJObfURXsPvT
				msg {ingrediantList}
			#if_PpwVXkRuqBaSqerb
			setadd i 1
		if i|<|{recipes.Length} jump #while_NNHkDEWoLakPWhes
		msg &eType &a/in craft [name]&e to craft something
		quit
	#if_GVJMhQqIuGQONgdT
	set i 0
	msg &eResources:
	#while_ZYHtcmBOSzkSVNYI
		ifnot inventory[{i}]|=|0 msg &f> &6{blocks[{i}].name}&f (x{inventory[{i}]})
		setadd i 1
	if i|<|{blocks.Length} jump #while_ZYHtcmBOSzkSVNYI
	msg &eTools:
...
#checkRecipeAfford
	set j 0
	set {runArg2} false
	if recipes[{runArg1}].condition|=|"" jump #ifnot_tYaYSjSBNrAjRnWt
		ifnot {recipes[{runArg1}].condition} quit
	#ifnot_tYaYSjSBNrAjRnWt
	ifnot isTool({recipes[{runArg1}].output.id}) jump #if_McvXoGrMYHanoxHm
		if {recipes[{runArg1}].output.id}|>=|recipes[{runArg1}].output.count quit
	#if_McvXoGrMYHanoxHm
	#while_bTlcBrQoKlbdpHLB
		...
		setadd j 1
	if j|<|{recipes[{runArg1}].ingredients.Length} jump #while_bTlcBrQoKlbdpHLB
	set {runArg2} true
quit

#getBlockByName
	set {runArg1}
	if blocks[{runArg2}].name|=|"" jump #ifnot_JySJUvKWVosfIsld
		set {runArg1} {runArg2}
		quit
	#ifnot_JySJUvKWVosfIsld
	set i 0
	#while_jsBhJIXyUOHkSWLJ
		ifnot blocks[{i}].name|=|runArg2 jump #if_xhCRsTEmObfJPLBb
			set {runArg1} {i}
			quit
		#if_xhCRsTEmObfJPLBb
		setadd i 1
	if i|<|{blocks.Length} jump #while_jsBhJIXyUOHkSWLJ
quit
...
#getRecipeByOutput
	...
	set {pname}
	set i 0
	#while_ZQRZRYBtEXvIMoPa
		ifnot recipes[{i}].output.id|=|bid jump #if_ukDTzqAeLgnbLcnT
			call #checkRecipeAfford|{i}|canAfford|{c}
			ifnot canAfford jump #if_fQLOeLBsYVudZOCC
				set {pname} {i}
				quit
			#if_fQLOeLBsYVudZOCC
		#if_ukDTzqAeLgnbLcnT
		setadd i 1
	if i|<|{recipes.Length} jump #while_ZQRZRYBtEXvIMoPa
quit
